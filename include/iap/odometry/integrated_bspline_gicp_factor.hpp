...
class IntegratedSplineGICPFactor : public gtsam::NonlinearFactor {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  using shared_ptr = std::shared_ptr<IntegratedSplineGICPFactor>;
  using JacobianMode = IntegratedBSplineGICPFactor::JacobianMode;
  using RobustKernel = IntegratedBSplineGICPFactor::RobustKernel;
  using ProfilingStats = IntegratedBSplineGICPFactor::ProfilingStats;
  using LinearizationCheckResult = IntegratedBSplineGICPFactor::LinearizationCheckResult;
  using NumericReferenceCheckResult = IntegratedBSplineGICPFactor::NumericReferenceCheckResult;
  using DegeneracyThresholds = IntegratedBSplineGICPFactor::DegeneracyThresholds;
  using DegeneracyDiagnostics = IntegratedBSplineGICPFactor::DegeneracyDiagnostics;
  ...
  const std::vector<int>& point_indices() const { return ctx_.point_indices; }
  const ProfilingStats& last_profiling_stats() const { return last_profile_; }
  int num_inliers() const { return static_cast<int>(accepted_inlier_count_); }
  double inlier_fraction() const;
  std::vector<Eigen::Vector4d> deskewed_source_points(const gtsam::Values& values, bool local = true) const;
 ...
 private:
  ...
  std::array<gtsam::Pose3, kBSplineControlPointCount> control_poses(const gtsam::Values& values) const;
  void update_pose(const gtsam::Values& values) const;
  void update_correspondences() const;
  PointJacobianArray point_jacobians(
    const std::array<gtsam::Pose3, kBSplineControlPointCount>& control_poses,
    const Eigen::Vector3d& source_point) const;
  ...
  std::shared_ptr<const gtsam_points::iVox> target_;
  std::shared_ptr<const gtsam_points::PointCloud> source_;
  std::array<double, kBSplineControlPointCount> basis_weights_{};

  mutable gtsam::Pose3 bucket_pose_;
  mutable PoseJacobianArray pose_jacobians_{};
  mutable Eigen::Vector4d blended_quaternion_coeffs_ = Eigen::Vector4d::Zero();
  mutable QuaternionJacobianArray blend_quaternion_jacobians_{};
  mutable std::vector<long> correspondences_;
  mutable std::vector<Eigen::Matrix3d> mahalanobis_;
  mutable std::vector<double> correspondence_sq_distances_;
  mutable std::vector<double> correspondence_best_scores_;
