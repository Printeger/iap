/*
 * Coco-LIC: Continuous-Time Tightly-Coupled LiDAR-Inertial-Camera
 * Odometry using Non-Uniform B-spline 
...
class Trajectory : public Se3Spline<SplineOrder, double> {
 public:
  ...
  // Get UWB pose at a specific time
  SE3d GetUWBPoseNURBS(int64_t timestamp) const {
    SE3d T_ItoG = GetIMUPoseNURBS(timestamp);
    SE3d T_UtoI = GetSensorEP(UWBSensor).se3;
    return T_ItoG * T_UtoI;
  }
