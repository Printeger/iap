...
int main(int argc, char** argv) {
  ...
  node->declare_parameter("hover_duration", 0.0);
  ...
  const double hover_duration =
      nonnegative_or_default(node->get_parameter("hover_duration").as_double(), 0.0);
  ...
  RCLCPP_INFO(
      node->get_logger(),
      "circle command: center=(%.2f, %.2f, %.2f), radius=%.2f, period=%.2f, hover=%.2f, rate=%.2f",
      center_x,
      center_y,
      center_z,
      radius,
  ...
  while (rclcpp::ok()) {
    ...
    const bool hovering = elapsed < hover_duration;
    const double circle_t = hovering ? 0.0 : elapsed - hover_duration;
    ...
    cmd.velocity.x = hovering ? 0.0 : -radius * omega * s;
    cmd.velocity.y = hovering ? 0.0 : radius * omega * c;
    cmd.velocity.z = 0.0;
    cmd.acceleration.x = hovering ? 0.0 : -radius * omega * omega * c;
    cmd.acceleration.y = hovering ? 0.0 : -radius * omega * omega * s;
    cmd.acceleration.z = 0.0;

    if (yaw_mode == "tangent") {
      cmd.yaw = phase + kPi / 2.0;
      cmd.yaw_dot = hovering ? 0.0 : omega;
