...
std::string summarize_keys(const gtsam::KeyVector& raw_keys, std::size_t limit);
gtsam::KeyVector support_pose_keys(const iap::SplineLocalSupport& support);
...
BSplineGravityStateMode parse_gravity_state_mode(const std::string& mode, bool* used_fallback = nullptr) {
  if (used_fallback) {
    *used_fallback = false;
  }

  if (mode == "external_reference" || mode == "EXTERNAL_REFERENCE" || mode == "external-reference") {
    return BSplineGravityStateMode::EXTERNAL_REFERENCE;
  }
  if (mode == "shared_optimized" || mode == "SHARED_OPTIMIZED" || mode == "shared-optimized") {
    return BSplineGravityStateMode::SHARED_OPTIMIZED;
  }
...
const char* to_string(BSplineGravityStateMode mode) {
  switch (mode) {
    case BSplineGravityStateMode::SHARED_OPTIMIZED:
      return "shared_optimized";
    case BSplineGravityStateMode::EXTERNAL_REFERENCE:
      return "external_reference";
  }
  return "shared_optimized";
}

BSplineGravityReferenceSource parse_gravity_reference_source(const std::string& source, bool* used_fallback = nullptr) {
  if (used_fallback) {
    *used_fallback = false;
  }

  if (source == "config_vector" || source == "CONFIG_VECTOR" || source == "config-vector") {
    return BSplineGravityReferenceSource::CONFIG_VECTOR;
  }
  if (source == "startup_seed" || source == "STARTUP_SEED" || source == "startup-seed") {
    return BSplineGravityReferenceSource::STARTUP_SEED;
  }
...
const char* to_string(BSplineGravityReferenceSource source) {
  switch (source) {
    case BSplineGravityReferenceSource::STARTUP_SEED:
      return "startup_seed";
    case BSplineGravityReferenceSource::CONFIG_VECTOR:
      return "config_vector";
  }
  return "startup_seed";
}

BSplineVelocityStateMode parse_velocity_state_mode(const std::string& mode, bool* used_fallback = nullptr) {
  if (used_fallback) {
    *used_fallback = false;
  }

  if (mode == "keep_but_not_optimize" ||
      mode == "KEEP_BUT_NOT_OPTIMIZE" ||
      mode == "keep-but-not-optimize") {
    return BSplineVelocityStateMode::KEEP_BUT_NOT_OPTIMIZE;
  }
  if (mode == "optimize" || mode == "OPTIMIZE") {
    return BSplineVelocityStateMode::OPTIMIZE;
  }
...
const char* to_string(BSplineVelocityStateMode mode) {
  switch (mode) {
    case BSplineVelocityStateMode::OPTIMIZE:
      return "optimize";
    case BSplineVelocityStateMode::KEEP_BUT_NOT_OPTIMIZE:
      return "keep_but_not_optimize";
  }
  return "optimize";
}

BSplineVelocityModePolicy parse_velocity_mode_policy(const std::string& mode, bool* used_fallback = nullptr) {
  if (used_fallback) {
    *used_fallback = false;
  }

  if (mode == "auto_disable_without_gnss" ||
      mode == "AUTO_DISABLE_WITHOUT_GNSS" ||
      mode == "auto-disable-without-gnss") {
    return BSplineVelocityModePolicy::AUTO_DISABLE_WITHOUT_GNSS;
  }
  if (mode == "always_optimize" || mode == "ALWAYS_OPTIMIZE" || mode == "always-optimize") {
    return BSplineVelocityModePolicy::ALWAYS_OPTIMIZE;
  }
...
const char* to_string(BSplineVelocityModePolicy mode) {
  switch (mode) {
    case BSplineVelocityModePolicy::ALWAYS_OPTIMIZE:
      return "always_optimize";
    case BSplineVelocityModePolicy::AUTO_DISABLE_WITHOUT_GNSS:
      return "auto_disable_without_gnss";
  }
  return "always_optimize";
}

BSplineBiasStateMode parse_bias_state_mode(const std::string& mode, bool* used_fallback = nullptr) {
  if (used_fallback) {
    *used_fallback = false;
  }

  if (mode == "lagged_keyed" || mode == "LAGGED_KEYED" || mode == "lagged-keyed") {
    return BSplineBiasStateMode::LAGGED_KEYED;
  }
  if (mode == "shared_singleton" || mode == "SHARED_SINGLETON" || mode == "shared-singleton") {
    return BSplineBiasStateMode::SHARED_SINGLETON;
  }
...
const char* to_string(BSplineBiasStateMode mode) {
  switch (mode) {
    case BSplineBiasStateMode::SHARED_SINGLETON:
      return "shared_singleton";
    case BSplineBiasStateMode::LAGGED_KEYED:
      return "lagged_keyed";
  }
  return "shared_singleton";
}

iap::CTLocalFrontend::FrontendSeedMode parse_frontend_seed_mode(
  ...
  if (mode == "imu_forward_prediction" ||
    ...
    return iap::CTLocalFrontend::FrontendSeedMode::IMU_FORWARD_PREDICTION;
  }
  if (mode == "last_pose_copy" || mode == "LAST_POSE_COPY" || mode == "last-pose-copy") {
    return iap::CTLocalFrontend::FrontendSeedMode::LAST_POSE_COPY;
  }
...
std::string yaw_isolation_experiment_name_from_flags(
  ...
  const bool disable_velocity_factor,
...
const char* frontend_frame_profile_csv_header() {
  return "frame_id,stamp,frontend_mode,frontend_only_mode,use_legacy_two_stage_path,frontend_seed_mode,"
         "frontend_seed_source,frontend_seed_fallback_used,frontend_seed_imu_sample_count,"
         "bucket_mode,actual_bucket_count,total_source_points,"
...
const char* jump_diagnostics_csv_header() {
  return "frame_id,frame_stamp,raw_frame_stamp,scan_begin_time,scan_end_time,frontend_target_time,bucket_query_time,"
         ...
         "delta_start_to_frontend_pitch_rad,delta_start_to_frontend_roll_rad,"
         "delta_frontend_to_postsolve_query_translation_norm,delta_frontend_to_postsolve_query_rotation_rad,"
         "delta_frontend_to_postsolve_query_pitch_rad,delta_frontend_to_postsolve_query_roll_rad,"
         "delta_frontend_to_postsolve_strict_local_translation_norm,delta_frontend_to_postsolve_strict_local_rotation_rad,"
         "delta_frontend_to_postsolve_strict_local_pitch_rad,delta_frontend_to_postsolve_strict_local_roll_rad,"
         "delta_postsolve_query_to_final_translation_norm,delta_postsolve_query_to_final_rotation_rad,"
         "delta_postsolve_query_to_final_pitch_rad,delta_postsolve_query_to_final_roll_rad,"
         "delta_postsolve_strict_local_to_final_translation_norm,delta_postsolve_strict_local_to_final_rotation_rad,"
         "delta_postsolve_strict_local_to_final_pitch_rad,delta_postsolve_strict_local_to_final_roll_rad,"
         ...
         "delta_frontend_to_final_yaw_rad,delta_frontend_to_final_pitch_rad,delta_frontend_to_final_roll_rad,"
         "delta_frontend_to_final_dx,delta_frontend_to_final_dy,delta_frontend_to_final_dz,"
         "current_velocity_norm,current_velocity_heading_rad,current_velocity_heading_valid,"
         "velocity_factor_count,prior_factor_count,uses_shared_imu_state,"
         "frontend_world_to_lidar_yaw,frontend_world_to_imu_yaw,final_world_to_lidar_yaw,final_world_to_imu_yaw,"
         "lidar_to_imu_extrinsic_yaw,yaw_chain_consistency_flag,"
         "gyro_bias_norm,accel_bias_norm,gravity_world_x,gravity_world_y,gravity_world_z,gravity_dir_tilt_rad,"
         ...
         "postsolve_strict_local_value_source_kind,postsolve_value_source_consistent,"
         "postsolve_query_frame_convention_kind,postsolve_extrinsic_application_kind,"
         "final_materialization_source_kind,new_frame_consumes_final_truth_only,"
         "strict_local_query_support_key_count,strict_local_query_support_keys_summary,strict_local_query_reason,"
         ...
         "carried_boundary_oldest_key_summary,oldest_survivor_key_summary,"
         "retained_pose_control_key_count,strict_local_needed_support_key_count,"
         "retained_minus_strict_local_key_excess,carried_prior_retained_keys_summary,"
         "uses_local_lidar_layout_override,"
...
gtsam::Pose3 pose3_from_isometry(const Eigen::Isometry3d& pose) {
...
PoseResidualComponents decompose_pose_residual(
  const gtsam::Pose3& reference_pose,
  const gtsam::Pose3& target_pose) {
  PoseResidualComponents result;
  const auto translation_delta = target_pose.translation() - reference_pose.translation();
  result.dx = translation_delta.x();
  result.dy = translation_delta.y();
  result.dz = translation_delta.z();

  // Compare the absolute yaw/pitch/roll coordinates of the two poses instead of
  // expanding the relative rotation matrix directly. This keeps the reported
  // components near the minimal residual around the current operating point and
  // avoids equivalent-but-large Euler representations near pi.
  result.yaw_rad =
    normalize_angle_rad(target_pose.rotation().yaw() - reference_pose.rotation().yaw());
  result.pitch_rad =
    normalize_angle_rad(target_pose.rotation().pitch() - reference_pose.rotation().pitch());
  result.roll_rad =
    normalize_angle_rad(target_pose.rotation().roll() - reference_pose.rotation().roll());
  return result;
}

double planar_velocity_norm(const Eigen::Vector3d& velocity_world_imu) {
  return std::hypot(velocity_world_imu.x(), velocity_world_imu.y());
}

double velocity_heading_rad(const Eigen::Vector3d& velocity_world_imu) {
  return normalize_angle_rad(std::atan2(velocity_world_imu.y(), velocity_world_imu.x()));
}

double pose_yaw_rad(const gtsam::Pose3& pose) {
  return normalize_angle_rad(pose.rotation().yaw());
}

double gravity_dir_tilt_rad(const gtsam::Vector3& gravity_world) {
  const double gravity_norm = gravity_world.norm();
  if (!(gravity_norm > 1e-9) || !std::isfinite(gravity_norm)) {
    return 0.0;
  }
  const auto gravity_unit = gravity_world / gravity_norm;
  const double cos_theta = std::clamp(static_cast<double>(gravity_unit.dot(gtsam::Vector3::UnitZ())), -1.0, 1.0);
  return std::acos(cos_theta);
}

std::string classify_yaw_chain_consistency_flag(
  const double frontend_world_to_lidar_yaw,
  const double frontend_world_to_imu_yaw,
  const double final_world_to_lidar_yaw,
  const double final_world_to_imu_yaw,
  const double lidar_to_imu_extrinsic_yaw) {
  ...
  const double frontend_imu_delta =
    normalize_angle_rad(final_world_to_imu_yaw - frontend_world_to_imu_yaw);
  const double frontend_lidar_delta =
    normalize_angle_rad(final_world_to_lidar_yaw - frontend_world_to_lidar_yaw);
  const double interpretation_shift =
    std::abs(normalize_angle_rad(frontend_imu_delta - frontend_lidar_delta));
  if (interpretation_shift > 0.10) {
    return "frontend_final_interpretation_shift";
  }

  const double frontend_chain_gap = std::abs(normalize_angle_rad(
    (frontend_world_to_imu_yaw - frontend_world_to_lidar_yaw) - lidar_to_imu_extrinsic_yaw));
  const double final_chain_gap = std::abs(normalize_angle_rad(
    (final_world_to_imu_yaw - final_world_to_lidar_yaw) - lidar_to_imu_extrinsic_yaw));
  if (frontend_chain_gap > 0.10 || final_chain_gap > 0.10) {
    return "lidar_imu_chain_mismatch";
  }

  const double absolute_delta_gap =
    std::abs(std::abs(frontend_imu_delta) - std::abs(frontend_lidar_delta));
  if (std::abs(lidar_to_imu_extrinsic_yaw) > 0.05 && absolute_delta_gap > 0.10) {
    return "extrinsic_yaw_suspicious";
  }

  return "none";
}

void populate_frontend_final_residual_components(
  glim::OdometryEstimationBSpline::JumpDiagnosticsRow* row,
  const gtsam::Pose3& frontend_pose,
  const gtsam::Pose3& final_pose) {
  if (!row) {
    return;
  }

  const auto residual = decompose_pose_residual(frontend_pose, final_pose);
  row->delta_frontend_to_final_yaw_rad = residual.yaw_rad;
  row->delta_frontend_to_final_pitch_rad = residual.pitch_rad;
  row->delta_frontend_to_final_roll_rad = residual.roll_rad;
  row->delta_frontend_to_final_dx = residual.dx;
  row->delta_frontend_to_final_dy = residual.dy;
  row->delta_frontend_to_final_dz = residual.dz;
}

void populate_pose_residual_component_pair(
  const gtsam::Pose3& source_pose,
  const gtsam::Pose3& target_pose,
  double* pitch_rad,
  double* roll_rad) {
  if (!pitch_rad || !roll_rad) {
    return;
  }
  const auto residual = decompose_pose_residual(source_pose, target_pose);
  *pitch_rad = residual.pitch_rad;
  *roll_rad = residual.roll_rad;
}

void populate_yaw_residual_shootout_probes(
  glim::OdometryEstimationBSpline::JumpDiagnosticsRow* row,
  const gtsam::Pose3& frontend_pose_lidar,
  const gtsam::Pose3& frontend_pose_imu,
  const gtsam::Pose3& final_pose_lidar,
  const gtsam::Pose3& final_pose_imu,
  const Eigen::Vector3d& current_velocity_world_imu,
  const std::size_t velocity_factor_count,
  const std::size_t prior_factor_count,
  const bool uses_shared_imu_state,
  const iap::BSplineFixedLagSharedState& shared_state,
  const gtsam::Pose3& T_lidar_imu_pose) {
  if (!row) {
    return;
  }

  row->current_velocity_norm = current_velocity_world_imu.norm();
  const double horizontal_speed = planar_velocity_norm(current_velocity_world_imu);
  row->current_velocity_heading_valid = horizontal_speed >= 0.10;
  row->current_velocity_heading_rad = row->current_velocity_heading_valid
    ? velocity_heading_rad(current_velocity_world_imu)
    : 0.0;
  row->velocity_factor_count = velocity_factor_count;
  row->prior_factor_count = prior_factor_count;
  row->uses_shared_imu_state = uses_shared_imu_state;

  row->frontend_world_to_lidar_yaw = pose_yaw_rad(frontend_pose_lidar);
  row->frontend_world_to_imu_yaw = pose_yaw_rad(frontend_pose_imu);
  row->final_world_to_lidar_yaw = pose_yaw_rad(final_pose_lidar);
  row->final_world_to_imu_yaw = pose_yaw_rad(final_pose_imu);
  row->lidar_to_imu_extrinsic_yaw = pose_yaw_rad(T_lidar_imu_pose);
  row->yaw_chain_consistency_flag = classify_yaw_chain_consistency_flag(
    row->frontend_world_to_lidar_yaw,
    row->frontend_world_to_imu_yaw,
    row->final_world_to_lidar_yaw,
    row->final_world_to_imu_yaw,
    row->lidar_to_imu_extrinsic_yaw);

  row->gyro_bias_norm = shared_state.gyro_bias.norm();
  row->accel_bias_norm = shared_state.accel_bias.norm();
  row->gravity_world_x = shared_state.gravity.x();
  row->gravity_world_y = shared_state.gravity.y();
  row->gravity_world_z = shared_state.gravity.z();
  row->gravity_dir_tilt_rad = gravity_dir_tilt_rad(shared_state.gravity);
}

std::pair<double, double> layout_domain_bounds(const std::shared_ptr<const iap::SplineStateLayout>& layout) {
  if (!layout) {
    return {0.0, 0.0};
  }
  ...
  if (controls.size() < iap::kBSplineControlPointCount ||
      knots.size() != controls.size() + iap::kBSplineControlPointCount) {
    return {0.0, 0.0};
  }
...
gtsam::Pose3 evaluate_pose_from_layout(
  ...
  auto evaluator = std::make_shared<iap::SplineEvaluator>(layout);
  return evaluator->eval_pose(values, *support, sensor);
}

struct LayoutQueryDiagnostics {
  bool layout_available{false};
  bool query_time_in_domain{false};
...
LayoutQueryDiagnostics inspect_layout_query(
  ...
  double query_time,
  iap::SplineSensorId sensor) {
  LayoutQueryDiagnostics diagnostics;
  diagnostics.layout_available = static_cast<bool>(layout);
  const auto [domain_begin, domain_end] = layout_domain_bounds(layout);
  diagnostics.domain_begin = domain_begin;
  diagnostics.domain_end = domain_end;
  diagnostics.query_time_in_domain =
    diagnostics.layout_available &&
    domain_begin <= query_time + 1e-9 &&
    query_time <= domain_end + 1e-9;
  if (!layout) {
    return diagnostics;
  }
  if (const auto support = layout->support_at(query_time, sensor)) {
    diagnostics.has_support = true;
...
std::string classify_strict_local_query_reason(
  const LayoutQueryDiagnostics& strict_local_query,
  const gtsam::KeyVector& frontend_support_keys,
  const bool used_fallback_legacy_eval) {
  if (!strict_local_query.layout_available) {
  ...
  if (is_contiguous_support_shift(frontend_support, strict_support)) {
    return "boundary_shift";
  }
...
void populate_start_pose_support_diagnostics(
  glim::OdometryEstimationBSpline::JumpDiagnosticsRow* row,
  const LayoutQueryDiagnostics& start_layout,
  const LayoutQueryDiagnostics& strict_layout) {
  if (!row) {
    return;
  }

  row->start_pose_support_key_count = start_layout.support_keys.size();
  row->start_pose_support_keys_summary = summarize_keys(start_layout.support_keys, 8);
  row->start_pose_support_mismatch_flag = false;
  row->start_pose_support_mismatch_reason = "none";

  if (!strict_layout.layout_available) {
    row->start_pose_support_mismatch_flag = true;
    row->start_pose_support_mismatch_reason = "strict_layout_unavailable";
    return;
  }

  if (!strict_layout.query_time_in_domain || !strict_layout.has_support) {
    row->start_pose_support_mismatch_flag = true;
    row->start_pose_support_mismatch_reason = "query_time_layout_domain_mismatch";
    return;
  }

  if (sort_unique_keys(start_layout.support_keys) != sort_unique_keys(strict_layout.support_keys)) {
    row->start_pose_support_mismatch_flag = true;
...
void write_frontend_frame_profile_row(std::FILE* file, const iap::FrontendFrameProfile& profile) {
  ...
  add(csv_escape(profile.frontend_seed_mode));
  add(csv_escape(profile.frontend_seed_source));
  add(profile.frontend_seed_fallback_used ? 1 : 0);
  add(profile.frontend_seed_imu_sample_count);
...
void write_jump_diagnostics_row(std::FILE* file, const glim::OdometryEstimationBSpline::JumpDiagnosticsRow& row_data) {
  if (!file) {
    return;
  }
  ...
  add(row_data.frontend_pose_query_time);
  add(row_data.current_segment_id);
  add(csv_escape(row_data.start_pose_source_kind));
  add(row_data.start_pose_frozen_before_factor_injection ? 1 : 0);
  add(row_data.start_pose_frozen_before_solver_update ? 1 : 0);
  add(csv_escape(row_data.frontend_seed_mode));
  add(csv_escape(row_data.frontend_seed_source));
  add(row_data.frontend_seed_fallback_used ? 1 : 0);
  add(row_data.frontend_seed_imu_sample_count);
  add(row_data.start_pose_tx);
  add(row_data.start_pose_ty);
  add(row_data.start_pose_tz);
  add(row_data.start_pose_qx);
  add(row_data.start_pose_qy);
  add(row_data.start_pose_qz);
  add(row_data.start_pose_qw);
  add(row_data.frontend_pose_tx);
  add(row_data.frontend_pose_ty);
  add(row_data.frontend_pose_tz);
  add(row_data.frontend_pose_qx);
  ...
  add(row_data.final_pose_qx);
  add(row_data.final_pose_qy);
  add(row_data.final_pose_qz);
  ...
  add(row_data.delta_start_to_frontend_pitch_rad);
  add(row_data.delta_start_to_frontend_roll_rad);
  ...
  add(row_data.delta_frontend_to_postsolve_query_pitch_rad);
  add(row_data.delta_frontend_to_postsolve_query_roll_rad);
  ...
  add(row_data.delta_frontend_to_postsolve_strict_local_pitch_rad);
  add(row_data.delta_frontend_to_postsolve_strict_local_roll_rad);
  add(row_data.delta_postsolve_query_to_final_translation_norm);
  add(row_data.delta_postsolve_query_to_final_rotation_rad);
  add(row_data.delta_postsolve_query_to_final_pitch_rad);
  add(row_data.delta_postsolve_query_to_final_roll_rad);
  ...
  add(row_data.delta_postsolve_strict_local_to_final_pitch_rad);
  add(row_data.delta_postsolve_strict_local_to_final_roll_rad);
  ...
  add(row_data.delta_postsolve_active_window_to_postsolve_strict_local_pitch_rad);
  add(row_data.delta_postsolve_active_window_to_postsolve_strict_local_roll_rad);
  add(row_data.delta_frontend_to_final_translation_norm);
  add(row_data.delta_frontend_to_final_rotation_rad);
  add(row_data.delta_frontend_to_final_yaw_rad);
  add(row_data.delta_frontend_to_final_pitch_rad);
  add(row_data.delta_frontend_to_final_roll_rad);
  add(row_data.delta_frontend_to_final_dx);
  add(row_data.delta_frontend_to_final_dy);
  add(row_data.delta_frontend_to_final_dz);
  add(row_data.current_velocity_norm);
  add(row_data.current_velocity_heading_rad);
  add(row_data.current_velocity_heading_valid ? 1 : 0);
  add(row_data.velocity_factor_count);
  add(row_data.prior_factor_count);
  add(row_data.uses_shared_imu_state ? 1 : 0);
  add(row_data.frontend_world_to_lidar_yaw);
  add(row_data.frontend_world_to_imu_yaw);
  add(row_data.final_world_to_lidar_yaw);
  add(row_data.final_world_to_imu_yaw);
  add(row_data.lidar_to_imu_extrinsic_yaw);
  add(csv_escape(row_data.yaw_chain_consistency_flag));
  add(row_data.gyro_bias_norm);
  add(row_data.accel_bias_norm);
  add(row_data.gravity_world_x);
  add(row_data.gravity_world_y);
  add(row_data.gravity_world_z);
  add(row_data.gravity_dir_tilt_rad);
  add(row_data.lidar_layout_domain_begin);
  add(row_data.lidar_layout_domain_end);
  add(row_data.start_pose_support_key_count);
  add(csv_escape(row_data.start_pose_support_keys_summary));
  add(row_data.start_pose_support_mismatch_flag ? 1 : 0);
  add(csv_escape(row_data.start_pose_support_mismatch_reason));
  add(row_data.lidar_support_key_count);
  ...
  add(csv_escape(row_data.postsolve_active_window_value_source_kind));
  ...
  add(csv_escape(row_data.postsolve_strict_local_value_source_kind));
  add(row_data.postsolve_value_source_consistent ? 1 : 0);
  add(csv_escape(row_data.postsolve_query_frame_convention_kind));
  add(csv_escape(row_data.postsolve_extrinsic_application_kind));
  add(csv_escape(row_data.final_materialization_source_kind));
  add(row_data.new_frame_consumes_final_truth_only ? 1 : 0);
  add(row_data.strict_local_query_support_key_count);
  add(csv_escape(row_data.strict_local_query_support_keys_summary));
  add(csv_escape(row_data.strict_local_query_reason));
  add(row_data.match_ratio);
  add(row_data.inlier_ratio);
  add(row_data.points_in_bucket);
  add(row_data.candidate_correspondence_count);
  add(row_data.accepted_correspondence_count);
  add(row_data.accept_ratio);
  add(row_data.registration_delta_translation_norm);
  add(row_data.registration_delta_rotation_rad);
  add(row_data.factor_total_ms);
  add(row_data.target_map_prep_ms);
  add(row_data.target_point_count);
  add(row_data.target_voxel_count);
  add(row_data.target_snapshot_clone_ms);
  add(row_data.target_voxel_lookup_prep_ms);
  ...
  add(csv_escape(row_data.oldest_survivor_key_summary));
  add(row_data.retained_pose_control_key_count);
  add(row_data.strict_local_needed_support_key_count);
  add(row_data.retained_minus_strict_local_key_excess);
  add(csv_escape(row_data.carried_prior_retained_keys_summary));
  add(row_data.uses_local_lidar_layout_override ? 1 : 0);
  add(csv_escape(row_data.frontend_pose_query_support_keys_summary));
  row << '\n';
  std::fputs(row.str().c_str(), file);
}
...
gtsam::Key bspline_gyro_bias_key(std::size_t index = 0) {
  return iap::bspline_gyro_bias_key(index);
}

gtsam::Key bspline_accel_bias_key(std::size_t index = 0) {
  return iap::bspline_accel_bias_key(index);
}
...
std::string key_to_string(gtsam::Key key) {
...
std::string summarize_keys(const gtsam::KeyVector& raw_keys, std::size_t limit = 8) {
  const auto keys = sort_unique_keys(raw_keys);
  if (keys.empty()) {
    return "";
  }

  std::ostringstream oss;
  const std::size_t display_count = std::min(limit, keys.size());
  ...
  if (keys.size() > display_count) {
    oss << ";+" << (keys.size() - display_count);
  }
  return oss.str();
}

std::string summarize_oldest_key(const gtsam::KeyVector& raw_keys) {
  const auto keys = sort_unique_keys(raw_keys);
  return keys.empty() ? std::string{} : key_to_string(keys.front());
}
...
bool is_pose_control_key(gtsam::Key key) {
  return gtsam::Symbol(key).chr() == 's';
}

std::size_t count_pose_control_keys(const gtsam::KeyVector& keys) {
  return static_cast<std::size_t>(std::count_if(keys.begin(), keys.end(), is_pose_control_key));
}
...
gtsam::KeyVector auxiliary_keys_from_indices(
  const std::vector<std::size_t>& auxiliary_indices,
  bool include_clock) {
  gtsam::KeyVector keys;
  keys.reserve(auxiliary_indices.size() * (include_clock ? 2U : 1U));
  for (const auto auxiliary_index : auxiliary_indices) {
    keys.push_back(iap::bspline_velocity_key(auxiliary_index));
    if (include_clock) {
      keys.push_back(iap::bspline_clock_key(auxiliary_index));
    }
  }
  return sort_unique_keys(std::move(keys));
}
...
bool jump_threshold_exceeded(const glim::OdometryEstimationBSpline::JumpDiagnosticsRow& row) {
  return row.delta_start_to_frontend_translation_norm > 1.0 ||
    row.delta_start_to_frontend_rotation_rad > 0.3 ||
    row.delta_frontend_to_final_translation_norm > 1.0 ||
    row.delta_frontend_to_final_rotation_rad > 0.3;
}
...
bool OdometryEstimationBSpline::gravity_graph_key_enabled() const {
  return gravity_state_mode_ == BSplineGravityStateMode::SHARED_OPTIMIZED;
}

bool OdometryEstimationBSpline::gravity_external_reference_enabled() const {
  return gravity_state_mode_ == BSplineGravityStateMode::EXTERNAL_REFERENCE;
}

bool OdometryEstimationBSpline::bias_shared_singleton_mode() const {
  return bias_state_mode_ == BSplineBiasStateMode::SHARED_SINGLETON;
}

bool OdometryEstimationBSpline::bias_lagged_keyed_mode() const {
  return bias_state_mode_ == BSplineBiasStateMode::LAGGED_KEYED;
}

gtsam::Vector3 OdometryEstimationBSpline::sanitized_gravity_vector(const gtsam::Vector3& gravity) const {
  const double gravity_norm = gravity.norm();
  if (std::isfinite(gravity_norm) && gravity_norm > 1e-9) {
    return gravity;
  }
  return gtsam::Vector3(0.0, 0.0, 9.80665);
}

void OdometryEstimationBSpline::set_external_gravity_reference(
  const gtsam::Vector3& gravity_world,
  const BSplineGravityReferenceSource source) {
  external_gravity_reference_ = sanitized_gravity_vector(gravity_world);
  gravity_reference_source_ = source;
  fixed_lag_registry_.shared_state().gravity = external_gravity_reference_;
}

gtsam::Vector3 OdometryEstimationBSpline::authoritative_gravity_reference() const {
  if (gravity_external_reference_enabled()) {
    return external_gravity_reference_;
  }
  return sanitized_gravity_vector(fixed_lag_registry_.shared_state().gravity);
}

const char* OdometryEstimationBSpline::runtime_gravity_reference_source_name() const {
  if (gravity_external_reference_enabled()) {
    return to_string(gravity_reference_source_);
  }
  return "shared_optimized_graph_state";
}

const char* OdometryEstimationBSpline::runtime_bias_source_of_truth_name() const {
  return bias_lagged_keyed_mode()
    ? "active_lagged_bias_keys"
    : "shared_singleton_registry";
}

bool OdometryEstimationBSpline::runtime_bias_transition_prior_enabled() const {
  return bias_lagged_keyed_mode();
}

double OdometryEstimationBSpline::runtime_bias_transition_prior_strength() const {
  return runtime_bias_transition_prior_enabled() ? imu_ct_bias_inf_scale_ : 0.0;
}

bool OdometryEstimationBSpline::runtime_bias_can_be_survivor_anchor() const {
  return bias_shared_singleton_mode();
}

const char* OdometryEstimationBSpline::runtime_bias_writeback_mode_name() const {
  return bias_lagged_keyed_mode()
    ? "lagged_authoritative_with_mirror_cache"
    : "shared_singleton_authoritative_writeback";
}

const char* OdometryEstimationBSpline::runtime_frontend_seed_mode_name() const {
  return iap::CTLocalFrontend::frontend_seed_mode_name(frontend_seed_mode_);
}

const char* OdometryEstimationBSpline::runtime_frontend_seed_source_name() const {
  if (runtime_frontend_seed_imu_success_observed_ && runtime_frontend_seed_fallback_used_) {
    return "mixed_imu_forward_prediction_with_fallback";
  }
  if (runtime_frontend_seed_imu_success_observed_) {
...
OdometryEstimationBSpline::FrontendSeedObservation OdometryEstimationBSpline::inspect_frontend_seed_observation(
  const double seed_end_stamp) const {
  FrontendSeedObservation observation;
  observation.mode = runtime_frontend_seed_mode_name();
  observation.source = "last_pose_copy";
  if (!imu_forward_prediction_enabled() || frames.empty() || !frames.back()) {
    return observation;
  }

  const auto samples = create_frontend_seed_imu_samples(frames.back()->stamp, seed_end_stamp);
  observation.imu_sample_count = samples.size();
  if (samples.empty()) {
    observation.source = "imu_forward_prediction_fallback_last_pose_copy";
    observation.fallback_used = true;
    return observation;
  }

  observation.source = "imu_forward_prediction";
  return observation;
}

void OdometryEstimationBSpline::record_frontend_seed_observation(
  const FrontendSeedObservation& observation) const {
  runtime_frontend_seed_observed_ = true;
  runtime_frontend_seed_fallback_used_ =
    runtime_frontend_seed_fallback_used_ || observation.fallback_used;
  runtime_frontend_seed_imu_sample_count_ =
    std::max(runtime_frontend_seed_imu_sample_count_, observation.imu_sample_count);
...
gtsam::Key OdometryEstimationBSpline::gyro_bias_key_for_auxiliary(const std::size_t auxiliary_index) const {
  return bspline_gyro_bias_key(bias_lagged_keyed_mode() ? auxiliary_index : 0U);
}

gtsam::Key OdometryEstimationBSpline::accel_bias_key_for_auxiliary(const std::size_t auxiliary_index) const {
  return bspline_accel_bias_key(bias_lagged_keyed_mode() ? auxiliary_index : 0U);
}

bool OdometryEstimationBSpline::authoritative_previous_bias_auxiliary_index(
  ...
  for (const auto& segment : fixed_lag_registry_.segments()) {
    if (segment.auxiliary_index == auxiliary_index) {
      if (have_previous_index && previous_auxiliary_index) {
        *previous_auxiliary_index = previous_index;
      }
      return have_previous_index;
    }

    if (!have_previous_index || previous_index != segment.auxiliary_index) {
      previous_index = segment.auxiliary_index;
...
gtsam::KeyVector OdometryEstimationBSpline::bias_anchorable_survivor_keys(const gtsam::KeyVector& keys) const {
  return iap::filter_bspline_survivor_anchor_keys(keys, runtime_bias_can_be_survivor_anchor());
}

void OdometryEstimationBSpline::seed_bias_values_for_auxiliary(
  const std::size_t auxiliary_index,
  ...
  const auto seed_bias = [&](gtsam::Key key, const gtsam::Vector3& fallback) {
    if (values->exists(key)) {
      return;
    }
    if (fixed_lag_registry_.auxiliary_values().exists(key)) {
      values->insert(key, fixed_lag_registry_.auxiliary_values().at<gtsam::Vector3>(key));
      return;
    }

    std::size_t previous_auxiliary_index = 0;
    if (bias_lagged_keyed_mode() &&
        authoritative_previous_bias_auxiliary_index(auxiliary_index, &previous_auxiliary_index)) {
      const gtsam::Key predecessor_key = gtsam::Symbol(key).chr() == 'j'
        ? bspline_gyro_bias_key(previous_auxiliary_index)
        : bspline_accel_bias_key(previous_auxiliary_index);
      if (values->exists(predecessor_key)) {
        values->insert(key, values->at<gtsam::Vector3>(predecessor_key));
        return;
      }
      if (fixed_lag_registry_.auxiliary_values().exists(predecessor_key)) {
        values->insert(key, fixed_lag_registry_.auxiliary_values().at<gtsam::Vector3>(predecessor_key));
        return;
      }
    }

    values->insert(key, fallback);
  };

  seed_bias(
    gyro_bias_key_for_auxiliary(auxiliary_index),
    bias_lagged_keyed_mode() ? bias_bootstrap_gyro_ : fixed_lag_registry_.shared_state().gyro_bias);
  seed_bias(
    accel_bias_key_for_auxiliary(auxiliary_index),
    bias_lagged_keyed_mode() ? bias_bootstrap_accel_ : fixed_lag_registry_.shared_state().accel_bias);
}

void OdometryEstimationBSpline::seed_bias_values_for_active_segments(gtsam::Values* values) const {
  if (!values) {
    return;
  }

  if (bias_shared_singleton_mode()) {
    if (!values->exists(bspline_gyro_bias_key())) {
      values->insert(bspline_gyro_bias_key(), fixed_lag_registry_.shared_state().gyro_bias);
    }
    if (!values->exists(bspline_accel_bias_key())) {
      values->insert(bspline_accel_bias_key(), fixed_lag_registry_.shared_state().accel_bias);
    }
    return;
  }

  for (const auto& segment : fixed_lag_registry_.segments()) {
    seed_bias_values_for_auxiliary(segment.auxiliary_index, values);
  }
}

void OdometryEstimationBSpline::mirror_bias_cache_from_values(const gtsam::Values& values) {
  if (bias_shared_singleton_mode()) {
  ...
  for (const auto& segment : fixed_lag_registry_.segments()) {
    const auto gyro_key = gyro_bias_key_for_auxiliary(segment.auxiliary_index);
    if (values.exists(gyro_key)) {
      if (fixed_lag_registry_.auxiliary_values().exists(gyro_key)) {
        fixed_lag_registry_.auxiliary_values().update(gyro_key, values.at<gtsam::Vector3>(gyro_key));
      } else {
        fixed_lag_registry_.auxiliary_values().insert(gyro_key, values.at<gtsam::Vector3>(gyro_key));
      }
    }

    const auto accel_key = accel_bias_key_for_auxiliary(segment.auxiliary_index);
    if (values.exists(accel_key)) {
      if (fixed_lag_registry_.auxiliary_values().exists(accel_key)) {
        fixed_lag_registry_.auxiliary_values().update(accel_key, values.at<gtsam::Vector3>(accel_key));
      } else {
        fixed_lag_registry_.auxiliary_values().insert(accel_key, values.at<gtsam::Vector3>(accel_key));
      }
    }
  }

  for (auto it = fixed_lag_registry_.segments().rbegin(); it != fixed_lag_registry_.segments().rend(); ++it) {
    const auto gyro_key = gyro_bias_key_for_auxiliary(it->auxiliary_index);
    const auto accel_key = accel_bias_key_for_auxiliary(it->auxiliary_index);
    bool wrote_latest_bias = false;
    if (values.exists(gyro_key)) {
      fixed_lag_registry_.shared_state().gyro_bias = values.at<gtsam::Vector3>(gyro_key);
      wrote_latest_bias = true;
    }
    if (values.exists(accel_key)) {
      fixed_lag_registry_.shared_state().accel_bias = values.at<gtsam::Vector3>(accel_key);
      wrote_latest_bias = true;
    }

    if (wrote_latest_bias) {
      break;
    }
  }
}

gtsam::Values OdometryEstimationBSpline::build_postsolve_authoritative_values(
  const gtsam::Values& solve_values,
  const bool navigation_layer_enabled) const {
  gtsam::Values authoritative_values = fixed_lag_registry_.control_buffer().values();

  for (const auto key : solve_values.keys()) {
    if (authoritative_values.exists(key)) {
      authoritative_values.update(key, solve_values.at(key));
    } else {
      authoritative_values.insert(key, solve_values.at(key));
    }
  }
  ...
  const auto predict_velocity_from_values = [&](const ActiveSplineSegmentConstraint& segment) {
    std::array<gtsam::Pose3, iap::kBSplineControlPointCount> segment_poses;
    for (std::size_t k = 0; k < iap::kBSplineControlPointCount; ++k) {
      segment_poses[k] = authoritative_values.at<gtsam::Pose3>(
        iap::bspline_control_point_key(segment.control_indices[k]));
    }
    return iap::IntegratedBSplineVelocityFactor::predict_velocity(
      segment_poses,
      0.0,
  ...
  for (const auto& segment : fixed_lag_registry_.segments()) {
    ...
    if (!authoritative_values.exists(velocity_key)) {
      authoritative_values.insert(velocity_key, predict_velocity_from_values(segment));
    }

    const gtsam::Key clock_key = iap::bspline_clock_key(segment.auxiliary_index);
    if (!authoritative_values.exists(clock_key) && solve_values.exists(clock_key)) {
      authoritative_values.insert(clock_key, solve_values.at<gtsam::Vector2>(clock_key));
    }
  }

  seed_bias_values_for_active_segments(&authoritative_values);
  return authoritative_values;
}

void OdometryEstimationBSpline::sync_postsolve_registry_mirrors(
  const gtsam::Values& postsolve_authoritative_values,
  const int frame_id,
  const bool update_shared_state_from_values) {
  if (update_shared_state_from_values) {
    fixed_lag_registry_.update_shared_state_from_values(postsolve_authoritative_values);
  }
  mirror_bias_cache_from_values(postsolve_authoritative_values);
  maybe_latch_isolation_freeze_anchor(fixed_lag_registry_.shared_state());
  apply_effective_shared_imu_state_to_registry(frame_id);

  fixed_lag_registry_.clear_auxiliary_values();
  const auto predict_velocity_from_values = [&](const ActiveSplineSegmentConstraint& segment) {
    std::array<gtsam::Pose3, iap::kBSplineControlPointCount> segment_poses;
    for (std::size_t k = 0; k < iap::kBSplineControlPointCount; ++k) {
      segment_poses[k] = postsolve_authoritative_values.at<gtsam::Pose3>(
        iap::bspline_control_point_key(segment.control_indices[k]));
    }
    return iap::IntegratedBSplineVelocityFactor::predict_velocity(
      segment_poses,
  ...
  for (const auto& segment : fixed_lag_registry_.segments()) {
    const gtsam::Key velocity_key = iap::bspline_velocity_key(segment.auxiliary_index);
    if (postsolve_authoritative_values.exists(velocity_key)) {
      fixed_lag_registry_.auxiliary_values().insert(
        velocity_key,
        postsolve_authoritative_values.at<gtsam::Vector3>(velocity_key));
    } else {
      fixed_lag_registry_.auxiliary_values().insert(velocity_key, predict_velocity_from_values(segment));
    }

    const gtsam::Key clock_key = iap::bspline_clock_key(segment.auxiliary_index);
    if (postsolve_authoritative_values.exists(clock_key)) {
      fixed_lag_registry_.auxiliary_values().insert(
        clock_key,
        postsolve_authoritative_values.at<gtsam::Vector2>(clock_key));
    }
  }
...
OdometryEstimationBSpline::PostsolveEvaluationContext
OdometryEstimationBSpline::build_postsolve_evaluation_context(
  const gtsam::Values& solve_values,
  const ActiveSplineSegmentConstraint* current_segment,
  const double query_time,
  const bool navigation_layer_enabled,
  const char* value_source_kind) const {
  PostsolveEvaluationContext context;
  context.authoritative_values = build_postsolve_authoritative_values(solve_values, navigation_layer_enabled);
  context.active_window_layout = active_window_layout_ ? active_window_layout_ : build_active_window_layout();
  if (context.active_window_layout == active_window_layout_) {
    context.active_window_evaluator = active_window_evaluator_;
  } else if (context.active_window_layout) {
    context.active_window_evaluator = std::make_shared<iap::SplineEvaluator>(context.active_window_layout);
  }
  context.strict_local_layout =
    current_segment ? create_segment_lidar_layout(*current_segment) : std::shared_ptr<const iap::SplineStateLayout>{};
  context.query_time = query_time;
  context.active_window_value_source_kind =
    (value_source_kind && *value_source_kind) ? value_source_kind : "postsolve_authoritative_values";
  context.strict_local_value_source_kind = context.active_window_value_source_kind;
  context.value_source_consistent =
    context.active_window_value_source_kind == context.strict_local_value_source_kind;
  context.frame_convention_kind =
    "query_pose=world_to_lidar;compare_pose=world_to_imu_via_right_multiply_T_lidar_imu";
  context.extrinsic_application_kind = "query_pose_lidar_right_multiply_T_lidar_imu";
  context.final_materialization_source_kind =
    "selected_final_postsolve_query_from_postsolve_authoritative_values";
  context.new_frame_consumes_final_truth_only = true;
  return context;
}

void OdometryEstimationBSpline::record_postsolve_evaluation_observation(
  const PostsolveEvaluationContext& context) const {
  const auto merge_runtime_kind = [&](std::string* current, const std::string& value) {
    if (!current || value.empty()) {
      return;
    }
    if (!runtime_postsolve_observed_ || *current == "unknown") {
      *current = value;
  ...
  merge_runtime_kind(
    &runtime_postsolve_active_window_value_source_kind_,
    context.active_window_value_source_kind);
  merge_runtime_kind(
    &runtime_postsolve_strict_local_value_source_kind_,
    context.strict_local_value_source_kind);
  merge_runtime_kind(
    &runtime_postsolve_query_frame_convention_kind_,
    context.frame_convention_kind);
  merge_runtime_kind(
    &runtime_postsolve_extrinsic_application_kind_,
    context.extrinsic_application_kind);
  merge_runtime_kind(
    &runtime_final_materialization_source_kind_,
    context.final_materialization_source_kind);

  if (!runtime_postsolve_observed_) {
    runtime_postsolve_value_source_consistent_ = context.value_source_consistent;
    runtime_new_frame_consumes_final_truth_only_ = context.new_frame_consumes_final_truth_only;
  } else {
    runtime_postsolve_value_source_consistent_ =
      runtime_postsolve_value_source_consistent_ && context.value_source_consistent;
    runtime_new_frame_consumes_final_truth_only_ =
      runtime_new_frame_consumes_final_truth_only_ && context.new_frame_consumes_final_truth_only;
  }
  runtime_postsolve_observed_ = true;
}

void OdometryEstimationBSpline::write_frame_bias_from_values(
  const gtsam::Values& values,
  const std::size_t auxiliary_index,
  ...
  const auto accel_key = accel_bias_key_for_auxiliary(auxiliary_index);
  const auto gyro_key = gyro_bias_key_for_auxiliary(auxiliary_index);
  frame->imu_bias.head<3>() = values.exists(accel_key)
    ? values.at<gtsam::Vector3>(accel_key)
    : fixed_lag_registry_.shared_state().accel_bias;
  frame->imu_bias.tail<3>() = values.exists(gyro_key)
    ? values.at<gtsam::Vector3>(gyro_key)
    : fixed_lag_registry_.shared_state().gyro_bias;
}

bool OdometryEstimationBSpline::solve_has_gnss_constraints(const ActiveSplineSegmentConstraint& segment) const {
  if (frontend_only_mode_ || !fixed_lag_registry_.shared_state().gnss_anchor_initialized) {
    return false;
  }

  return std::any_of(segment.gnss_epochs.begin(), segment.gnss_epochs.end(), [&](const auto& epoch) {
    return std::any_of(epoch.sats.begin(), epoch.sats.end(), [&](const auto& sat) {
      return !sat.excluded && sat.elevation >= gnss_min_elevation_;
    });
  });
}

bool OdometryEstimationBSpline::solve_has_gnss_constraints() const {
  return std::any_of(
    fixed_lag_registry_.segments().begin(),
    fixed_lag_registry_.segments().end(),
    [&](const auto& segment) { return solve_has_gnss_constraints(segment); });
}

bool OdometryEstimationBSpline::velocity_explicit_optimization_enabled(const bool solve_has_gnss_constraints) const {
  if (velocity_state_mode_ == BSplineVelocityStateMode::KEEP_BUT_NOT_OPTIMIZE) {
    return false;
  }
  if (velocity_mode_policy_ == BSplineVelocityModePolicy::AUTO_DISABLE_WITHOUT_GNSS &&
      !solve_has_gnss_constraints) {
    return false;
  }
  return true;
}

bool OdometryEstimationBSpline::velocity_optimized_for_solve(const bool solve_has_gnss_constraints) const {
  return velocity_explicit_optimization_enabled(solve_has_gnss_constraints) || solve_has_gnss_constraints;
}

BSplineVelocityStateMode OdometryEstimationBSpline::resolved_velocity_state_mode(
  const bool solve_has_gnss_constraints) const {
  return velocity_explicit_optimization_enabled(solve_has_gnss_constraints)
    ? BSplineVelocityStateMode::OPTIMIZE
    : BSplineVelocityStateMode::KEEP_BUT_NOT_OPTIMIZE;
}

void OdometryEstimationBSpline::record_runtime_mode_resolution(
  const bool solve_has_gnss_constraints,
  const bool velocity_explicit_optimization_enabled,
  const bool velocity_optimized) {
  runtime_has_gnss_constraints_ = runtime_has_gnss_constraints_ || solve_has_gnss_constraints;
  runtime_velocity_optimized_ = runtime_velocity_optimized_ || velocity_optimized;
  runtime_velocity_state_mode_ = velocity_explicit_optimization_enabled
    ? BSplineVelocityStateMode::OPTIMIZE
    : BSplineVelocityStateMode::KEEP_BUT_NOT_OPTIMIZE;
}

void OdometryEstimationBSpline::sync_runtime_mode_metadata(const bool force) const {
  const auto gravity_reference = authoritative_gravity_reference();
  std::ostringstream signature;
  signature << to_string(gravity_state_mode_) << '|'
            << runtime_gravity_reference_source_name() << '|'
            << std::fixed << std::setprecision(9)
            << gravity_reference.x() << ','
            << gravity_reference.y() << ','
            << gravity_reference.z() << '|'
            << to_string(bias_state_mode_) << '|'
            << runtime_bias_source_of_truth_name() << '|'
            << (runtime_bias_optimized_ ? 1 : 0) << '|'
            << (runtime_bias_transition_prior_enabled() ? 1 : 0) << '|'
            << runtime_bias_transition_prior_strength() << '|'
            << (runtime_bias_can_be_survivor_anchor() ? 1 : 0) << '|'
            << runtime_bias_writeback_mode_name() << '|'
            << runtime_frontend_seed_mode_name() << '|'
            << (imu_forward_prediction_enabled() ? 1 : 0) << '|'
            ...
            << (runtime_postsolve_observed_ ? 1 : 0) << '|'
            << runtime_postsolve_active_window_value_source_kind_ << '|'
            << runtime_postsolve_strict_local_value_source_kind_ << '|'
            << (runtime_postsolve_value_source_consistent_ ? 1 : 0) << '|'
            << runtime_postsolve_query_frame_convention_kind_ << '|'
            << runtime_postsolve_extrinsic_application_kind_ << '|'
            << runtime_final_materialization_source_kind_ << '|'
            << (runtime_new_frame_consumes_final_truth_only_ ? 1 : 0) << '|'
            << params->smoother_lag << '|'
            << ctrl_point_prediction_inf_scale_ << '|'
            << ctrl_point_smoothness_inf_scale_ << '|'
            << ctrl_point_marginal_inf_scale_ << '|'
            << to_string(velocity_state_mode_) << '|'
            << to_string(runtime_velocity_state_mode_) << '|'
            << to_string(velocity_mode_policy_) << '|'
            << (runtime_has_gnss_constraints_ ? 1 : 0) << '|'
            << (runtime_velocity_optimized_ ? 1 : 0);
  if (!force && signature.str() == last_runtime_mode_metadata_signature_) {
    return;
  }

  nlohmann::json patch;
  patch["runtime_gravity_state_mode"] = to_string(gravity_state_mode_);
  patch["runtime_gravity_reference_source"] = runtime_gravity_reference_source_name();
  patch["runtime_gravity_reference_vector"] = {
    gravity_reference.x(),
    gravity_reference.y(),
    gravity_reference.z(),
  };
  patch["runtime_bias_state_mode"] = to_string(bias_state_mode_);
  patch["runtime_bias_optimized"] = runtime_bias_optimized_;
  patch["runtime_bias_source_of_truth"] = runtime_bias_source_of_truth_name();
  patch["runtime_bias_transition_prior_enabled"] = runtime_bias_transition_prior_enabled();
  patch["runtime_bias_transition_prior_strength"] = runtime_bias_transition_prior_strength();
  patch["runtime_bias_can_be_survivor_anchor"] = runtime_bias_can_be_survivor_anchor();
  patch["runtime_bias_writeback_mode"] = runtime_bias_writeback_mode_name();
  patch["runtime_frontend_seed_mode"] = runtime_frontend_seed_mode_name();
  patch["runtime_imu_forward_prediction_enabled"] = imu_forward_prediction_enabled();
  ...
  patch["runtime_postsolve_active_window_value_source_kind"] =
    runtime_postsolve_active_window_value_source_kind_;
  patch["runtime_postsolve_strict_local_value_source_kind"] =
    runtime_postsolve_strict_local_value_source_kind_;
  patch["runtime_postsolve_value_source_consistent"] = runtime_postsolve_value_source_consistent_;
  patch["runtime_postsolve_query_frame_convention_kind"] =
    runtime_postsolve_query_frame_convention_kind_;
  patch["runtime_postsolve_extrinsic_application_kind"] =
    runtime_postsolve_extrinsic_application_kind_;
  patch["runtime_final_materialization_source_kind"] =
    runtime_final_materialization_source_kind_;
  patch["runtime_new_frame_consumes_final_truth_only"] =
    runtime_new_frame_consumes_final_truth_only_;
  patch["runtime_smoother_lag"] = params->smoother_lag;
  patch["runtime_ctrl_point_prediction_inf_scale"] = ctrl_point_prediction_inf_scale_;
  patch["runtime_ctrl_point_smoothness_inf_scale"] = ctrl_point_smoothness_inf_scale_;
  patch["runtime_ctrl_point_marginal_inf_scale"] = ctrl_point_marginal_inf_scale_;
  ...
  patch["runtime_velocity_optimized"] = runtime_velocity_optimized_;
  iap::merge_run_info_metadata(patch);
  last_runtime_mode_metadata_signature_ = signature.str();
}

bool OdometryEstimationBSpline::any_shared_imu_freeze_experiment_enabled() const {
  return (gravity_graph_key_enabled() && exp_freeze_gravity_) || exp_freeze_gyro_bias_ || exp_freeze_accel_bias_;
}

bool OdometryEstimationBSpline::any_soft_gravity_experiment_enabled() const {
  return gravity_graph_key_enabled() && gravity_mode_ != BSplineGravityMode::NORMAL;
}

bool OdometryEstimationBSpline::any_gravity_isolation_experiment_enabled() const {
  return gravity_graph_key_enabled() && (exp_freeze_gravity_ || any_soft_gravity_experiment_enabled());
}
...
void OdometryEstimationBSpline::maybe_latch_isolation_freeze_anchor(
  ...
  logger->info(
    ...
    isolation_freeze_anchor_.gyro_bias.z(),
    isolation_freeze_anchor_.accel_bias.x(),
    isolation_freeze_anchor_.accel_bias.y(),
...
iap::BSplineFixedLagSharedState OdometryEstimationBSpline::effective_shared_imu_state(
  ...
  if (gravity_external_reference_enabled()) {
    state.gravity = authoritative_gravity_reference();
  }
...
void OdometryEstimationBSpline::apply_effective_shared_imu_state_to_registry(const int frame_id) {
  if (!any_yaw_isolation_experiment_enabled() && !gravity_external_reference_enabled()) {
    return;
  }
  auto frozen_state = effective_shared_imu_state(frame_id);
  if (gravity_external_reference_enabled()) {
    frozen_state.gravity = authoritative_gravity_reference();
  }
  fixed_lag_registry_.set_shared_imu_state(
    frozen_state.gyro_bias,
    frozen_state.accel_bias,
    frozen_state.gravity);
  if (gravity_external_reference_enabled() || any_gravity_isolation_experiment_enabled()) {
...
void OdometryEstimationBSpline::enforce_frozen_shared_values(
  ...
  } else if (bias_lagged_keyed_mode()) {
    for (const auto key : values->keys()) {
      if (gtsam::Symbol(key).chr() == 'j') {
        candidate.gyro_bias = values->at<gtsam::Vector3>(key);
      }
    }
  }
  if (values->exists(bspline_accel_bias_key())) {
    ...
    for (const auto key : values->keys()) {
      if (gtsam::Symbol(key).chr() == 'k') {
        candidate.accel_bias = values->at<gtsam::Vector3>(key);
      }
    }
  }
  if (gravity_graph_key_enabled() && values->exists(bspline_gravity_key())) {
  ...
  if (exp_freeze_gyro_bias_) {
    if (bias_shared_singleton_mode()) {
      update_or_insert_vec3(bspline_gyro_bias_key(), frozen_state.gyro_bias);
    } else {
      for (const auto key : values->keys()) {
        if (gtsam::Symbol(key).chr() == 'j') {
          update_or_insert_vec3(key, frozen_state.gyro_bias);
        }
      }
    }
  }
  if (exp_freeze_accel_bias_) {
    if (bias_shared_singleton_mode()) {
      ...
      for (const auto key : values->keys()) {
        if (gtsam::Symbol(key).chr() == 'k') {
          update_or_insert_vec3(key, frozen_state.accel_bias);
        }
      }
    }
  }
  if (gravity_graph_key_enabled() && (exp_freeze_gravity_ || any_soft_gravity_experiment_enabled())) {
...
: OdometryEstimationCPU(params),
  ...
  {
    const std::string configured_gravity_state_mode =
      config.param<std::string>("odometry_estimation", "gravity_state_mode", "shared_optimized");
    bool used_gravity_state_mode_fallback = false;
    gravity_state_mode_ = parse_gravity_state_mode(
      configured_gravity_state_mode,
      &used_gravity_state_mode_fallback);
    if (used_gravity_state_mode_fallback) {
      logger->warn(
        "unsupported odometry_estimation.gravity_state_mode='{}'; fallback to '{}'",
        configured_gravity_state_mode,
        to_string(gravity_state_mode_));
    }
  }
  {
    const std::string configured_gravity_reference_source =
      config.param<std::string>("odometry_estimation", "gravity_reference_source", "startup_seed");
    bool used_gravity_reference_source_fallback = false;
    gravity_reference_source_ = parse_gravity_reference_source(
      configured_gravity_reference_source,
      &used_gravity_reference_source_fallback);
    if (used_gravity_reference_source_fallback) {
      logger->warn(
        "unsupported odometry_estimation.gravity_reference_source='{}'; fallback to '{}'",
        configured_gravity_reference_source,
        to_string(gravity_reference_source_));
    }
  }
  gravity_reference_vector_config_ = config.param<Eigen::Vector3d>(
    "odometry_estimation",
  ...
  exp_freeze_gyro_bias_ =
    config.param<bool>("odometry_estimation", "exp_freeze_gyro_bias", false);
  exp_freeze_accel_bias_ =
    config.param<bool>("odometry_estimation", "exp_freeze_accel_bias", false);
  exp_disable_velocity_factor_ =
    config.param<bool>("odometry_estimation", "exp_disable_velocity_factor", false);
  exp_disable_current_velocity_prior_ =
    config.param<bool>("odometry_estimation", "exp_disable_current_velocity_prior", false);
  {
    const std::string configured_bias_state_mode =
      config.param<std::string>("odometry_estimation", "bias_state_mode", "shared_singleton");
    bool used_bias_state_mode_fallback = false;
    bias_state_mode_ = parse_bias_state_mode(
      configured_bias_state_mode,
      &used_bias_state_mode_fallback);
    if (used_bias_state_mode_fallback) {
      logger->warn(
        "unsupported odometry_estimation.bias_state_mode='{}'; fallback to '{}'",
        configured_bias_state_mode,
        to_string(bias_state_mode_));
    }
  }
  {
    const std::string configured_velocity_state_mode =
      config.param<std::string>("odometry_estimation", "velocity_state_mode", "optimize");
    bool used_velocity_state_mode_fallback = false;
    velocity_state_mode_ = parse_velocity_state_mode(
      configured_velocity_state_mode,
      &used_velocity_state_mode_fallback);
    if (used_velocity_state_mode_fallback) {
      logger->warn(
        "unsupported odometry_estimation.velocity_state_mode='{}'; fallback to '{}'",
        configured_velocity_state_mode,
        to_string(velocity_state_mode_));
    }
  }
  {
    const std::string configured_velocity_mode_policy =
      config.param<std::string>("odometry_estimation", "velocity_mode_policy", "always_optimize");
    bool used_velocity_mode_policy_fallback = false;
    velocity_mode_policy_ = parse_velocity_mode_policy(
      configured_velocity_mode_policy,
      &used_velocity_mode_policy_fallback);
    if (used_velocity_mode_policy_fallback) {
      logger->warn(
        "unsupported odometry_estimation.velocity_mode_policy='{}'; fallback to '{}'",
        configured_velocity_mode_policy,
        to_string(velocity_mode_policy_));
    }
  }
  {
    const std::string configured_frontend_seed_mode =
      config.param<std::string>("odometry_estimation", "frontend_seed_mode", "last_pose_copy");
    bool used_frontend_seed_mode_fallback = false;
    frontend_seed_mode_ = parse_frontend_seed_mode(
      configured_frontend_seed_mode,
      &used_frontend_seed_mode_fallback);
    if (used_frontend_seed_mode_fallback) {
      logger->warn(
        "unsupported odometry_estimation.frontend_seed_mode='{}'; fallback to '{}'",
        configured_frontend_seed_mode,
        runtime_frontend_seed_mode_name());
    }
  }
  ...
  jump_diagnostics_enabled_ = log_config.profiling.jump_diagnostics;
  ...
  jump_diagnostics_csv_path_ =
    iap::LogPaths::instance().profiling_path(log_config.profiling.jump_diagnostics_file).string();
  ...
  fixed_lag_registry_.set_bias_shared_singleton(bias_shared_singleton_mode());
  fixed_lag_registry_.set_gravity_shared_optimized(gravity_graph_key_enabled());
  fixed_lag_registry_.set_shared_imu_state(
    params.imu_bias.tail<3>(),
    params.imu_bias.head<3>(),
    Eigen::Vector3d::UnitZ() * 9.80665);
  bias_bootstrap_gyro_ = fixed_lag_registry_.shared_state().gyro_bias;
  bias_bootstrap_accel_ = fixed_lag_registry_.shared_state().accel_bias;
  if (gravity_external_reference_enabled()) {
    const gtsam::Vector3 configured_reference = sanitized_gravity_vector(gravity_reference_vector_config_);
    if (gravity_reference_source_ == BSplineGravityReferenceSource::CONFIG_VECTOR) {
      set_external_gravity_reference(configured_reference, BSplineGravityReferenceSource::CONFIG_VECTOR);
    } else {
      set_external_gravity_reference(
        fixed_lag_registry_.shared_state().gravity,
        BSplineGravityReferenceSource::STARTUP_SEED);
    }
  } else {
    fixed_lag_registry_.shared_state().gravity = sanitized_gravity_vector(fixed_lag_registry_.shared_state().gravity);
  }
  last_applied_gravity_ = authoritative_gravity_reference();
  runtime_bias_optimized_ = true;
  runtime_velocity_state_mode_ = resolved_velocity_state_mode(false);
  runtime_velocity_optimized_ = velocity_optimized_for_solve(false);
  ...
  logger->info("odometry_bspline initialized frontend_mode={} frontend_only_mode={} use_legacy_bspline_two_stage_path={} bspline_unified_solver_mode={} final_pose_surface={} gravity_state_mode={} gravity_reference_source={} gravity_reference_x={:.6f} gravity_reference_y={:.6f} gravity_reference_z={:.6f} bias_state_mode={} frontend_seed_mode={} velocity_state_mode={} velocity_mode_policy={} yaw_isolation_experiment={} gravity_mode={} gravity_fixed_norm_value={:.6f} gravity_tilt_limit_rad={:.6f} gravity_warmup_freeze_frames={} freeze_gravity={} freeze_gyro_bias={} freeze_accel_bias={} disable_velocity_factor={} disable_current_velocity_prior={} lidar_gpu_backend={} lidar_bucket_mode={} lidar_bucket_time_eps={:.6f} lidar_max_buckets={} lidar_fixed_buckets={} knot_mode={} nominal_dt={:.4f} compatibility_sample_dt={:.4f} lidar_target_mode={} lidar_jacobian_mode={} lidar_k_candidates={} lidar_accept_ratio={:.3f} lidar_score_gap={:.3f} lidar_snapshot_window={} lidar_snapshot_min_frames={} lidar_snapshot_min_points={} lidar_snapshot_max_age={:.3f} lidar_outlier_thresh={:.3f} lidar_robust_kernel={} lidar_robust_width={:.3f} lidar_robust_w_floor={:.3f} lidar_profile={} lidar_validate={} ct_pipeline_profile={} lidar_baseline_csv={} lidar_baseline_path={}",
    ...
    to_string(gravity_state_mode_),
    runtime_gravity_reference_source_name(),
    authoritative_gravity_reference().x(),
    authoritative_gravity_reference().y(),
    authoritative_gravity_reference().z(),
    to_string(bias_state_mode_),
    runtime_frontend_seed_mode_name(),
    to_string(velocity_state_mode_),
    to_string(velocity_mode_policy_),
    ...
    lidar_baseline_csv_path_);
  sync_runtime_mode_metadata(true);
}
...
OdometryEstimationBSpline::ActiveSplineTargetReference OdometryEstimationBSpline::create_active_target_reference() const {
  ...
  if (lidar_target_mode_ == BSplineLidarTargetMode::GLOBAL_IVOX_REFERENCE && ct_target_ivox_ &&
    ...
    target_ref.voxel_count = target_ref.target_snapshot->num_voxels();
  ...
  if (snapshot_policy_accepted || (!global_reference_available && inserted)) {
    ...
    target_ref.voxel_count = snapshot->num_voxels();
    target_ref.build_ms = std::chrono::duration<double, std::milli>(Clock::now() - t_start).count();
    return target_ref;
  }
  ...
  target_ref.voxel_count = target_ref.target_snapshot ? target_ref.target_snapshot->num_voxels() : 0;
  target_ref.build_ms = std::chrono::duration<double, std::milli>(Clock::now() - t_start).count();
  return target_ref;
}
...
std::vector<iap::CTLocalFrontend::IMUSample> OdometryEstimationBSpline::create_frontend_seed_imu_samples(
  const double start_stamp,
  const double end_stamp) const {
  std::vector<iap::CTLocalFrontend::IMUSample> samples;
  if (!imu_integration || end_stamp <= start_stamp) {
    return samples;
  }

  std::vector<double> delta_times;
  std::vector<Eigen::Matrix<double, 7, 1>> imu_data;
  imu_integration->find_imu_data(start_stamp, end_stamp, delta_times, imu_data);
  (void)delta_times;
  if (imu_data.empty()) {
    return samples;
  }
  ...
  for (const auto& imu : imu_data) {
    const double imu_stamp = imu[0];
    if (imu_stamp + kImuSampleWindowEps < start_stamp || imu_stamp - kImuSampleWindowEps > end_stamp) {
      continue;
    }
    if (std::abs(imu_stamp - last_stamp) <= kImuSampleWindowEps) {
      continue;
    }
    samples.push_back(iap::CTLocalFrontend::IMUSample{
      imu_stamp,
      imu.block<3, 1>(4, 0),
      imu.block<3, 1>(1, 0),
    });
...
void OdometryEstimationBSpline::update_marginal_prior_information(
  ...
  try {
    marginal_prior_.carried_prior =
      iap::build_bspline_carried_prior(graph, values, carried_prior_retained_keys, previous_prior);
...
OdometryEstimationBSpline::BoundaryBridgeSelection
OdometryEstimationBSpline::build_boundary_bridge_selection(
  const iap::SplineActiveStateSet& active_state_set,
  const gtsam::KeyVector& strict_local_query_support_keys,
  const std::vector<std::size_t>& fallback_control_indices,
  const std::size_t current_auxiliary_index,
  const bool velocity_in_graph,
  const bool include_clock_key,
  const gtsam::KeyVector& navigation_retained_keys,
  const gtsam::Values& values) const {
  BoundaryBridgeSelection selection;
  const auto active_solve_keys = sort_unique_keys(active_state_set.active_keys());

  auto filter_to_active_existing_keys = [&](const gtsam::KeyVector& keys) {
    ...
    for (const auto key : keys) {
      ...
      if (std::find(active_solve_keys.begin(), active_solve_keys.end(), key) == active_solve_keys.end()) {
        continue;
      }
      append_unique_key(&filtered, key);
    }
    return sort_unique_keys(std::move(filtered));
  };

  gtsam::KeyVector strict_local_needed_support_keys = filter_to_active_existing_keys(
    sort_unique_keys(strict_local_query_support_keys));
  if (strict_local_needed_support_keys.empty()) {
    strict_local_needed_support_keys = filter_to_active_existing_keys(
      pose_keys_from_control_indices(fallback_control_indices));
  }
  selection.strict_local_needed_support_keys = strict_local_needed_support_keys;

  gtsam::KeyVector carried_prior_retained_keys = strict_local_needed_support_keys;
  auto append_retained_if_active = [&](const gtsam::Key key) {
    if (!values.exists(key)) {
      return;
    }
    if (std::find(active_solve_keys.begin(), active_solve_keys.end(), key) == active_solve_keys.end()) {
    ...
    append_unique_key(&carried_prior_retained_keys, key);
  };

  if (velocity_in_graph) {
    append_retained_if_active(iap::bspline_velocity_key(current_auxiliary_index));
  }
  if (include_clock_key) {
    append_retained_if_active(iap::bspline_clock_key(current_auxiliary_index));
  }
  if (bias_lagged_keyed_mode()) {
    append_retained_if_active(gyro_bias_key_for_auxiliary(current_auxiliary_index));
    append_retained_if_active(accel_bias_key_for_auxiliary(current_auxiliary_index));
  } else {
    append_retained_if_active(iap::bspline_gyro_bias_key());
    append_retained_if_active(iap::bspline_accel_bias_key());
  }
  for (const auto key : navigation_retained_keys) {
    append_retained_if_active(key);
  }

  selection.carried_prior_retained_keys = filter_to_active_existing_keys(carried_prior_retained_keys);
  selection.boundary_anchor_keys = bias_anchorable_survivor_keys(selection.carried_prior_retained_keys);
  return selection;
}
...
void OdometryEstimationBSpline::append_active_segment_constraint(
  ...
  segment.target_voxel_count = target_ref.voxel_count;
  ...
  segment.target_snapshot_clone_ms = target_ref.target_snapshot_clone_ms;
  segment.target_voxel_lookup_prep_ms = target_ref.target_voxel_lookup_prep_ms;
  segment.target_covariance_prep_ms = target_ref.target_covariance_prep_ms;
  segment.source_to_target_transform_ms = target_ref.source_to_target_transform_ms;
...
std::shared_ptr<iap::IntegratedBSplineGICPFactor> OdometryEstimationBSpline::get_or_create_cpu_lidar_factor(
  ...
  if (!key_match) {
    ...
    factor->set_enable_profiling(
      lidar_factor_profile_ || lidar_warn_degeneracy_ || lidar_export_baseline_csv_ || jump_diagnostics_enabled_);
...
std::shared_ptr<iap::IntegratedBSplineGICPFactorGPU> OdometryEstimationBSpline::get_or_create_gpu_bucket_lidar_factor(
  ...
  const bool enable_profile_surface =
    ...
    lidar_profile_numeric_reference_ || jump_diagnostics_enabled_;
...
EstimationFrame::ConstPtr OdometryEstimationBSpline::insert_frame_ct_lidar_legacy_two_stage(
  ...
  if (frames.empty()) {
    ...
    fixed_lag_registry_.set_shared_imu_state(
      ...
      authoritative_gravity_reference());
    maybe_latch_isolation_freeze_anchor(fixed_lag_registry_.shared_state());
    ...
    record_runtime_mode_resolution(false, false, false);
  ...
  if (frontend_only_mode_) {
    record_runtime_mode_resolution(false, false, false);
    record_frontend_seed_observation(FrontendSeedObservation{
      ct_local_result.pose_diagnostics.seed_mode,
      ct_local_result.pose_diagnostics.seed_source,
    ...
    if (!frontend_controls.empty()) {
      ...
      write_frame_bias_from_values(
        ct_local_result.local_values,
        frontend_controls.back().index,
        new_frame.get());
    }
    ...
    if (jump_diagnostics_enabled_) {
      JumpDiagnosticsRow jump_row;
      jump_row.frame_id = new_frame->id;
      jump_row.frame_stamp = raw_frame->stamp;
      ...
      jump_row.representative_time = ct_local_result.pose_diagnostics.representative_time > 0.0
        ? ct_local_result.pose_diagnostics.representative_time
        : ct_local_result.pose_diagnostics.query_time;
      jump_row.bucket_representative_time = ct_local_result.pose_diagnostics.bucket_representative_time;
      jump_row.start_pose_query_time = ct_local_result.pose_diagnostics.query_time;
      jump_row.frontend_pose_query_time = ct_local_result.pose_diagnostics.query_time;
      jump_row.current_segment_id = ct_local_result.layout.controls().empty()
        ? -1LL
        : static_cast<long long>(ct_local_result.layout.controls().back().index);
      jump_row.start_pose_source_kind = ct_local_result.pose_diagnostics.seed_source;
      jump_row.start_pose_frozen_before_factor_injection = true;
      jump_row.start_pose_frozen_before_solver_update = true;
      jump_row.frontend_seed_mode = ct_local_result.pose_diagnostics.seed_mode;
      jump_row.frontend_seed_source = ct_local_result.pose_diagnostics.seed_source;
      jump_row.frontend_seed_fallback_used = ct_local_result.pose_diagnostics.seed_fallback_used;
      jump_row.frontend_seed_imu_sample_count = ct_local_result.pose_diagnostics.seed_imu_sample_count;
      ...
      const gtsam::Pose3 final_pose_lidar = pose3_from_isometry(new_frame->T_world_lidar);
      ...
      assign_pose_components(
        start_pose_imu,
        &jump_row.start_pose_tx,
        &jump_row.start_pose_ty,
        &jump_row.start_pose_tz,
        &jump_row.start_pose_qx,
        &jump_row.start_pose_qy,
        &jump_row.start_pose_qz,
        &jump_row.start_pose_qw);
      assign_pose_components(
        frontend_pose_imu,
        &jump_row.frontend_pose_tx,
        &jump_row.frontend_pose_ty,
        &jump_row.frontend_pose_tz,
        &jump_row.frontend_pose_qx,
        &jump_row.frontend_pose_qy,
      ...
      assign_pose_components(
        ...
        &jump_row.final_pose_tz,
        &jump_row.final_pose_qx,
        &jump_row.final_pose_qy,
        &jump_row.final_pose_qz,
        &jump_row.final_pose_qw);
      jump_row.delta_start_to_frontend_translation_norm =
      ...
      populate_pose_residual_component_pair(
        start_pose_imu,
        frontend_pose_imu,
        &jump_row.delta_start_to_frontend_pitch_rad,
        &jump_row.delta_start_to_frontend_roll_rad);
      ...
      populate_pose_residual_component_pair(
        frontend_pose_imu,
        postsolve_query_pose_imu,
        &jump_row.delta_frontend_to_postsolve_query_pitch_rad,
        &jump_row.delta_frontend_to_postsolve_query_roll_rad);
      ...
      populate_pose_residual_component_pair(
        frontend_pose_imu,
        postsolve_query_pose_imu,
        &jump_row.delta_frontend_to_postsolve_strict_local_pitch_rad,
        &jump_row.delta_frontend_to_postsolve_strict_local_roll_rad);
      ...
      populate_pose_residual_component_pair(
        postsolve_query_pose_imu,
        final_pose_imu,
        &jump_row.delta_postsolve_query_to_final_pitch_rad,
        &jump_row.delta_postsolve_query_to_final_roll_rad);
      ...
      populate_pose_residual_component_pair(
        postsolve_query_pose_imu,
        final_pose_imu,
        &jump_row.delta_postsolve_strict_local_to_final_pitch_rad,
        &jump_row.delta_postsolve_strict_local_to_final_roll_rad);
      ...
      jump_row.delta_postsolve_active_window_to_postsolve_strict_local_pitch_rad = 0.0;
      jump_row.delta_postsolve_active_window_to_postsolve_strict_local_roll_rad = 0.0;
      ...
      jump_row.delta_frontend_to_final_rotation_rad =
        pose_rotation_delta_rad(frontend_pose_imu, final_pose_imu);
      populate_frontend_final_residual_components(&jump_row, frontend_pose_imu, final_pose_imu);
      populate_yaw_residual_shootout_probes(
        &jump_row,
        ct_local_result.pose_diagnostics.optimized_pose,
        frontend_pose_imu,
        final_pose_lidar,
        final_pose_imu,
        ...
        pose3_from_isometry(T_lidar_imu));
      jump_row.lidar_layout_domain_begin = ct_local_result.pose_diagnostics.layout_domain_begin;
      jump_row.lidar_layout_domain_end = ct_local_result.pose_diagnostics.layout_domain_end;
      jump_row.start_pose_support_key_count = ct_local_result.pose_diagnostics.query_support_keys.size();
      jump_row.start_pose_support_keys_summary =
        summarize_keys(ct_local_result.pose_diagnostics.query_support_keys);
      jump_row.start_pose_support_mismatch_flag = false;
      jump_row.start_pose_support_mismatch_reason = "none";
      jump_row.lidar_support_key_count = ct_local_result.pose_diagnostics.lidar_support_keys.size();
      jump_row.lidar_support_keys_summary = summarize_keys(ct_local_result.pose_diagnostics.lidar_support_keys);
      ...
      jump_row.strict_local_query_support_key_count =
        ct_local_result.pose_diagnostics.query_support_keys.size();
      jump_row.strict_local_query_support_keys_summary =
        summarize_keys(ct_local_result.pose_diagnostics.query_support_keys);
      jump_row.strict_local_query_reason = "none";
      ...
      jump_row.candidate_correspondence_count =
        ct_local_result.pose_diagnostics.candidate_correspondence_count;
      jump_row.accepted_correspondence_count =
        ct_local_result.pose_diagnostics.accepted_correspondence_count;
      jump_row.accept_ratio = ct_local_result.pose_diagnostics.accept_ratio;
      jump_row.registration_delta_translation_norm =
        ct_local_result.pose_diagnostics.registration_delta_translation_norm;
      jump_row.registration_delta_rotation_rad =
        ct_local_result.pose_diagnostics.registration_delta_rotation_rad;
      jump_row.factor_total_ms = ct_local_result.pose_diagnostics.factor_total_ms;
      jump_row.target_map_prep_ms = ct_frontend_input.target_map_prep_ms;
      jump_row.target_point_count = ct_frontend_input.target_point_count;
      jump_row.target_voxel_count = ct_frontend_input.target_voxel_count;
      jump_row.target_snapshot_clone_ms = ct_frontend_input.target_snapshot_clone_ms;
      jump_row.target_voxel_lookup_prep_ms = ct_frontend_input.target_voxel_lookup_prep_ms;
      jump_row.target_covariance_prep_ms = ct_frontend_input.target_covariance_prep_ms;
      jump_row.source_to_target_transform_ms = ct_frontend_input.source_to_target_transform_ms;
      jump_row.solver_update_ms = 0.0;
      ...
      if (!frames.empty() && frames.back()) {
        ...
        jump_row.pose_guess_rotation_rad = pose_rotation_delta_rad(previous_pose_imu, start_pose_imu);
      }
      jump_row.oldest_survivor_key_summary =
        summarize_oldest_key(ct_local_result.backend_summary.active_pose_keys);
  ...
  const bool solve_has_gnss = solve_has_gnss_constraints();
  const bool velocity_explicit_optimization_for_solve = velocity_explicit_optimization_enabled(solve_has_gnss);
  ...
  seed_bias_values_for_active_segments(&values);
  ...
  const auto predict_segment_velocity = [&](const ActiveSplineSegmentConstraint& segment) {
  ...
  for (const auto& segment : fixed_lag_registry_.segments()) {
    ...
    if (!velocity_optimized_for_solve(solve_has_gnss_constraints(segment))) {
      continue;
    }
    ...
    values.insert(velocity_key, predict_segment_velocity(segment));
  }
  ...
  gtsam::KeyVector batch_navigation_retained_keys;
  if (fixed_lag_registry_.shared_state().gnss_anchor_initialized) {
    if (values.exists(ecef_origin_key)) {
      batch_navigation_retained_keys.push_back(ecef_origin_key);
    }
    if (values.exists(ecef_rot_key)) {
      batch_navigation_retained_keys.push_back(ecef_rot_key);
    }
  }
  const ActiveSplineSegmentConstraint* current_batch_segment =
    fixed_lag_registry_.segments().empty() ? nullptr : &fixed_lag_registry_.segments().back();
  const BoundaryBridgeSelection batch_boundary_bridge_selection = current_batch_segment
    ? build_boundary_bridge_selection(
        active_state_set,
        ct_local_result.pose_diagnostics.query_support_keys,
        std::vector<std::size_t>(
          current_batch_segment->control_indices.begin(),
          current_batch_segment->control_indices.end()),
        current_batch_segment->auxiliary_index,
        velocity_optimized_for_solve(solve_has_gnss_constraints(*current_batch_segment)),
        !current_batch_segment->gnss_epochs.empty(),
        batch_navigation_retained_keys,
        values)
    : BoundaryBridgeSelection{};
  const iap::BSplineMarginalizationPartition marginalization_partition =
    current_batch_segment
      ? iap::build_bspline_marginalization_partition(
          active_state_set,
          batch_boundary_bridge_selection.strict_local_needed_support_keys,
          batch_boundary_bridge_selection.carried_prior_retained_keys,
          batch_boundary_bridge_selection.boundary_anchor_keys)
  ...
  for (std::size_t i = 0; i < fixed_lag_registry_.segments().size(); ++i) {
    ...
    const gtsam::Key velocity_key = iap::bspline_velocity_key(segment.auxiliary_index);
    const bool segment_has_gnss_constraints = solve_has_gnss_constraints(segment);
    const bool segment_velocity_explicit_optimization =
      velocity_explicit_optimization_enabled(segment_has_gnss_constraints);
    const bool segment_velocity_in_graph = velocity_optimized_for_solve(segment_has_gnss_constraints);

    if (segment_velocity_in_graph && !values.exists(velocity_key)) {
      if (fixed_lag_registry_.auxiliary_values().exists(velocity_key)) {
        values.insert(velocity_key, fixed_lag_registry_.auxiliary_values().at<gtsam::Vector3>(velocity_key));
      } else {
        values.insert(velocity_key, predict_segment_velocity(segment));
    ...
    if (segment_velocity_explicit_optimization && !exp_disable_velocity_factor_) {
    ...
    for (const auto& imu_sample : segment.imu_samples) {
      ...
      const gtsam::Key segment_gyro_bias_key = gyro_bias_key_for_auxiliary(segment.auxiliary_index);
      const gtsam::Key segment_accel_bias_key = accel_bias_key_for_auxiliary(segment.auxiliary_index);
      ...
      iap::IntegratedSplineIMUFactor::shared_ptr imu_factor;
      if (gravity_external_reference_enabled()) {
        imu_factor = std::make_shared<iap::IntegratedSplineIMUFactor>(
          ctx,
          segment_gyro_bias_key,
          segment_accel_bias_key,
          authoritative_gravity_reference(),
      ...
      if (marginalization_partition.should_marginalize_factor(imu_factor->keys())) {
  ...
  const auto frozen_shared_noise =
    gtsam::noiseModel::Isotropic::Precision(3, isolation_freeze_prior_precision_);
  ...
  if (!information_prior_attached && use_marginal_prior) {
    ...
    if (velocity_explicit_optimization_for_solve &&
        !exp_disable_current_velocity_prior_ &&
        marginal_prior_.has_velocity &&
        values.exists(iap::bspline_velocity_key(marginal_prior_.auxiliary_index))) {
  ...
  if (bias_shared_singleton_mode()) {
    graph.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      gyro_bias_key,
      shared_state.gyro_bias,
      (shared_state_frozen && exp_freeze_gyro_bias_) ? frozen_shared_noise : imu_bias_noise);
    graph.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      accel_bias_key,
      shared_state.accel_bias,
      (shared_state_frozen && exp_freeze_accel_bias_) ? frozen_shared_noise : imu_bias_noise);
  } else {
    const auto active_bias_auxiliary_indices = fixed_lag_registry_.active_auxiliary_indices();
    if (!active_bias_auxiliary_indices.empty()) {
      const auto first_gyro_key = gyro_bias_key_for_auxiliary(active_bias_auxiliary_indices.front());
      const auto first_accel_key = accel_bias_key_for_auxiliary(active_bias_auxiliary_indices.front());
      if (values.exists(first_gyro_key)) {
        graph.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
          first_gyro_key,
          values.at<gtsam::Vector3>(first_gyro_key),
          (shared_state_frozen && exp_freeze_gyro_bias_) ? frozen_shared_noise : imu_bias_noise);
      }
      if (values.exists(first_accel_key)) {
        graph.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
          first_accel_key,
          values.at<gtsam::Vector3>(first_accel_key),
          (shared_state_frozen && exp_freeze_accel_bias_) ? frozen_shared_noise : imu_bias_noise);
      }
      for (std::size_t i = 1; i < active_bias_auxiliary_indices.size(); ++i) {
        const auto prev_gyro_key = gyro_bias_key_for_auxiliary(active_bias_auxiliary_indices[i - 1]);
        const auto curr_gyro_key = gyro_bias_key_for_auxiliary(active_bias_auxiliary_indices[i]);
        auto gyro_between = std::make_shared<gtsam::BetweenFactor<gtsam::Vector3>>(
          prev_gyro_key,
          curr_gyro_key,
          gtsam::Vector3::Zero(),
          imu_bias_noise);
        graph.add(gyro_between);
        if (marginalization_partition.should_marginalize_factor(gtsam::KeyVector{prev_gyro_key, curr_gyro_key})) {
          marginalization_graph.add(gyro_between);
        }

        const auto prev_accel_key = accel_bias_key_for_auxiliary(active_bias_auxiliary_indices[i - 1]);
        const auto curr_accel_key = accel_bias_key_for_auxiliary(active_bias_auxiliary_indices[i]);
        auto accel_between = std::make_shared<gtsam::BetweenFactor<gtsam::Vector3>>(
          prev_accel_key,
          curr_accel_key,
          gtsam::Vector3::Zero(),
          imu_bias_noise);
        graph.add(accel_between);
        if (marginalization_partition.should_marginalize_factor(gtsam::KeyVector{prev_accel_key, curr_accel_key})) {
          marginalization_graph.add(accel_between);
        }
      }
    }
  }
  if (gravity_graph_key_enabled()) {
    graph.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      gravity_key,
      shared_state.gravity,
      (shared_state_frozen && exp_freeze_gravity_) ? frozen_shared_noise : gravity_noise);
  }
  ...
  fixed_lag_registry_.update_shared_state_from_values(values);
  mirror_bias_cache_from_values(values);
  maybe_latch_isolation_freeze_anchor(fixed_lag_registry_.shared_state());
  apply_effective_shared_imu_state_to_registry(current);
  fixed_lag_registry_.clear_auxiliary_values();
  for (const auto& segment : fixed_lag_registry_.segments()) {
    ...
    } else if (!fixed_lag_registry_.auxiliary_values().exists(velocity_key)) {
      fixed_lag_registry_.auxiliary_values().insert(velocity_key, predict_segment_velocity(segment));
    }
    const gtsam::Key clock_key = iap::bspline_clock_key(segment.auxiliary_index);
  ...
  }
  {
    ...
    update_marginal_prior_information(
      ...
      values,
      marginalization_partition.carried_prior_retained_keys,
  ...
  new_frame->T_world_lidar = Eigen::Isometry3d(start_pose.matrix());
  new_frame->T_world_imu = new_frame->T_world_lidar * T_lidar_imu;
  const gtsam::Key current_velocity_key = iap::bspline_velocity_key(control_window_->states()[1].index);
  if (values.exists(current_velocity_key)) {
    new_frame->v_world_imu = values.at<gtsam::Vector3>(current_velocity_key);
  } else if (fixed_lag_registry_.auxiliary_values().exists(current_velocity_key)) {
    new_frame->v_world_imu = fixed_lag_registry_.auxiliary_values().at<gtsam::Vector3>(current_velocity_key);
  } else {
    new_frame->v_world_imu.setZero();
  }
  write_frame_bias_from_values(values, control_window_->states()[1].index, new_frame.get());
  ...
  if (jump_diagnostics_enabled_) {
    ...
    jump_row.target_covariance_prep_ms = ct_frontend_input.target_covariance_prep_ms;
    jump_row.source_to_target_transform_ms = ct_frontend_input.source_to_target_transform_ms;
    jump_row.solver_update_ms = pipeline_timing.lm_optimize_ms;
    jump_row.recalculated_lidar_factor_count = 0;
    jump_row.recalculated_lidar_current_segment_factor_count = 0;
    jump_row.recalculated_lidar_same_support_factor_count = 0;
    if (!frames.empty() && frames.back()) {
    ...
    jump_row.carried_boundary_oldest_key_summary =
      summarize_oldest_key(bias_anchorable_survivor_keys(previous_carried_prior.retained_keys));
    jump_row.oldest_survivor_key_summary =
      summarize_oldest_key(marginalization_partition.boundary_anchor_keys);
    jump_row.retained_pose_control_key_count =
      count_pose_control_keys(marginalization_partition.carried_prior_retained_keys);
    jump_row.strict_local_needed_support_key_count =
      count_pose_control_keys(marginalization_partition.strict_local_needed_support_keys);
    jump_row.retained_minus_strict_local_key_excess =
      static_cast<long long>(jump_row.retained_pose_control_key_count) -
      static_cast<long long>(jump_row.strict_local_needed_support_key_count);
    jump_row.carried_prior_retained_keys_summary =
      summarize_keys(marginalization_partition.carried_prior_retained_keys);
    jump_row.uses_local_lidar_layout_override = false;
    jump_row.frontend_pose_query_support_keys_summary =
      summarize_keys(ct_local_result.pose_diagnostics.query_support_keys);
    jump_row.frontend_target_time_offset_vs_representative =
...
EstimationFrame::ConstPtr OdometryEstimationBSpline::insert_frame_ct_lidar_unified_graph(
  ...
  if (jump_diagnostics_enabled_ && !fixed_lag_registry_.segments().empty()) {
    const auto& current_segment = fixed_lag_registry_.segments().back();
    auto shadow_input = make_local_layer_delta_input(current_segment, factor_layout);
    shadow_input.imu_layout_override = create_segment_imu_layout(current_segment);
  ...
  const ActiveSplineSegmentConstraint* current_unified_segment =
    fixed_lag_registry_.segments().empty() ? nullptr : &fixed_lag_registry_.segments().back();
  const BoundaryBridgeSelection unified_boundary_bridge_selection = current_unified_segment
    ? build_boundary_bridge_selection(
        active_state_set,
        frontend_shadow_result.pose_diagnostics.query_support_keys,
        std::vector<std::size_t>(
          current_unified_segment->control_indices.begin(),
          current_unified_segment->control_indices.end()),
        current_unified_segment->auxiliary_index,
        velocity_optimized,
        navigation_contribution.activation.include_clock_states,
        navigation_contribution.activation.retained_keys,
        values)
    : BoundaryBridgeSelection{};
  const iap::BSplineMarginalizationPartition marginalization_partition =
    current_unified_segment
      ? iap::build_bspline_marginalization_partition(
          active_state_set,
          unified_boundary_bridge_selection.strict_local_needed_support_keys,
          unified_boundary_bridge_selection.carried_prior_retained_keys,
          unified_boundary_bridge_selection.boundary_anchor_keys)
      : iap::build_bspline_marginalization_partition(active_state_set);
  ...
  control_window_->update_from_values(values);
  const gtsam::Values postsolve_authoritative_values =
    build_postsolve_authoritative_values(values, navigation_layer_enabled);
  sync_postsolve_registry_mirrors(
    postsolve_authoritative_values,
    current,
    true);
  const iap::SplineActiveStateSet post_active_state_set =
    fixed_lag_registry_.active_state_set(postsolve_authoritative_values, min_active_stamp, include_clock_states);
  ...
  const ActiveSplineSegmentConstraint* current_postsolve_segment =
    fixed_lag_registry_.segments().empty() ? nullptr : &fixed_lag_registry_.segments().back();
  const auto postsolve_context = build_postsolve_evaluation_context(
    postsolve_authoritative_values,
    current_postsolve_segment,
    raw_frame->stamp,
    navigation_layer_enabled,
    "postsolve_authoritative_values");
  record_postsolve_evaluation_observation(postsolve_context);
  sync_runtime_mode_metadata();
  ...
  auto append_lidar_result = [&](const iap::BSplineLocalLayerContribution::LidarFactorHandle& handle, bool current_factor) {
    const double factor_error = handle.factor->error(postsolve_context.authoritative_values);
  ...
  const auto postsolve_publish_query = evaluate_postsolve_publish_pose(
    postsolve_context.active_window_layout,
    postsolve_context.active_window_evaluator,
    postsolve_context.authoritative_values,
    postsolve_context.query_time,
    legacy_postsolve_fallback_pose,
    iap::SplineSensorId::Lidar);
  const auto postsolve_strict_local_query = evaluate_postsolve_layout_pose(
    postsolve_context.strict_local_layout,
    nullptr,
    postsolve_context.authoritative_values,
    postsolve_context.query_time,
    legacy_postsolve_fallback_pose,
  ...
  new_frame->T_world_imu = new_frame->T_world_lidar * T_lidar_imu;
  const gtsam::Key current_velocity_key = iap::bspline_velocity_key(control_window_->states()[1].index);
  if (postsolve_context.authoritative_values.exists(current_velocity_key)) {
    new_frame->v_world_imu =
      postsolve_context.authoritative_values.at<gtsam::Vector3>(current_velocity_key);
  } else {
    new_frame->v_world_imu.setZero();
  }
  write_frame_bias_from_values(
    postsolve_context.authoritative_values,
    control_window_->states()[1].index,
    new_frame.get());
  const gtsam::Key current_clock_key = iap::bspline_clock_key(control_window_->states()[1].index);
  if (postsolve_context.authoritative_values.exists(current_clock_key)) {
    const auto clock = postsolve_context.authoritative_values.at<gtsam::Vector2>(current_clock_key);
    new_frame->clk_bias = clock(0);
    new_frame->clk_drift = clock(1);
  } else if (!frames.empty() && frames.back()) {
    new_frame->clk_bias = frames.back()->clk_bias;
  ...
  auto deskewed_points = current_handle_it->factor->deskewed_source_points(
    postsolve_context.authoritative_values,
    true);
  const auto deskewed_covs = covariance_estimation->estimate(deskewed_points, raw_frame->neighbors);
  for (int i = 0; i < new_frame->frame->size(); ++i) {
  ...
  frontend_frame_profile.frontend_seed_mode = frontend_seed_observation.mode;
  frontend_frame_profile.frontend_seed_source = frontend_seed_observation.source;
  frontend_frame_profile.frontend_seed_fallback_used = frontend_seed_observation.fallback_used;
  frontend_frame_profile.frontend_seed_imu_sample_count = frontend_seed_observation.imu_sample_count;
  ...
  if (jump_diagnostics_enabled_) {
    JumpDiagnosticsRow jump_row;
    jump_row.frame_id = frontend_frame_profile.frame_id;
    jump_row.frame_stamp = frontend_frame_profile.stamp;
    jump_row.raw_frame_stamp = raw_frame->stamp;
    jump_row.scan_begin_time = raw_frame->stamp;
    jump_row.scan_end_time = raw_frame->scan_end_time;
    ...
    jump_row.start_pose_frozen_before_factor_injection = true;
    jump_row.start_pose_frozen_before_solver_update = true;
    jump_row.frontend_seed_mode = frontend_seed_observation.mode;
    jump_row.frontend_seed_source = frontend_seed_observation.source;
    jump_row.frontend_seed_fallback_used = frontend_seed_observation.fallback_used;
    jump_row.frontend_seed_imu_sample_count = frontend_seed_observation.imu_sample_count;

    const auto start_layout_diag =
    ...
    const gtsam::Pose3 frontend_pose_imu =
      frontend_pose_lidar * pose3_from_isometry(T_lidar_imu);
    const gtsam::Pose3 final_pose_imu = pose3_from_isometry(new_frame->T_world_imu);
    assign_pose_components(
      start_pose_imu,
      &jump_row.start_pose_tx,
    ...
    populate_pose_residual_component_pair(
      postsolve_query_pose_imu,
      postsolve_strict_local_pose_imu,
      &jump_row.delta_postsolve_active_window_to_postsolve_strict_local_pitch_rad,
      &jump_row.delta_postsolve_active_window_to_postsolve_strict_local_roll_rad);
    jump_row.delta_frontend_to_final_translation_norm =
      pose_translation_delta_norm(frontend_pose_imu, final_pose_imu);
    jump_row.delta_frontend_to_final_rotation_rad =
      pose_rotation_delta_rad(frontend_pose_imu, final_pose_imu);
    ...
    populate_yaw_residual_shootout_probes(
      ...
      final_pose_imu,
      new_frame->v_world_imu,
      solver_update_row.active_window_velocity_factor_count,
      solver_update_row.active_window_prior_factor_count,
      local_contribution.uses_shared_imu_state,
      fixed_lag_registry_.shared_state(),
      pose3_from_isometry(T_lidar_imu));
    jump_row.lidar_layout_domain_begin = frontend_shadow_result.pose_diagnostics.valid
      ? frontend_shadow_result.pose_diagnostics.layout_domain_begin
      : fallback_domain_begin;
    jump_row.lidar_layout_domain_end = frontend_shadow_result.pose_diagnostics.valid
      ? frontend_shadow_result.pose_diagnostics.layout_domain_end
      : fallback_domain_end;
    populate_start_pose_support_diagnostics(&jump_row, start_layout_diag, strict_layout_diag);
    jump_row.lidar_support_key_count = frontend_shadow_result.pose_diagnostics.lidar_support_keys.size();
    jump_row.lidar_support_keys_summary =
      summarize_keys(frontend_shadow_result.pose_diagnostics.lidar_support_keys);
    jump_row.frontend_pose_support_key_count = frontend_shadow_result.pose_diagnostics.valid
    ...
    jump_row.strict_local_query_support_keys_summary =
      summarize_keys(postsolve_strict_local_query.query_diagnostics.support_keys);
    jump_row.strict_local_query_reason =
      classify_strict_local_query_reason(
        postsolve_strict_local_query.query_diagnostics,
        frontend_support_keys,
        postsolve_strict_local_query.used_fallback_legacy_eval);
    ...
    jump_row.candidate_correspondence_count =
      frontend_shadow_result.pose_diagnostics.candidate_correspondence_count;
    jump_row.accepted_correspondence_count =
      frontend_shadow_result.pose_diagnostics.accepted_correspondence_count;
    jump_row.accept_ratio = frontend_shadow_result.pose_diagnostics.accept_ratio;
    jump_row.registration_delta_translation_norm =
      frontend_shadow_result.pose_diagnostics.registration_delta_translation_norm;
    jump_row.registration_delta_rotation_rad =
      frontend_shadow_result.pose_diagnostics.registration_delta_rotation_rad;
    jump_row.factor_total_ms = frontend_shadow_result.pose_diagnostics.factor_total_ms;
    jump_row.target_map_prep_ms = timing.target_build_ms;
    if (!fixed_lag_registry_.segments().empty()) {
      const auto& current_segment = fixed_lag_registry_.segments().back();
      jump_row.target_point_count = current_segment.target_point_count;
      jump_row.target_voxel_count = current_segment.target_voxel_count;
      jump_row.target_snapshot_clone_ms = current_segment.target_snapshot_clone_ms;
    ...
    jump_row.carried_prior_retained_keys_summary =
      summarize_keys(marginalization_partition.carried_prior_retained_keys);
    jump_row.postsolve_active_window_value_source_kind =
      postsolve_context.active_window_value_source_kind;
    jump_row.postsolve_strict_local_value_source_kind =
      postsolve_context.strict_local_value_source_kind;
    jump_row.postsolve_value_source_consistent =
      postsolve_context.value_source_consistent;
    jump_row.postsolve_query_frame_convention_kind =
      postsolve_context.frame_convention_kind;
    jump_row.postsolve_extrinsic_application_kind =
      postsolve_context.extrinsic_application_kind;
    jump_row.final_materialization_source_kind =
      postsolve_context.final_materialization_source_kind;
    jump_row.new_frame_consumes_final_truth_only =
      postsolve_context.new_frame_consumes_final_truth_only;
    jump_row.uses_local_lidar_layout_override =
...
EstimationFrame::ConstPtr OdometryEstimationBSpline::insert_frame_ct_lidar_incremental_graph(
  ...
  const bool solve_has_gnss = solve_has_gnss_constraints(current_segment);
  const bool velocity_explicit_optimization_for_solve = velocity_explicit_optimization_enabled(solve_has_gnss);
  const bool velocity_optimized = velocity_optimized_for_solve(solve_has_gnss);
  record_runtime_mode_resolution(solve_has_gnss, velocity_explicit_optimization_for_solve, velocity_optimized);
  const FrontendSeedObservation frontend_seed_observation =
  ...
  fixed_lag_registry_.seed_shared_values(
    mirror_values,
    navigation_layer_enabled && fixed_lag_registry_.shared_state().gnss_anchor_initialized);
  seed_bias_values_for_active_segments(&mirror_values);
  ...
  const auto predict_segment_velocity = [&](const ActiveSplineSegmentConstraint& segment) {
    std::array<gtsam::Pose3, iap::kBSplineControlPointCount> segment_poses{};
    for (std::size_t k = 0; k < iap::kBSplineControlPointCount; ++k) {
      segment_poses[k] = mirror_values.at<gtsam::Pose3>(iap::bspline_control_point_key(segment.control_indices[k]));
    }
    return iap::IntegratedBSplineVelocityFactor::predict_velocity(
      segment_poses,
      0.0,
  ...
  for (const auto auxiliary_index : boundary_auxiliary_indices) {
    ...
    if (bias_lagged_keyed_mode()) {
      const gtsam::Key boundary_gyro_bias_key = gyro_bias_key_for_auxiliary(auxiliary_index);
      if (key_known(boundary_gyro_bias_key)) {
        append_unique_key(&delta.active_aux_keys, boundary_gyro_bias_key);
      }
      const gtsam::Key boundary_accel_bias_key = accel_bias_key_for_auxiliary(auxiliary_index);
      if (key_known(boundary_accel_bias_key)) {
        append_unique_key(&delta.active_aux_keys, boundary_accel_bias_key);
      }
    }
  ...
  if (velocity_optimized && !key_known(current_velocity_key)) {
  ...
  for (const auto auxiliary_index : boundary_auxiliary_indices) {
    ...
    if (bias_lagged_keyed_mode()) {
      const gtsam::Key boundary_gyro_bias_key = gyro_bias_key_for_auxiliary(auxiliary_index);
      if (key_known(boundary_gyro_bias_key)) {
        delta.new_stamps[boundary_gyro_bias_key] = current_segment.scan_end;
      }
      const gtsam::Key boundary_accel_bias_key = accel_bias_key_for_auxiliary(auxiliary_index);
      if (key_known(boundary_accel_bias_key)) {
        delta.new_stamps[boundary_accel_bias_key] = current_segment.scan_end;
      }
    }
    if (navigation_layer_enabled) {
      const gtsam::Key clock_key = iap::bspline_clock_key(auxiliary_index);
  ...
  const gtsam::Key accel_bias_key = bspline_accel_bias_key();
  const gtsam::Key gravity_key = bspline_gravity_key();
  const gtsam::Key current_gyro_bias_key = gyro_bias_key_for_auxiliary(current_segment.auxiliary_index);
  const gtsam::Key current_accel_bias_key = accel_bias_key_for_auxiliary(current_segment.auxiliary_index);
  if (bias_shared_singleton_mode()) {
    ...
    if (!key_known(current_gyro_bias_key)) {
      delta.new_values.insert(current_gyro_bias_key, mirror_values.at<gtsam::Vector3>(current_gyro_bias_key));
      delta.new_stamps[current_gyro_bias_key] = current_segment.scan_end;
      append_unique_key(&newly_announced_keys, current_gyro_bias_key);
    }
    if (!key_known(current_accel_bias_key)) {
      delta.new_values.insert(current_accel_bias_key, mirror_values.at<gtsam::Vector3>(current_accel_bias_key));
      delta.new_stamps[current_accel_bias_key] = current_segment.scan_end;
      append_unique_key(&newly_announced_keys, current_accel_bias_key);
    }
  }
  if (gravity_graph_key_enabled() &&
      !fixed_lag_registry_.persistent_key_announced(gravity_key) &&
      !key_known(gravity_key)) {
    delta.new_values.insert(gravity_key, shared_state.gravity);
    append_unique_key(&newly_announced_keys, gravity_key);
  }

  if (bias_shared_singleton_mode() && delta.new_values.exists(gyro_bias_key)) {
    delta.new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      gyro_bias_key,
      shared_state.gyro_bias,
      (shared_state_frozen && exp_freeze_gyro_bias_) ? frozen_shared_noise : imu_bias_noise);
  }
  if (bias_shared_singleton_mode() && delta.new_values.exists(accel_bias_key)) {
    delta.new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      accel_bias_key,
      shared_state.accel_bias,
      (shared_state_frozen && exp_freeze_accel_bias_) ? frozen_shared_noise : imu_bias_noise);
  ...
  if (bias_lagged_keyed_mode()) {
    const bool have_previous_bias =
      previous_segment &&
      key_known(gyro_bias_key_for_auxiliary(previous_segment->auxiliary_index)) &&
      key_known(accel_bias_key_for_auxiliary(previous_segment->auxiliary_index));
    if (have_previous_bias) {
      const gtsam::Key previous_gyro_bias_key = gyro_bias_key_for_auxiliary(previous_segment->auxiliary_index);
      const gtsam::Key previous_accel_bias_key = accel_bias_key_for_auxiliary(previous_segment->auxiliary_index);
      delta.new_factors.emplace_shared<gtsam::BetweenFactor<gtsam::Vector3>>(
        previous_gyro_bias_key,
        current_gyro_bias_key,
        gtsam::Vector3::Zero(),
        imu_bias_noise);
      delta.new_factors.emplace_shared<gtsam::BetweenFactor<gtsam::Vector3>>(
        previous_accel_bias_key,
        current_accel_bias_key,
        gtsam::Vector3::Zero(),
        imu_bias_noise);
    } else {
      delta.new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
        current_gyro_bias_key,
        mirror_values.at<gtsam::Vector3>(current_gyro_bias_key),
        (shared_state_frozen && exp_freeze_gyro_bias_) ? frozen_shared_noise : imu_bias_noise);
      delta.new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
        current_accel_bias_key,
        mirror_values.at<gtsam::Vector3>(current_accel_bias_key),
        (shared_state_frozen && exp_freeze_accel_bias_) ? frozen_shared_noise : imu_bias_noise);
    }
  }
  if (gravity_graph_key_enabled() && delta.new_values.exists(gravity_key)) {
    delta.new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      gravity_key,
      shared_state.gravity,
      (shared_state_frozen && exp_freeze_gravity_) ? frozen_shared_noise : gravity_noise);
  }
  if (bias_shared_singleton_mode() && shared_state_frozen && exp_freeze_gyro_bias_) {
    delta.new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      gyro_bias_key,
      shared_state.gyro_bias,
      frozen_shared_noise);
  }
  if (bias_shared_singleton_mode() && shared_state_frozen && exp_freeze_accel_bias_) {
    delta.new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      accel_bias_key,
      shared_state.accel_bias,
      frozen_shared_noise);
  }
  if (bias_lagged_keyed_mode() && shared_state_frozen && exp_freeze_gyro_bias_) {
    delta.new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      current_gyro_bias_key,
      shared_state.gyro_bias,
      frozen_shared_noise);
  }
  if (bias_lagged_keyed_mode() && shared_state_frozen && exp_freeze_accel_bias_) {
    delta.new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      current_accel_bias_key,
      shared_state.accel_bias,
      frozen_shared_noise);
  }
  if (gravity_graph_key_enabled() && shared_state_frozen && exp_freeze_gravity_) {
    delta.new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
      gravity_key,
      shared_state.gravity,
      frozen_shared_noise);
  ...
  // Shared singleton states remain persistent keys only for the modes that still
  // use them (for example shared gravity, or legacy shared bias mode). Lagged
  // bias keys are stamped as ordinary auxiliary states and retire with the lag.
  ...
  if (velocity_explicit_optimization_for_solve &&
      !exp_disable_current_velocity_prior_ &&
      delta.new_values.exists(current_velocity_key)) {
    delta.new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Vector3>>(
  ...
  const BoundaryBridgeSelection incremental_boundary_bridge_selection =
    build_boundary_bridge_selection(
      active_state_set,
      frontend_shadow_result.pose_diagnostics.query_support_keys,
      std::vector<std::size_t>(
        current_segment.control_indices.begin(),
        current_segment.control_indices.end()),
      current_segment.auxiliary_index,
      velocity_optimized,
      navigation_contribution.activation.include_clock_states,
      navigation_contribution.activation.retained_keys,
      mirror_values);
  ...
  if (delta.new_values.exists(current_velocity_key) || key_known(current_velocity_key)) {
    append_unique_key(&delta.query_keys, current_velocity_key);
    append_unique_key(&delta.mirror_sync_keys, current_velocity_key);
  }
  if (bias_lagged_keyed_mode()) {
    append_unique_key(&delta.query_keys, current_gyro_bias_key);
    append_unique_key(&delta.query_keys, current_accel_bias_key);
    append_unique_key(&delta.mirror_sync_keys, current_gyro_bias_key);
    append_unique_key(&delta.mirror_sync_keys, current_accel_bias_key);
  }
  ...
  for (const auto auxiliary_index : boundary_auxiliary_indices) {
    ...
    if (bias_lagged_keyed_mode()) {
      const gtsam::Key boundary_gyro_bias_key = gyro_bias_key_for_auxiliary(auxiliary_index);
      if (key_known(boundary_gyro_bias_key)) {
        append_unique_key(&delta.query_keys, boundary_gyro_bias_key);
        append_unique_key(&delta.mirror_sync_keys, boundary_gyro_bias_key);
      }
      const gtsam::Key boundary_accel_bias_key = accel_bias_key_for_auxiliary(auxiliary_index);
      if (key_known(boundary_accel_bias_key)) {
        append_unique_key(&delta.query_keys, boundary_accel_bias_key);
        append_unique_key(&delta.mirror_sync_keys, boundary_accel_bias_key);
      }
    }
    if (navigation_layer_enabled) {
      const gtsam::Key clock_key = iap::bspline_clock_key(auxiliary_index);
  ...
  gtsam::KeyVector shared_imu_keys;
  if (bias_shared_singleton_mode()) {
    shared_imu_keys.push_back(gyro_bias_key);
    shared_imu_keys.push_back(accel_bias_key);
  }
  if (gravity_graph_key_enabled()) {
    shared_imu_keys.push_back(gravity_key);
  }
  ...
  auto solver_result = unified_graph_solver_->apply_delta(delta);
  ...
  const gtsam::Values postsolve_authoritative_values =
    build_postsolve_authoritative_values(solver_result.estimate_subset, navigation_layer_enabled);
  sync_postsolve_registry_mirrors(
    postsolve_authoritative_values,
    current,
    local_contribution.uses_shared_imu_state || navigation_layer_enabled);

  const iap::SplineActiveStateSet post_active_state_set =
    fixed_lag_registry_.active_state_set(
      postsolve_authoritative_values,
      min_active_stamp,
      navigation_layer_enabled);
  {
  ...
  new_frame->T_world_lidar = Eigen::Isometry3d(selected_final_pose_query.pose_lidar.matrix());
  new_frame->T_world_imu = new_frame->T_world_lidar * T_lidar_imu;
  new_frame->v_world_imu = postsolve_context.authoritative_values.exists(current_velocity_key)
    ? postsolve_context.authoritative_values.at<gtsam::Vector3>(current_velocity_key)
    : Eigen::Vector3d::Zero();
  write_frame_bias_from_values(
    postsolve_context.authoritative_values,
  ...
  if (jump_diagnostics_enabled_) {
    ...
    const auto strict_lidar_layout = local_input.lidar_layout_override;
    const auto start_layout_diag =
      inspect_layout_query(strict_lidar_layout, jump_row.start_pose_query_time, iap::SplineSensorId::Lidar);
    const auto strict_layout_diag = start_layout_diag;
    const auto [fallback_domain_begin, fallback_domain_end] = layout_domain_bounds(strict_lidar_layout);
    ...
    gtsam::KeyVector carried_boundary_keys = pose_keys_from_control_indices(boundary_control_indices);
    const auto boundary_aux_keys = auxiliary_keys_from_indices(boundary_auxiliary_indices, navigation_layer_enabled);
    carried_boundary_keys.insert(
      carried_boundary_keys.end(),
      boundary_aux_keys.begin(),
      boundary_aux_keys.end());
    jump_row.carried_boundary_oldest_key_summary =
      summarize_oldest_key(sort_unique_keys(std::move(carried_boundary_keys)));
    jump_row.oldest_survivor_key_summary =
      summarize_oldest_key(incremental_boundary_bridge_selection.boundary_anchor_keys);
    jump_row.retained_pose_control_key_count =
      count_pose_control_keys(incremental_boundary_bridge_selection.carried_prior_retained_keys);
    jump_row.strict_local_needed_support_key_count =
      count_pose_control_keys(incremental_boundary_bridge_selection.strict_local_needed_support_keys);
    jump_row.retained_minus_strict_local_key_excess =
      static_cast<long long>(jump_row.retained_pose_control_key_count) -
      static_cast<long long>(jump_row.strict_local_needed_support_key_count);
    jump_row.carried_prior_retained_keys_summary =
      summarize_keys(incremental_boundary_bridge_selection.carried_prior_retained_keys);
    jump_row.postsolve_active_window_value_source_kind =
      postsolve_context.active_window_value_source_kind;
    jump_row.postsolve_strict_local_value_source_kind =
      postsolve_context.strict_local_value_source_kind;
...
void OdometryEstimationBSpline::maybe_write_jump_diagnostics(const JumpDiagnosticsRow& row) {
  if (!jump_diagnostics_enabled_) {
    return;
  }

  std::FILE* file = std::fopen(
    jump_diagnostics_csv_path_.c_str(),
    jump_diagnostics_header_written_ ? "a" : "w");
  if (!file) {
    logger->warn(
      "failed to open jump diagnostics csv '{}': {}",
      jump_diagnostics_csv_path_,
      std::strerror(errno));
    return;
  }

  if (!jump_diagnostics_header_written_) {
    std::fputs(jump_diagnostics_csv_header(), file);
    jump_diagnostics_header_written_ = true;
  }

  write_jump_diagnostics_row(file, row);
  std::fclose(file);
}

void OdometryEstimationBSpline::maybe_log_jump_event(const JumpDiagnosticsRow& row) const {
  if (!jump_diagnostics_enabled_ || !jump_threshold_exceeded(row)) {
    return;
  }
  ...
  logger->warn(
    ...
    row.lidar_layout_domain_end,
    row.start_pose_source_kind,
...
iap::CTLocalFrontend::Input OdometryEstimationBSpline::make_frontend_input(
  ...
  input.target_ivox =
    target_ref.target_snapshot && !target_ref.target_snapshot->voxel_points().empty()
      ...
      : nullptr;
  input.target_point_count = target_ref.point_count;
  input.target_voxel_count = target_ref.voxel_count;
  ...
  input.seed_mode = frontend_seed_mode_;
  input.use_lagged_bias = bias_lagged_keyed_mode();
  input.use_external_gravity = gravity_external_reference_enabled();
  input.gravity_world = authoritative_gravity_reference();
...
iap::CTLocalFrontend::LayerInput OdometryEstimationBSpline::make_local_layer_input(
  ...
  const bool gnss_constraints = solve_has_gnss_constraints();
  ...
  input.enable_lidar_factor_profiling =
    lidar_factor_profile_ || lidar_warn_degeneracy_ || lidar_export_baseline_csv_ ||
    lidar_factor_internal_profile_enabled_ || jump_diagnostics_enabled_;
  input.enable_graph_problem_size = frontend_frame_profile_enabled_ && graph_problem_size_enabled_;
  ...
  input.enable_velocity_factor =
    velocity_explicit_optimization_enabled(gnss_constraints) && !exp_disable_velocity_factor_;
  input.use_lagged_bias = bias_lagged_keyed_mode();
  input.use_external_gravity = gravity_external_reference_enabled();
  input.gravity_world = authoritative_gravity_reference();
...
iap::CTLocalFrontend::LayerInput OdometryEstimationBSpline::make_local_layer_delta_input(
  ...
  std::shared_ptr<const iap::SplineStateLayout> factor_layout) const {
  iap::CTLocalFrontend::LayerInput input;
  const bool gnss_constraints = solve_has_gnss_constraints(segment);
  input.graph_context.layout = std::move(factor_layout);
...
void OdometryEstimationBSpline::reset_unified_graph_solver() {
  if (unified_solver_mode_ == iap::BSplineUnifiedSolverMode::INCREMENTAL_SMOOTHER) {
    ...
    incremental_params.enableDetailedResults = solver_update_profile_enabled_ || jump_diagnostics_enabled_;
