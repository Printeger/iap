...
## Unreleased
- test(planner-p1-soft-objective-acceptance-incompatibility): IAP-RQ-320 / IAP-RQ-400 / IAP-RQ-410 — retained fresh smooth-CVaR pair `d7f1a11470ab454398968eebc6288b60` / `a0a1d5ad774a4b6f91c27709abc6f5d4` at clean `d1a7d2a`. Both sole v4 preflights passed and the pair's sole formal analysis `fb41d1aaf6934fdd8d6441fb39d35f2e` generated 39/39 nonempty figures, but aligned mean fell `0.4196563658 -> 0.4184994876` while max rose `0.4236384817 -> 0.4242653019`. The same pair also failed baseline post-startup P0 freshness and strict workspace/install runtime-path provenance because reproducible binaries had been rebuilt under `/tmp`; those failures are retained and not waived. A deterministic fixed-200 tie counterexample now proves that mean, normalized LSE (`T=0.01`), and smooth CVaR (`alpha=0.90`, `T=0.01`) can each strictly descend when one sample rises by epsilon and the other 199 fall by epsilon, even though exact max strictly increases. The test-only optimizer entry now copies the production aggregation provenance into its v4 trace, closing a focused-suite evidence gap without affecting runtime optimization. No production behavior or fixed invariant changed. The three prescribed H4 alternatives therefore cannot logically guarantee the existing simultaneous mean/max formal gate; a design decision is required before further formal runs, and P1-3 remains prohibited.
