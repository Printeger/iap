...
- fix(planner-p1-2-health-gradient-admission): IAP-RQ-320 / IAP-RQ-400 / IAP-RQ-410 — repair P0 freshness diagnostics, accepted-context classification, P1 gradient effectiveness, and same-generation retry storms without changing the fixed `0.00001` integrity weight, the 1.0 s stale timeout, or P5 semantics.
  ...
  - Added generation-gated `P1ReplanAdmission`: one rejected stale generation permits one expensive attempt, repeated ticks defer while the existing polynomial executes, and the next healthy generation unlocks one retry. P5 runtime/final/emergency paths bypass this controller.
  - The P1-2 analyzer now derives CSV/JSON/dashboard/final verdicts from structured hard-gate records, requires 16 nonempty PNGs, separates spatial/temporal/coverage evidence, selects the exact accepted-snapshot risk cloud by recorded frame/stamp, and renders unavailable scene evidence fail-closed.
  - Focused verification before runtime validation: analyzer suites P1-1/P1-2/P5-1/P0-6 = 15/39/111/2 passing; CTests `test_p1_integrity_cost` 19, `test_planning_risk_context` 7, `test_p0_risk_grid_runtime` 30, and `test_p1_replan_admission` 3 passing; aggregate IAP and planner builds pass. Historical workspace lint/XML failures remain separately recorded. Fresh P1-1/P1-2 evidence is pending the prescribed one-shot run.
