...
Observation: The same fixed-200 acceptance mismatch is mathematical, not a remaining temperature/tail tuning question. Let the initial profile be `c=(C,...,C)` and the candidate be `c'=(C+epsilon,C-epsilon,...,C-epsilon)` with 200 samples and positive sufficiently small `epsilon`. Exact max rises by `epsilon`; mean falls by `198*epsilon/200`. At the tied profile, fixed mean, normalized LSE, and entropy-normalized smooth CVaR are differentiable and permutation-symmetric, and each assigns gradient weight `1/200` to every sample. Their common directional derivative is therefore `(1-199)/200=-0.99`, so all three scalar objectives strictly decrease for sufficiently small `epsilon` while exact max strictly increases. `P1IntegrityCostTest.Fixed200SmoothObjectivesCannotImplyHardMaxNonRegressionAtTie` evaluates the actual prescribed LSE and smooth-CVaR formulas at `epsilon=1e-5` and passes this four-way inequality. The optimizer's additional base and anchor terms cannot strengthen a missing implication from the P1 scalar to exact max.

Verdict: BLOCKED BY DESIGN (formal FAIL; deterministic algorithm/acceptance incompatibility).

Conclusion: The three permitted H4 modes have been exhausted: fixed mean, normalized LSE at `T=0.01`, and smooth CVaR at `alpha=0.90`, `T=0.01` each retained a fresh formal counterexample with lower aligned mean and higher aligned max, and the common fixed-200 tie construction proves that no one of these differentiable scalar descents can guarantee the current hard exact-max gate. Continuing to tune `T`, `alpha`, seeds, anchor, or reference displacement cannot repair that logical implication. A design authority must choose whether to (1) authorize an exact-max hard candidate/optimizer constraint, (2) authorize a nonsmooth/epigraph max objective, or (3) redefine formal effectiveness to the chosen differentiable scalar plus mean. Until that choice, fixed lambda `1e-5`, P0/P5 semantics, and all evidence remain unchanged; P1-3 is not authorized or run.
