...
Observation: The same fixed-200 acceptance mismatch is mathematical, not a remaining temperature/tail tuning question. Define one deterministic feasible trajectory-profile family with exactly one scalar degree of freedom: `c(x)=(C+x,C-x,...,C-x)` over 200 samples. No nonzero member can satisfy the formal gate: for `x>0`, exact max is `C+x>C`; for `x<0`, mean is `C-198x/200>C`; and `x=0` has no strict improvement. An all-samples-down direction is outside this fixture's declared feasible set. At `x=0`, fixed mean, normalized LSE, and entropy-normalized smooth CVaR are differentiable and permutation-symmetric, and each assigns gradient weight `1/200` to every sample. Their common derivative is `(1-199)/200=-0.99`, so all three objectives strictly descend for sufficiently small positive `x`, necessarily choosing the max-regressing side of a feasible family that contains no gate-passing alternative. `P1IntegrityCostTest.Fixed200OneDimensionalFixtureHasNoMeanMaxFeasibleImprovement` evaluates the actual prescribed LSE and smooth-CVaR formulas at `epsilon=1e-5` and proves both sides of the feasible-family exclusion. The optimizer's additional base and anchor terms cannot create a missing gate-feasible member.

Verdict: BLOCKED BY DESIGN (formal FAIL; deterministic algorithm/acceptance incompatibility).

Conclusion: The three permitted H4 modes have been exhausted: fixed mean, normalized LSE at `T=0.01`, and smooth CVaR at `alpha=0.90`, `T=0.01` cannot satisfy the same one-dimensional fixed-200 feasible fixture under the simultaneous mean/max gate. Continuing to tune `T`, `alpha`, seeds, anchor, or reference displacement cannot add a gate-feasible member to that family. The fresh CVaR pair above remains retained but is not accepted formal evidence because baseline P0 freshness, runtime-path provenance, and the obsolete manifest baseline identity fail closed. A design authority must choose whether to (1) authorize an exact-max hard candidate/optimizer constraint, (2) authorize a nonsmooth/epigraph max objective, or (3) redefine formal effectiveness to the chosen differentiable scalar plus mean. Until that choice, fixed lambda `1e-5`, P0/P5 semantics, and all evidence remain unchanged; P1-3 is not authorized or run.
