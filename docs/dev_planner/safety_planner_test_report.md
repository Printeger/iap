...
### Superseding P1 fixed-lambda diagnostic entry terminal record — 2026-08-06 context fix

Observation: Fresh enabled run `d00bac8e15a84bb7b06412513cebafc0` completed at clean HEAD `b8a6fd3895a707be53ec96cc0e6eb114c6c935fd`, export `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455`, and bag `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T200703Z`. Recorder, metadata, manifest, validator, and v4 provenance finalized. Its sole preflight invocation returned `passed=true`, `errors=[]`; its sole diagnostic invocation returned PASS and generated all ten required figures. Across 14 attempts, all 56 candidates were optimizer-successful, rank-eligible, P1-descending, and fixed `200/200`. Every attempt had exactly one selected and accepted winner; all 14 winners strictly reduced mean or max, had negative raw-gradient/displacement alignment, and used `shared_forward_time_window` replacement evidence. The run recorded 56 distinct final control-point hashes and 84 nonzero final control-point/risk-profile pairwise entries. No rejected-retained branch occurred because each selected winner replaced its incumbent.

Verdict: PASS (entry-to-formal only).

Conclusion: This record supersedes the generated diagnostic-only terminal wording for run `d00bac8e15a84bb7b06412513cebafc0` and authorizes one wholly fresh, serial fixed-lambda P1-1/P1-2 formal pair. It does not authorize or execute P1-3.

## 2026-08-06 fixed-lambda post-corner smoke diagnosis

Observation: Fresh run `8bd9e2eb18a242a89b64c64346ce113d` at clean HEAD `ba2d44b917d01092c94672504fb41836708aab6a` passed the one-shot preflight verifier and one-shot diagnostic analyzer. Attempt 16 contained 35 unsupported lattice samples. Its 138 failing interpolation corners were all `inflated_neighbor` (`raw_occupied=0`, `inflated_occupied=1`) and were bound to stable snapshot/occupancy generations `26/30` with a fixed cloud stamp. The collocated base-collision samples remained free, so the P0 result is a legitimate interpolation-support result and no P0 semantic change is warranted.

Observation: Attempt 17 then produced full `200/200` support. The selected optimizer candidate reported mean/max `0.4118128307/0.4247065045`, negative raw-gradient/displacement alignment, and an accepted replacement decision. After STEP3 feasibility refinement, the authoritative accepted profile reported `0.4145949651/0.4232666943`. The refinement improved max but regressed mean relative to the selected trajectory, so the published trajectory no longer closes the evidence-backed replacement decision.

Verdict: FAIL (entry-to-formal gate).

Conclusion: P0 occupied attribution is closed without changing P0. Formal scheduling remains blocked until final feasibility refinement is covered by the same fixed-snapshot P1 non-regression decision and the authoritative accepted profile agrees with the recorded replacement disposition. This terminal record supersedes the analyzer-only PASS for progression purposes.

### Diagnostic entry-gate terminal record (superseding)

Observation: The one-shot v4 preflight returned `passed=true`, `errors=[]`, and the one-shot diagnostic produced ten nonempty figures with PASS for run `18e6a02f7a9b4b14ab51ab9caa085869` at clean HEAD `8603579ad109ae0b3e4f8e11bc1353cba6868d59`. Attempts 19–21 each contain four full-support optimizer-success candidates and exactly one rank-eligible selected winner. Every selected winner has fixed-200 mean/max self-descent and negative raw-gradient/displacement alignment; every attempt retains four distinct final control-point hashes. Attempt 19 candidate 1 improved the incumbent and was published. The final authoritative accepted profile is objective-applied with `200/200` valid samples, mean `0.3929817277089576`, max `0.4220151185804595`, and no fallback.
...
![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037033846/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 16 fixed-200 candidate rows were emitted across 4 attempts.

Run ID: `da84b983b1974671982952e59ea8b433`; HEAD: `0780b7c343831a81b0662f6a4f80775d5523075c`; runtime hashes: `bspline_library=33b926767dc4d841cefc90af650a16145846905c68a43cb80836f37531f0c8e6, launch=6babf7d586ed13d312f0b13830b53a55dcf1aed99e86687ebc8bb57e64b8f331, planner_executable=380655319ffc4c615f74f46bd8fc616ca3092b8437b6d3ad62e25ebddabbe5ff`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037033846`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T172353Z`; attempt/candidate/generation/query-base: `17/1/29/1657065616.4860821,17/2/29/1657065616.4860821,17/3/29/1657065616.4860821,17/4/29/1657065616.4860821,18/1/30/1657065616.9860859,18/2/30/1657065616.9860859,18/3/30/1657065616.9860859,18/4/30/1657065616.9860859,19/1/32/1657065617.9850826,19/2/32/1657065617.9850826,19/3/32/1657065617.9850826,19/4/32/1657065617.9850826,20/1/33/1657065618.4860861,20/2/33/1657065618.4860861,20/3/33/1657065618.4860861,20/4/33/1657065618.4860861`; snapshot/query-base window: `2026-08-06T17:23:53Z to 2026-08-06T17:24:24Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037033846/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037033846/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037033846/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037033846/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037033846/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037033846/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037033846/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037033846/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037033846/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `da84b983b1974671982952e59ea8b433`.
...
Observation: The one-shot v4 preflight returned `passed=true`, `errors=[]`, and the one-shot diagnostic produced ten nonempty figures with PASS for run `da84b983b1974671982952e59ea8b433` at clean HEAD `0780b7c343831a81b0662f6a4f80775d5523075c`. Attempts 17–20 each contain four full-support optimizer-success candidates, exactly one selected winner, fixed-200 mean/max self-descent, and negative raw-gradient/displacement alignment. Risk-first ranking chose candidate 1 in every attempt; attempt 18 improved the planning-epoch incumbent and was published. Every unselected row has `replacement_reason=not_selected`; every generation contains at most one final `p1_admission` and one acquisition. Later unsupported base fallbacks remain visible and are not counted as P1 effectiveness.
...
![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037913769/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 12 fixed-200 candidate rows were emitted across 3 attempts.

Run ID: `18e6a02f7a9b4b14ab51ab9caa085869`; HEAD: `8603579ad109ae0b3e4f8e11bc1353cba6868d59`; runtime hashes: `bspline_library=33b926767dc4d841cefc90af650a16145846905c68a43cb80836f37531f0c8e6, launch=6babf7d586ed13d312f0b13830b53a55dcf1aed99e86687ebc8bb57e64b8f331, planner_executable=380655319ffc4c615f74f46bd8fc616ca3092b8437b6d3ad62e25ebddabbe5ff`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037913769`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T173833Z`; attempt/candidate/generation/query-base: `19/1/29/1657065616.5010147,19/2/29/1657065616.5010147,19/3/29/1657065616.5010147,19/4/29/1657065616.5010147,20/1/31/1657065617.492399,20/2/31/1657065617.492399,20/3/31/1657065617.492399,20/4/31/1657065617.492399,21/1/32/1657065617.9924142,21/2/32/1657065617.9924142,21/3/32/1657065617.9924142,21/4/32/1657065617.9924142`; snapshot/query-base window: `2026-08-06T17:38:33Z to 2026-08-06T17:39:04Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037913769/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037913769/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037913769/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037913769/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037913769/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037913769/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037913769/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037913769/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786037913769/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `18e6a02f7a9b4b14ab51ab9caa085869`.
...
![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786040284900/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 8 fixed-200 candidate rows were emitted across 2 attempts.

Run ID: `8bd9e2eb18a242a89b64c64346ce113d`; HEAD: `ba2d44b917d01092c94672504fb41836708aab6a`; runtime hashes: `bspline_library=96ef5d2e62034e4e46ebe42fadadfde9aa90497f8a78bd83aaa7943b8b8b929b, launch=6babf7d586ed13d312f0b13830b53a55dcf1aed99e86687ebc8bb57e64b8f331, planner_executable=c64963cf75b9ca38f3b48f47c6983db8f5d3e9acd539b86fe1cb5ad351636fed`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786040284900`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T181804Z`; attempt/candidate/generation/query-base: `15/1/25/1657065614.4857893,15/2/25/1657065614.4857893,15/3/25/1657065614.4857893,15/4/25/1657065614.4857893,17/1/27/1657065615.4857955,17/2/27/1657065615.4857955,17/3/27/1657065615.4857955,17/4/27/1657065615.4857955`; snapshot/query-base window: `2026-08-06T18:18:04Z to 2026-08-06T18:18:35Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786040284900/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786040284900/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786040284900/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786040284900/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786040284900/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786040284900/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786040284900/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786040284900/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786040284900/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `8bd9e2eb18a242a89b64c64346ce113d`.
...
![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041255592/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 4 fixed-200 candidate rows were emitted across 1 attempts.

Run ID: `2765b06408b24412b6f1f6e182f43391`; HEAD: `993f3a9e4433560c9920308d7bda509103e6219a`; runtime hashes: `bspline_library=122fa73f9dce2a92f2c171476fb9f07eb73fb1a014cdac5a795e7fdbd1e74ebe, launch=6babf7d586ed13d312f0b13830b53a55dcf1aed99e86687ebc8bb57e64b8f331, planner_executable=4747321d3a2bc7975f7427023ff6ccbfd329698c87270dda83a7c8561b7fa2d3`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041255592`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T183415Z`; attempt/candidate/generation/query-base: `21/1/39/1657065620.9831257,21/2/39/1657065620.9831257,21/3/39/1657065620.9831257,21/4/39/1657065620.9831257`; snapshot/query-base window: `2026-08-06T18:34:15Z to 2026-08-06T18:34:46Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041255592/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041255592/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041255592/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041255592/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041255592/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041255592/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041255592/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041255592/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041255592/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `2765b06408b24412b6f1f6e182f43391`.

Verdict: PASS (diagnostic only).

Conclusion: This terminal record supersedes no other run and does not grant formal progression.

## 2026-08-06 fixed-lambda post-refinement smoke terminal

Observation: Fresh clean-HEAD run `2765b06408b24412b6f1f6e182f43391` at `993f3a9e4433560c9920308d7bda509103e6219a` passed the one-shot evidence-bundle preflight and diagnostic analyzer. Attempt 21 selected candidate 1 with optimizer seed/post mean/max `0.4009195021/0.4204885989` to `0.4006891667/0.4202856041`; the same-snapshot incumbent measured `0.4042561745/0.4251127702`. The authoritative final accepted profile is full `200/200` and exactly matches the optimizer post mean/max `0.4006891667/0.4202856041`. The final disposition is `p1_refined_risk_preference_improved`, with no rejected replacement requiring retained artifacts.

Verdict: PASS (entry-to-formal gate).

Conclusion: The STEP3 publication-closure regression is closed for this fresh run. This terminal record supersedes the earlier analyzer-only progression disclaimer for run `2765b06408b24412b6f1f6e182f43391` and permits a new serial P1-1/P1-2 formal pair; it does not itself establish formal effectiveness or authorize P1-3.

## 2026-08-06 diagnostic-smoke figure evidence (non-authoritative)

### Full scenario top-down

![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041893871/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 16 fixed-200 candidate rows were emitted across 4 attempts.

Run ID: `4e4c0347e5d04fcaa7b459ab78617ddf`; HEAD: `265ad960310b45f410a3ed2e103c72b43d073744`; runtime hashes: `bspline_library=122fa73f9dce2a92f2c171476fb9f07eb73fb1a014cdac5a795e7fdbd1e74ebe, launch=6babf7d586ed13d312f0b13830b53a55dcf1aed99e86687ebc8bb57e64b8f331, planner_executable=b173379250ede8e67c8154fc0dc88a5363bb52655273664dde7dce77bbb3463b`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041893871`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T184453Z`; attempt/candidate/generation/query-base: `23/1/37/1657065619.9891949,23/2/37/1657065619.9891949,23/3/37/1657065619.9891949,23/4/37/1657065619.9891949,24/1/39/1657065620.990191,24/2/39/1657065620.990191,24/3/39/1657065620.990191,24/4/39/1657065620.990191,25/1/40/1657065621.49019,25/2/40/1657065621.49019,25/3/40/1657065621.49019,25/4/40/1657065621.49019,26/1/41/1657065621.9902055,26/2/41/1657065621.9902055,26/3/41/1657065621.9902055,26/4/41/1657065621.9902055`; snapshot/query-base window: `2026-08-06T18:44:53Z to 2026-08-06T18:45:24Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041893871/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041893871/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041893871/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041893871/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041893871/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041893871/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041893871/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041893871/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786041893871/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `4e4c0347e5d04fcaa7b459ab78617ddf`.
...
![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042579693/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 20 fixed-200 candidate rows were emitted across 5 attempts.

Run ID: `e55de8677bb64bd58fd11989e7182253`; HEAD: `c812665094381392eba3950a080492bc1d7827d5`; runtime hashes: `bspline_library=122fa73f9dce2a92f2c171476fb9f07eb73fb1a014cdac5a795e7fdbd1e74ebe, launch=d3d8c5987124536d2b24a3f6207841d98490a382cc63d140e239ecbb68bd29d1, planner_executable=f811b68396f20256e640e6a63c6a594a9d821a12806e779c028be61340d9c6ea`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042579693`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T185619Z`; attempt/candidate/generation/query-base: `21/1/31/1657065617.4921715,21/2/31/1657065617.4921715,21/3/31/1657065617.4921715,21/4/31/1657065617.4921715,22/1/33/1657065618.4921842,22/2/33/1657065618.4921842,22/3/33/1657065618.4921842,22/4/33/1657065618.4921842,23/1/35/1657065619.4921679,23/2/35/1657065619.4921679,23/3/35/1657065619.4921679,23/4/35/1657065619.4921679,24/1/36/1657065619.9922018,24/2/36/1657065619.9922018,24/3/36/1657065619.9922018,24/4/36/1657065619.9922018,25/1/37/1657065620.4921939,25/2/37/1657065620.4921939,25/3/37/1657065620.4921939,25/4/37/1657065620.4921939`; snapshot/query-base window: `2026-08-06T18:56:19Z to 2026-08-06T18:56:50Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042579693/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042579693/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042579693/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042579693/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042579693/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042579693/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042579693/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042579693/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042579693/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `e55de8677bb64bd58fd11989e7182253`.

Verdict: PASS (diagnostic only).

Conclusion: This terminal record supersedes no other run and does not grant formal progression.

## P1-2 fixed-lambda formal pair 8 future-window attribution failure — 2026-08-06

Observation: The serial 90 s runs completed at clean HEAD `04a33ba`. P1-1 run `a108aec05ef844b19964b9f38540c078` used export `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042666221` and bag `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T185746Z`; P1-2 run `52960d0b50554dc18c00c29f33763300` used export `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786042776507` and bag `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T185936Z`. Both one-shot preflights passed. The prescribed formal analyzer was invoked exactly once as analysis `90edaf6b74a44ac1bccaf2da52e1efb8`, produced all 39 figures, reported `inconclusive=[]`, and wrote `metadata/safety_planner_analysis_summary.json`. Its whole-profile comparison failed because P1-2 mean/max were `0.41943539390358`/`0.42426361302371696`, versus P1-1 `0.4078115196209177`/`0.42837479021754565`. The accepted profiles covered unequal remaining trajectory lengths (`0.387965 m` versus `2.020000 m`), so the old mean included unshared, already-traversed low-risk P1-1 history. A deterministic terminal-common-arc replay over `0.387965 m` gives P1-2 `0.41943539390358`/`0.42426361302371696` versus P1-1 `0.4229636624769948`/`0.42837479021754565`, strictly reducing both statistics. The same retained artifacts exposed three evidence-classification defects: bounded startup rows were counted as persistent P0 failure, legal admission-only/acquire-only generations were counted as singleflight failures, and a metrics-only baseline was required to contain objective-applied displacement. P1-1 also lacked an exact accepted-snapshot risk-cloud publication because the periodic visualization publisher was throttled at that stamp.

Verdict: FAIL (formal analyzer result retained; deterministic diagnosis only).

Conclusion: This pair is retained and must not be re-analyzed or reused. The implementation now compares only the shared future trajectory arc while preserving raw profiles, treats bounded startup and legal lifecycle terminals explicitly, keeps baseline displacement diagnostic, and force-publishes the exact accepted snapshot cloud. No support, lambda, freshness, occupied, collision, P5, or candidate-selection gate was weakened. A fresh diagnostic smoke and then a fresh serial formal pair are required; this record grants neither progression nor P1-3 permission.

## 2026-08-06 diagnostic-smoke figure evidence (non-authoritative)

### Full scenario top-down

![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 16 fixed-200 candidate rows were emitted across 4 attempts.

Run ID: `086cf8f90659456f863c8d1ebfbaa2ca`; HEAD: `20e7abcaf358bcc049b3edcfed5b0cb4e832760e`; runtime hashes: `bspline_library=122fa73f9dce2a92f2c171476fb9f07eb73fb1a014cdac5a795e7fdbd1e74ebe, launch=d3d8c5987124536d2b24a3f6207841d98490a382cc63d140e239ecbb68bd29d1, planner_executable=5dd43da2f803e49b533649e8edcaba93eca5ccc837b35a0673b28d7244812173`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T192300Z`; attempt/candidate/generation/query-base: `20/1/37/1657065619.9820757,20/2/37/1657065619.9820757,20/3/37/1657065619.9820757,20/4/37/1657065619.9820757,21/1/39/1657065620.9830775,21/2/39/1657065620.9830775,21/3/39/1657065620.9830775,21/4/39/1657065620.9830775,22/1/40/1657065621.4841075,22/2/40/1657065621.4841075,22/3/40/1657065621.4841075,22/4/40/1657065621.4841075,23/1/41/1657065621.9830775,23/2/41/1657065621.9830775,23/3/41/1657065621.9830775,23/4/41/1657065621.9830775`; snapshot/query-base window: `2026-08-06T19:23:00Z to 2026-08-06T19:23:31Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `086cf8f90659456f863c8d1ebfbaa2ca`.

Verdict: PASS (diagnostic only).

Conclusion: This terminal record supersedes no other run and does not grant formal progression.

## P1 fixed-lambda future-window entry gate — 2026-08-06

Observation: Fresh enabled smoke run `086cf8f90659456f863c8d1ebfbaa2ca` at clean HEAD `20e7abcaf358bcc049b3edcfed5b0cb4e832760e` used export `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786044180654` and bag `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T192300Z`. Recorder, bag metadata, validator, manifest, runtime hashes, and v4 sidecars finalized. The one-shot bundle verifier returned `passed=true`, `errors=[]`; the one-shot explicit diagnostic returned PASS and generated all 10 required diagnostic figures. Four optimizer attempts emitted 16 candidates: every candidate was optimizer-successful, rank-eligible, and `200/200` before and after; every attempt had exactly one selected winner. Selected candidates strictly reduced both fixed-200 mean and max, with raw-gradient/displacement dot products `-0.000206892`, `-0.000148392`, `-0.000260357`, and `-0.000294576`. Final control-point hashes had cardinality 16, accepted/rejected/retained identity closed, and the lifecycle singleflight replay passed with at most one admission per generation.
...
![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 44 fixed-200 candidate rows were emitted across 11 attempts.

Run ID: `9a16ed929f94426ca3dd360b5b91a64c`; HEAD: `75f057af9dcf15ef11ff18d16df89e051c32e8f5`; runtime hashes: `bspline_library=7d7ec7bd923a8411a91c13b20e1c2b536203119bb80ffb78849bf6f30235e14b, launch=d3d8c5987124536d2b24a3f6207841d98490a382cc63d140e239ecbb68bd29d1, planner_executable=50e3dcfc3971e239d420258254241072028963dbb21eed97af1ae2d5b2a1fc05`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T194929Z`; attempt/candidate/generation/query-base: `21/1/35/1657065618.986629,21/2/35/1657065618.986629,21/3/35/1657065618.986629,21/4/35/1657065618.986629,22/1/37/1657065619.9866276,22/2/37/1657065619.9866276,22/3/37/1657065619.9866276,22/4/37/1657065619.9866276,23/1/39/1657065620.9855883,23/2/39/1657065620.9855883,23/3/39/1657065620.9855883,23/4/39/1657065620.9855883,24/1/41/1657065621.9866352,24/2/41/1657065621.9866352,24/3/41/1657065621.9866352,24/4/41/1657065621.9866352,25/1/43/1657065622.9865801,25/2/43/1657065622.9865801,25/3/43/1657065622.9865801,25/4/43/1657065622.9865801,26/1/45/1657065623.9866045,26/2/45/1657065623.9866045,26/3/45/1657065623.9866045,26/4/45/1657065623.9866045,27/1/47/1657065624.9866714,27/2/47/1657065624.9866714,27/3/47/1657065624.9866714,27/4/47/1657065624.9866714,28/1/49/1657065625.9866107,28/2/49/1657065625.9866107,28/3/49/1657065625.9866107,28/4/49/1657065625.9866107,29/1/51/1657065626.9955947,29/2/51/1657065626.9955947,29/3/51/1657065626.9955947,29/4/51/1657065626.9955947,30/1/53/1657065627.9866035,30/2/53/1657065627.9866035,30/3/53/1657065627.9866035,30/4/53/1657065627.9866035,31/1/55/1657065628.9866006,31/2/55/1657065628.9866006,31/3/55/1657065628.9866006,31/4/55/1657065628.9866006`; snapshot/query-base window: `2026-08-06T19:49:29Z to 2026-08-06T19:49:59Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `9a16ed929f94426ca3dd360b5b91a64c`.

Verdict: PASS (diagnostic only).

Conclusion: This terminal record supersedes no other run and does not grant formal progression.

### Superseding P1 fixed-lambda diagnostic entry terminal record — 2026-08-06

Observation: Fresh enabled run `9a16ed929f94426ca3dd360b5b91a64c` completed at clean HEAD `75f057af9dcf15ef11ff18d16df89e051c32e8f5`, export `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786045769025`, and bag `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T194929Z`. Recorder, metadata, manifest, validator, and v4 provenance finalized. Its sole preflight invocation returned `passed=true`, `errors=[]`; its sole diagnostic invocation returned PASS and generated all ten required diagnostic figures. Across 11 attempts, all 44 candidates were optimizer-successful, rank-eligible, and fixed `200/200`; every attempt had exactly one selected and accepted winner, every winner preserved mean/max with strict descent and negative raw-gradient/displacement alignment, and all incumbent rows recorded a positive `shared_forward_time_window`. The run produced 44 distinct final control-point hashes and 66 distinguishable final control-point/risk-profile pairwise entries.
...
![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 56 fixed-200 candidate rows were emitted across 14 attempts.

Run ID: `d00bac8e15a84bb7b06412513cebafc0`; HEAD: `b8a6fd3895a707be53ec96cc0e6eb114c6c935fd`; runtime hashes: `bspline_library=7d7ec7bd923a8411a91c13b20e1c2b536203119bb80ffb78849bf6f30235e14b, launch=d3d8c5987124536d2b24a3f6207841d98490a382cc63d140e239ecbb68bd29d1, planner_executable=50e3dcfc3971e239d420258254241072028963dbb21eed97af1ae2d5b2a1fc05`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T200703Z`; attempt/candidate/generation/query-base: `20/1/28/1657065615.9950902,20/2/28/1657065615.9950902,20/3/28/1657065615.9950902,20/4/28/1657065615.9950902,21/1/30/1657065616.9951053,21/2/30/1657065616.9951053,21/3/30/1657065616.9951053,21/4/30/1657065616.9951053,22/1/32/1657065617.9951828,22/2/32/1657065617.9951828,22/3/32/1657065617.9951828,22/4/32/1657065617.9951828,23/1/34/1657065618.9950826,23/2/34/1657065618.9950826,23/3/34/1657065618.9950826,23/4/34/1657065618.9950826,24/1/36/1657065619.9951041,24/2/36/1657065619.9951041,24/3/36/1657065619.9951041,24/4/36/1657065619.9951041,25/1/38/1657065620.9950969,25/2/38/1657065620.9950969,25/3/38/1657065620.9950969,25/4/38/1657065620.9950969,26/1/40/1657065621.9950895,26/2/40/1657065621.9950895,26/3/40/1657065621.9950895,26/4/40/1657065621.9950895,27/1/42/1657065622.9950886,27/2/42/1657065622.9950886,27/3/42/1657065622.9950886,27/4/42/1657065622.9950886,28/1/44/1657065623.9950943,28/2/44/1657065623.9950943,28/3/44/1657065623.9950943,28/4/44/1657065623.9950943,29/1/46/1657065624.9950802,29/2/46/1657065624.9950802,29/3/46/1657065624.9950802,29/4/46/1657065624.9950802,30/1/48/1657065625.9950857,30/2/48/1657065625.9950857,30/3/48/1657065625.9950857,30/4/48/1657065625.9950857,31/1/50/1657065626.9951015,31/2/50/1657065626.9951015,31/3/50/1657065626.9951015,31/4/50/1657065626.9951015,32/1/52/1657065627.9950995,32/2/52/1657065627.9950995,32/3/52/1657065627.9950995,32/4/52/1657065627.9950995,33/1/54/1657065628.9950776,33/2/54/1657065628.9950776,33/3/54/1657065628.9950776,33/4/54/1657065628.9950776`; snapshot/query-base window: `2026-08-06T20:07:03Z to 2026-08-06T20:07:34Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786046823455/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `d00bac8e15a84bb7b06412513cebafc0`.
...
![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048271899/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 36 fixed-200 candidate rows were emitted across 9 attempts.

Run ID: `011a3f77fe354724a1c5d07024676c2f`; HEAD: `f81474a4b6fb9ee73922e90d5ddbcf51809816be`; runtime hashes: `bspline_library=14a7af1481ebe09ce7dee593e342e8937ead7d19afc0f3f3f2eb3d6fc5e8a162, launch=d3d8c5987124536d2b24a3f6207841d98490a382cc63d140e239ecbb68bd29d1, planner_executable=0e61e0a3f5cbdd355a540188ca3806accfb231cc3c8c10cdcd171c9a0b5de9d3`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048271899`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T203111Z`; attempt/candidate/generation/query-base: `23/1/40/1657065621.4951036,23/2/40/1657065621.4951036,23/3/40/1657065621.4951036,23/4/40/1657065621.4951036,24/1/42/1657065622.4951141,24/2/42/1657065622.4951141,24/3/42/1657065622.4951141,24/4/42/1657065622.4951141,25/1/44/1657065623.4950716,25/2/44/1657065623.4950716,25/3/44/1657065623.4950716,25/4/44/1657065623.4950716,26/1/46/1657065624.4950848,26/2/46/1657065624.4950848,26/3/46/1657065624.4950848,26/4/46/1657065624.4950848,27/1/48/1657065625.4951005,27/2/48/1657065625.4951005,27/3/48/1657065625.4951005,27/4/48/1657065625.4951005,28/1/50/1657065626.4950964,28/2/50/1657065626.4950964,28/3/50/1657065626.4950964,28/4/50/1657065626.4950964,29/1/52/1657065627.4950812,29/2/52/1657065627.4950812,29/3/52/1657065627.4950812,29/4/52/1657065627.4950812,30/1/54/1657065628.4951015,30/2/54/1657065628.4951015,30/3/54/1657065628.4951015,30/4/54/1657065628.4951015,31/1/56/1657065629.4950826,31/2/56/1657065629.4950826,31/3/56/1657065629.4950826,31/4/56/1657065629.4950826`; snapshot/query-base window: `2026-08-06T20:31:11Z to 2026-08-06T20:31:42Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048271899/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048271899/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048271899/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048271899/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048271899/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048271899/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048271899/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048271899/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048271899/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `011a3f77fe354724a1c5d07024676c2f`.

Verdict: PASS (diagnostic only).

Conclusion: This terminal record supersedes no other run and does not grant formal progression.

### Superseding P1 fixed-lambda diagnostic entry record — 2026-08-06 enabled smoke

Observation: Fresh enabled run `011a3f77fe354724a1c5d07024676c2f` at clean HEAD `f81474a4b6fb9ee73922e90d5ddbcf51809816be` passed its sole v4 verifier with `errors=[]` and its sole explicit-bundle diagnostic analyzer with all ten required nonempty figures. Across 9 attempts, all 36 candidates were optimizer-successful, rank-eligible, fixed `200/200`, and P1-descending. Each attempt selected and accepted exactly one winner. Every winner strictly reduced both fixed-200 mean and max and had `raw P1 gradient · displacement < 0`. All 36 initial and final control-point hashes are distinct; the pairwise sidecar preserves 54 nonzero initial and 54 nonzero final control-point/profile comparisons.

Verdict: PASS (diagnostic entry-to-formal).

Conclusion: Candidate convergence, selection, replacement closure, fixed support, gradient alignment, diversity, lifecycle, P0 attribution, and provenance entry gates are green. This authorizes one wholly fresh serial 90-second P1-1/P1-2 formal pair and one analyzer invocation for that pair. It does not authorize P1-3.

### Superseding P1 fixed-lambda formal terminal record — 2026-08-06 dual-timestamp counterexample

Observation: Clean HEAD `af5a4a2` produced fresh serial P1-1 run `eb2093a86c694128bed41ce3f9077997` (export `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048494206`, bag `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T203454Z`) and P1-2 run `306f8baafa184fd2976be702174f9fd9` (export `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786048597745`, bag `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T203637Z`). Both sole v4 preflights passed with `errors=[]`. The pair's sole analyzer invocation, analysis `297909f855ce464ab8421342621dbec7`, wrote all 39 required figures and returned FAIL with `inconclusive=[]` and `warnings=[]`. The common terminal arc was `0.3842844473 m`; aligned P1-2 mean/max `0.4173131735/0.4248570827` were both strictly below P1-1 `0.4256441626/0.4304304593`, so fixed-lambda effectiveness passed. The only independent failure was recorded-scene identity: the non-republishing reference observation stored its later effective-segment epoch `1657065617.8430905` in `trajectory_start_stamp_s`, while the bag's original source B-spline starts at `1657065616.8330884`.
...
### Superseding P1 fixed-lambda formal terminal record — 2026-08-06 recorder-boundary counterexample

Observation: Clean HEAD `f7518ed` produced fresh serial P1-1 run `f560550d96614bcfa9208bb9f1de10fe` (export `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786049999077`, bag `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T205959Z`) and P1-2 run `e81b98a26d864cb68dfd2b442157ea14` (export `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786050159886`, bag `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T210239Z`). The pair's sole analyzer invocation, analysis `7a677e5327914de2aca1566d92eae9f3`, wrote all 39 required/supplementary figures nonempty and returned FAIL with `inconclusive=[]` and `warnings=[]`. The terminal common arc was `0.3795295688 m`; aligned P1-2 mean/max `0.4178118187/0.4246478342` were both strictly below P1-1 `0.4215958089/0.4272623794`. Manifest, fixed lambda, raw P0 health, fixed-200 profiles, objective application, candidate binding, gradient direction, context, coverage, candidate evidence, strict reduction, trajectory stability, P5 isolation, and PNG completeness all passed. The first failed hard gate was recorded-scene alignment: the last CSV profile sequence `89` named B-spline start `1657065690.4515567`, while shutdown stopped rosbag after sequence `88` at `1657065689.439588`; the closest recorded odom was consequently `0.1289773 s` earlier, outside the unchanged `0.100 s` gate.
...
### Superseding P1 fixed-lambda formal terminal record — 2026-08-06 fixed-mean peak counterexample

Observation: Clean HEAD `4cc7dee` produced fresh serial P1-1 run `06c11f5fd9b8468f8d0ed79294aa8971` (export `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786051338125`, bag `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T212218Z`) and P1-2 run `ad8c310a8b5442ea8973fd7ba99d259d` (export `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786051453174`, bag `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T212413Z`). Both sole preflights passed. The pair's sole analyzer invocation, analysis `13a0d3fee7bd4cdc9d88f89648d9a752`, wrote all 39 figures nonempty and returned FAIL with `inconclusive=[]` and `warnings=[]`. Scene alignment, exact accepted-profile binding, fixed support, objective application, candidate identity/diversity, normalized descent, lifecycle, P0/P5 isolation, and provenance all passed. On the common terminal arc `0.33831498723911424 m`, fixed-mean P1 lowered mean from `0.41893558519181384` to `0.41841266508110825` but raised max from `0.42346212203978284` to `0.42499302651835125`; the first failed hard gate was `strict_c_pi_mean_max_reduction`, with the derivative cause-exclusion failure `risk_profile_not_reduced`.
...
![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 36 fixed-200 candidate rows were emitted across 9 attempts.

Run ID: `a81f16f8ad4a4f7f94e6ccd15f7cb3d6`; HEAD: `7acd21608c61fc11c12ab8662f6a6e689b2d2abe`; runtime hashes: `bspline_library=3e29e0d1c2a414830515587e2bdd6fe6519e7ab1d46ae30564504a9b1e895c21, launch=8a1a230b981af244b774563fae9e25d5f517283cc36fdc7947b2559a2bb077fb, planner_executable=fbe7196842d9dc1831c72504985efa80701afdeec755d39fefe01cdc4cf985a4`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T214257Z`; attempt/candidate/generation/query-base: `26/1/39/1657065620.9891725,26/2/39/1657065620.9891725,26/3/39/1657065620.9891725,26/4/39/1657065620.9891725,27/1/41/1657065621.9901569,27/2/41/1657065621.9901569,27/3/41/1657065621.9901569,27/4/41/1657065621.9901569,28/1/43/1657065622.9901628,28/2/43/1657065622.9901628,28/3/43/1657065622.9901628,28/4/43/1657065622.9901628,29/1/45/1657065623.9891405,29/2/45/1657065623.9891405,29/3/45/1657065623.9891405,29/4/45/1657065623.9891405,30/1/47/1657065624.9901702,30/2/47/1657065624.9901702,30/3/47/1657065624.9901702,30/4/47/1657065624.9901702,31/1/49/1657065625.9901834,31/2/49/1657065625.9901834,31/3/49/1657065625.9901834,31/4/49/1657065625.9901834,32/1/51/1657065626.9891584,32/2/51/1657065626.9891584,32/3/51/1657065626.9891584,32/4/51/1657065626.9891584,33/1/53/1657065627.9891627,33/2/53/1657065627.9891627,33/3/53/1657065627.9891627,33/4/53/1657065627.9891627,34/1/55/1657065628.9901502,34/2/55/1657065628.9901502,34/3/55/1657065628.9901502,34/4/55/1657065628.9901502`; snapshot/query-base window: `2026-08-06T21:42:57Z to 2026-08-06T21:43:28Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `a81f16f8ad4a4f7f94e6ccd15f7cb3d6`.

Verdict: PASS (diagnostic only).

Conclusion: This terminal record supersedes no other run and does not grant formal progression.

### Superseding P1 fixed-lambda diagnostic terminal record — 2026-08-06 LSE entry gate

Observation: Clean HEAD `7acd21608c61fc11c12ab8662f6a6e689b2d2abe` produced fresh enabled run `a81f16f8ad4a4f7f94e6ccd15f7cb3d6`, export `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052577914`, and bag `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T214257Z`. Its sole verifier returned `passed=true, errors=[]`; its sole diagnostic analyzer returned PASS and embedded all ten nonempty figures above. All 36 candidates across nine attempts used `fixed_200_lse`, were optimizer-successful, rank-eligible, full fixed `200/200`, and P1-descending. Every attempt had exactly one selected and accepted winner; every winner preserved mean/max non-regression with strict descent and negative raw-gradient/displacement alignment. All 36 final control-point hashes were distinct; recorded maximum softmax contribution ranged from `0.00731037` to `0.0111510`.

Verdict: PASS (diagnostic entry-to-formal).

Conclusion: The LSE objective, fixed support, frozen normalization, candidate diversity, winner/replacement closure, P0 attribution, and v4 provenance are green. This authorizes exactly one wholly fresh serial 90-second P1-1/P1-2 pair and one formal analyzer invocation for that pair. It does not authorize P1-3.

### Superseding P1 fixed-lambda formal terminal record — 2026-08-06 LSE peak counterexample

Observation: Clean HEAD `559e134e914c1623efdce59f37b9aa9d35f1e849` produced the fresh serial 90-second P1-1 run `2ae433176f7842638fa0cd0767e3d64e` (export `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052860343`, bag `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T214740Z`) and P1-2 run `39a598c00b1d4944ab7adc3a4875d0f3` (export `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786052978205`, bag `test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T214938Z`). Both sole v4 preflights passed with `errors=[]`. The prescribed formal analyzer was invoked exactly once as analysis `f77a2480fc304409a6637470a9d4b44d`, produced all 39 nonempty figures, and reported `warnings=[]` and `inconclusive=[]`. Provenance, fixed support, objective application, candidate identity/diversity/descent, lifecycle/singleflight, P0 health/attribution, P5 isolation, timebase, topic, validator, and scene/profile binding gates all passed. On the aligned `0.2680614616 m` terminal arc, P1-2 strictly reduced mean `0.4204437873 -> 0.4187024725` but increased max `0.4244592145 -> 0.4245587735`; only `risk_profile_reduced` and its dependent cause-exclusion gate failed.

Verdict: FAIL (formal fixed-200 peak effectiveness).

Conclusion: This pair is retained and will never be reanalyzed. The normalized fixed-200 LSE objective does not satisfy the same formal mean/max gate in this fresh counterexample. Following H4, production now advances to the third and final prescribed smooth alternative: deterministic smooth CVaR with tail `alpha=0.90`, temperature `T=0.01 c_pi`, a stable softplus threshold solve, and analytic envelope-theorem gradient. Fixed lambda/support, P0/P5 semantics, and every hard gate remain unchanged. A new implementation commit, build, diagnostic smoke, and wholly fresh formal pair are required; P1-3 remains prohibited.

## 2026-08-06 diagnostic-smoke figure evidence (non-authoritative)

### Full scenario top-down

![Full scenario top-down](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500/p1_candidate_diagnostic_smoke/p1_diag_topdown_scene.png)

Observation: 56 fixed-200 candidate rows were emitted across 14 attempts.

Run ID: `805116b6bce9441886e15d216a0d4332`; HEAD: `0afb316f8660e9ca4d28590522ef72ed049c391b`; runtime hashes: `bspline_library=b4fb6f074cf4475fdbbfab927e40a5e191eccae6c516902b02d0182e48eb030d, launch=d40b97825d69bda208a7fef81d060c8fc7fce27d0e152bfead9e70eab4ad3573, planner_executable=05407f80f7d922b7b6412794e3eb7a3860c7f5aadcc48cdb3e39809e501f80b2`; export: `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500`; bag: `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T220823Z`; attempt/candidate/generation/query-base: `18/1/29/1657065616.4911127,18/2/29/1657065616.4911127,18/3/29/1657065616.4911127,18/4/29/1657065616.4911127,19/1/31/1657065617.4910848,19/2/31/1657065617.4910848,19/3/31/1657065617.4910848,19/4/31/1657065617.4910848,20/1/33/1657065618.4921012,20/2/33/1657065618.4921012,20/3/33/1657065618.4921012,20/4/33/1657065618.4921012,21/1/35/1657065619.4921196,21/2/35/1657065619.4921196,21/3/35/1657065619.4921196,21/4/35/1657065619.4921196,22/1/37/1657065620.4921122,22/2/37/1657065620.4921122,22/3/37/1657065620.4921122,22/4/37/1657065620.4921122,23/1/39/1657065621.4920917,23/2/39/1657065621.4920917,23/3/39/1657065621.4920917,23/4/39/1657065621.4920917,24/1/41/1657065622.4920959,24/2/41/1657065622.4920959,24/3/41/1657065622.4920959,24/4/41/1657065622.4920959,25/1/43/1657065623.4920754,25/2/43/1657065623.4920754,25/3/43/1657065623.4920754,25/4/43/1657065623.4920754,26/1/45/1657065624.492094,26/2/45/1657065624.492094,26/3/45/1657065624.492094,26/4/45/1657065624.492094,27/1/47/1657065625.4910817,27/2/47/1657065625.4910817,27/3/47/1657065625.4910817,27/4/47/1657065625.4910817,28/1/49/1657065626.4918232,28/2/49/1657065626.4918232,28/3/49/1657065626.4918232,28/4/49/1657065626.4918232,29/1/51/1657065627.4918313,29/2/51/1657065627.4918313,29/3/51/1657065627.4918313,29/4/51/1657065627.4918313,30/1/53/1657065628.4918153,30/2/53/1657065628.4918153,30/3/53/1657065628.4918153,30/4/53/1657065628.4918153,31/1/55/1657065629.4918265,31/2/55/1657065629.4918265,31/3/55/1657065629.4918265,31/4/55/1657065629.4918265`; snapshot/query-base window: `2026-08-06T22:08:23Z to 2026-08-06T22:08:54Z`; diagnostic (non-authoritative).
...
![Fan-out funnel](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500/p1_candidate_diagnostic_smoke/p1_diag_fanout_funnel.png)
...
![Mean/max delta scatter](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500/p1_candidate_diagnostic_smoke/p1_diag_mean_max_delta_scatter.png)
...
![Initial/final pairwise matrices](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500/p1_candidate_diagnostic_smoke/p1_diag_pairwise_matrices.png)
...
![Objective decomposition](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500/p1_candidate_diagnostic_smoke/p1_diag_objective_decomposition.png)
...
![Gradient/displacement](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500/p1_candidate_diagnostic_smoke/p1_diag_gradient_displacement.png)
...
![Per-attempt candidate profile](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500/p1_candidate_diagnostic_smoke/p1_diag_profile_comparison.png)
...
![P0 occupied/base-collision overlay](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500/p1_candidate_diagnostic_smoke/p1_diag_p0_occupancy_overlay.png)
...
![Lifecycle swimlane](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500/p1_candidate_diagnostic_smoke/p1_diag_lifecycle_swimlane.png)
...
![Artifact/provenance timeline](../../results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500/p1_candidate_diagnostic_smoke/p1_diag_artifact_provenance_timeline.png)
...
Observation: Analyzer completed once for run `805116b6bce9441886e15d216a0d4332`.

Verdict: PASS (diagnostic only).

Conclusion: This terminal record supersedes no other run and does not grant formal progression.

### Superseding P1 fixed-lambda diagnostic terminal record — 2026-08-06 smooth-CVaR entry gate

Observation: Clean HEAD `0afb316f8660e9ca4d28590522ef72ed049c391b` produced fresh enabled run `805116b6bce9441886e15d216a0d4332`, export `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786054103500`, and bag `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T220823Z`. Its sole verifier returned `passed=true, errors=[]`; its sole diagnostic analyzer returned PASS and embedded all ten nonempty figures. All 56 candidates across 14 attempts used `fixed_200_smooth_cvar`, `T=0.01`, `alpha=0.90`, and full fixed `200/200`; all were optimizer-successful, rank-eligible, and P1-descending. Every attempt had exactly one selected and published winner; every winner preserved mean/max non-regression with strict descent and negative raw-gradient/displacement alignment. All 56 final control-point hashes were distinct, and maximum smooth-tail contribution ranged from `0.00766968` to `0.0160273`.
...
### Superseding P1 fixed-lambda formal terminal record — 2026-08-06 smooth-objective incompatibility

Observation: Clean HEAD `d1a7d2ac918b9288805ea5bf9c15cd4d5082cfb7` produced the fresh serial 90-second P1-1 run `d7f1a11470ab454398968eebc6288b60` (export `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786055324163`, bag `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T222844Z`) and P1-2 run `a0a1d5ad774a4b6f91c27709abc6f5d4` (export `/home/dev/ws_iap/src/iap/results/planner_validation/exports/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_1786055470889`, bag `/home/dev/ws_iap/src/iap/results/planner_validation/bags/test_planner_p1_degraded_lidar_good_gnss_degraded_lidar_good_20260806T223110Z`). Recorder exit, metadata, validator, manifest, schema v4, and each bundle's sole preflight all passed with `errors=[]`. The pair's sole analyzer invocation, analysis `fb41d1aaf6934fdd8d6441fb39d35f2e`, generated 39/39 nonempty figures and reported `warnings=[]`, `inconclusive=[]`, `status=FAIL`. Fixed lambda/support, objective application, candidate identity/diversity/descent, lifecycle/singleflight, P1-2 P0 health, P5 isolation, timebase, required topics, profile coverage, scene binding, and trajectory stability passed. The retained pair additionally failed baseline post-startup P0 freshness and workspace/install runtime-path provenance because its recovered reproducible binaries lived under `/tmp`; neither failure is waived. On the aligned `0.2701304165750375 m` terminal arc, smooth CVaR strictly reduced mean `0.419656365836797 -> 0.41849948756628097` but increased max `0.4236384817259452 -> 0.42426530186016653`.

Observation: The same fixed-200 acceptance mismatch is mathematical, not a remaining temperature/tail tuning question. `P1IntegrityCostTest.ProductionFixed200ModesHaveNoGateFeasibleRiskDirection` derives the feasible family from planner objects rather than declaring an abstract profile vector. It constructs one cubic B-spline with four control points; the optimizer freezes the first three, so only the last point is active. One captured, full-support snapshot contains the affine field `c(p,t)=50+(0.02-t)p_y`, which spatial/temporal interpolation preserves exactly. Active X/Z changes are therefore risk-invariant, while active Y displacement `x` produces each fixed-lattice sample `c_i(x)=50+(0.02-t_i)B_i x`, where `B_i` is the last control point's spline basis weight. The production fixed-200 evaluation proves both `sum_i (0.02-t_i)B_i<0` (positive `x` lowers mean) and at least one positive coefficient (positive `x` raises exact max). Thus `x>0` violates max, `x<0` raises mean, `x=0` has no strict improvement, and X/Z cannot change either statistic. On this same `q/snapshot/lattice`, production fixed mean, normalized LSE (`T=0.01`), and smooth CVaR (`alpha=0.90`, `T=0.01`) all have negative active-Y gradient and strictly descend at the positive-Y probe that necessarily regresses max. Base feasibility can only remove directions from this risk-reachable set; it cannot create another risk direction.
...
### c18 retained evidence-hard-gate result

Clean c18 (`01eeb70`) completed all ten serial 90-second runs with 10/10 validator PASS and stopped at independent prequalification. Both primary pairs had incomplete metrics because every enabled scene lacked a unique accepted-profile checkpoint; reference runs also exposed one incomplete accepted context and null/soft localization errors of `0.645 m`/`1.010 m`. Consequently c18 is not a third comparable primary-effect failure and does not satisfy the terminal stop rule. No calibration, formal preflight/run/analyzer, or P1-3 execution occurred. Raw c18 evidence remains under `results/planner_validation/campaigns/p1-2-20260809-01eeb70-c18/`, with large occupancy CSVs losslessly gzipped.

The next clean campaign uses a formal-only, read-only observation of the actual executing incumbent inside `x=-9.5+/-0.4 m`, with the same immutable snapshot, real remaining trajectory, fixed 200 points, and occupancy corners. Metrics-only candidate evidence is now mandatory and export-bound. Geometry v2 adds exact-symmetric, lane-external survey pylons for LiDAR observability without changing tree/canopy parameters or route clearance.

Clean c19 (`24752f5`) completed all ten runs with 10/10 validator PASS, but the independent analyzer exited 1 before pair evaluation on a stale local variable left by the strict candidate-artifact refactor. c19 remains immutable operational-failure evidence and is not reanalyzed after the two-line repair. It contributes no scientific result; calibration/formal/analyzer/P1-3 remain unstarted.

### c20 retained evidence-hard-gate result

Clean c20 (`65fdaf3`) completed all ten serial 90-second runs with 10/10 validator PASS and stopped at independent prequalification (exit 2). It is not a third comparable primary-effect failure: both primary pairs had incomplete metrics. The selected checkpoint could bind an incomplete observation (`184..199/200`) or no accepted context, admitted candidate evidence at that identity often contained only one route, and enabled primary/null localization errors reached `56.994 m`/`51.602 m`. Mirror and soft-risk showed positive mean/CVaR changes, but their hard gates also failed; only null-reference and soft-reference passed per-run hard gates. No calibration, formal preflight/run/analyzer, or P1-3 execution occurred. Raw c20 evidence remains under `results/planner_validation/campaigns/p1-2-20260809-65fdaf3-c20/`; large occupancy/provenance CSVs are losslessly gzipped.
