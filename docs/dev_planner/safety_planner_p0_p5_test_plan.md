...
| ID | 实验目的 | Launch 命令 | 参数配置 | Topic 检查 | 保存产物 | 预期现象 | 失败判据 | 通过判据 | 下一步分支 |
|---|---|---|---|---|---|---|---|---|---|
| P1-1 | metrics-only 不改轨迹 | `ros2 launch iap test_planner.launch.py experiment:=p1_degraded_lidar_good p1.metrics_only:=true run_duration_s:=90 validation_duration_s:=90 start_rviz:=false run_validator:=true record_bag:=true` | P0+P1 metrics only | `T_BASE`, `T_P0_HEALTH`, `T_P1` | P1 CSV, trajectory overlay | `applied_to_objective=false`；trajectory 与 baseline 接近 | objective 被应用；轨迹明显偏离 baseline | metrics 产生且不改变行为 | PASS -> P1-2；FAIL -> debug metrics-only gate。 |
| P1-2 | enabled 后降低 mean/max risk | `ros2 launch iap test_planner.launch.py experiment:=p1_degraded_lidar_good p1.metrics_only:=false p1.lambda_integrity:=0.00001 run_duration_s:=90 validation_duration_s:=90 start_rviz:=false run_validator:=true record_bag:=true` | P1 enabled | `T_P0_HEALTH`, `T_P1` | P1 cost CSV, risk profile | mean/max `c_pi` 低于 baseline/P1 metrics-only | risk 不降；collision/feasibility 恶化 | risk 降且 validator pass | PASS -> P1-3；FAIL -> lambda/gradient debug。 |
| P1-3 | lambda sweep | `for l in 0.000001 0.000005 0.00001 0.00002; do ros2 launch iap test_planner.launch.py experiment:=p1_degraded_lidar_good p1.metrics_only:=false p1.lambda_integrity:=$l run_duration_s:=60 validation_duration_s:=60 start_rviz:=false run_validator:=true record_bag:=true; done` | 多 lambda | `T_P1` 每个 run | sweep summary | `grad_ratio` 大多在 0.05-0.20，risk/path tradeoff 可解释 | 全部过弱或过强；轨迹振荡 | 选出推荐 lambda | PASS -> P1-4；FAIL -> cost scaling debug。 |
