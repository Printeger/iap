...
目标：在固定候选与固定 P0 snapshot 上证明 P1 是温和的低权重 preference，
不替代碰撞/动力学门和 P5 hard gate。历史 Phase 3 v1
`run_p1_2_campaign.py` 已停止使用；c31/c32/c38 的旧 P1-2 `BLOCKED` 结论和
formal analyzer 调用次数 0 保持不变，v2 只能用于 future fresh campaign。

| ID | 实验目的 | Launch 命令 | 参数配置 | Topic 检查 | 保存产物 | 预期现象 | 失败判据 | 通过判据 | 下一步分支 |
|---|---|---|---|---|---|---|---|---|---|
| P1-1 | Off/metrics-only 无干扰 | future v2 harness：同一初值与 snapshot 的 P1 off/metrics-only pair | `applied_to_objective=false` | `T_BASE`, `T_P0_HEALTH`, `T_P1` | 控制点、候选身份、base objective、support/provenance | 两臂最终控制点、候选身份、base objective 相同，`200/200` | 任一 identity/objective/support 不同 | 无干扰合同全部成立 | PASS -> P1-2；FAIL -> v2 harness/isolation debug。 |
| P1-2 | 同快照因果机制 | 单一同伦走廊；10 个预定义 B-spline 初值；affine、精确镜像、flat-null、平滑 risk-island 冻结字段 | reference/enabled 同 initial hash、snapshot、时间基准、support signature | `T_P1`, `T_P0_HEALTH` | before/after objective、PL/IM、gradient/displacement | 非 null raw objective/mean PL 下降、min IM 上升、`gradient·displacement<0`；mirror 方向相反；null 不动 | identity/support 不同；方向/风险/安全/动力学门失败 | max PL 在 null 非劣界内且全部因果门通过；不用旧绝对降幅 | PASS -> P1-3；FAIL -> 固定字段/机制 debug。 |
| P1-3 | 权重与裁剪 sweep | future v2 harness；禁止使用历史 v1 campaign | raw lambda `1e-6,3e-6,1e-5,3e-5,1e-4`；normalized reference `1e-5`；budget `0.01,0.03,0.05,0.10,0.20,0.30`；clip `0.05,0.1,0.2` | `T_P1` | scaling/safety/feasibility/gradient-ratio summary | raw scaling 单调；至少 9/10 同向改善 | 安全/可行性失败；中位 ratio 或 P95 越界 | ratio 中位 5%–20%，P95 <=20%；按最小有效 budget、再最小 clip 冻结 | PASS -> P1-4；FAIL -> normalization/clip debug。 |
| P1-4 | Fresh 闭环确认 | future v2 harness；先 10 个 flat-null pair，再 10 个未参与 sweep 的 counterbalanced seed | 每次 fresh；唯一有效决策后结束；最长 60 s | `T_BASE`, `T_P1`, `T_TRAJ` | null tolerance、paired result、latency | mean PL/min IM 至少 9/10 改善，单侧 exact sign test `p<0.05` | 中位改善未越 null；路径/安全/动力学/latency 门失败 | 路径增长 <=5%；P95 latency <= baseline 110% 且在周期预算内 | PASS -> P1-5；FAIL -> fresh paired debug。 |
| P1-5 | Unknown/stale 策略 | future v2 harness：partial、全 unknown、stale、NaN | stale timeout `0.5/1.0/2.0 s`；冻结 low/medium/high unknown penalty | `T_P0_HEALTH`, `T_P1` | fallback、penalty、NaN/crash evidence | unknown 不被当低风险，证据不足明确 fallback | NaN/crash；向 unknown 吸引；fallback 缺失 | 全矩阵 fail-safe 且可解释 | PASS -> P1-6；FAIL -> unknown/stale policy debug。 |
| P1-6 | P5 权威隔离 | query-aligned future-risk 注入，P1 off/on + P5 on | P1 preference；P5 runtime/final on | `T_P1`, `T_P5_STATUS` | P1/P5 causal timeline | unsafe trajectory 始终由 P5 replan/reject | P1 产生 hard PASS 或绕过 final gate | P5 action/final gate 保持唯一权威 | PASS -> Phase 4；FAIL -> authority isolation debug。 |
...
| P2-2 | 同 attempt/snapshot 的 fork ranking | future fresh P2 campaign；fork primary/mirror/null/soft 冻结场景 | P2 enabled；同一次 planning attempt、同一候选集合、同一 P0 snapshot | `T_P2`, `T_P0_RVIZ` | candidate identity、snapshot、score decomposition | primary/mirror 按冻结风险方向重排；null no-op；soft 选低风险候选 | identity/snapshot 不同；selected 高风险且无 fallback | 四类场景的 score/winner 可解释 | PASS -> P2-3；FAIL -> ranking/scaling debug。旧 c38 的 P2 为 off，只能迁移场景，不能计证据。 |
