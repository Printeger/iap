...
The formal metrics-only side is consequently an observed remaining accepted trajectory rather than an unsupported publication-time extrapolation. The context row and exact cloud identify the new snapshot/generation/query base, while trajectory ID and effective segment start stamp identify the unchanged incumbent. Missing, stale, out-of-horizon, or failed observations remain fail closed.
