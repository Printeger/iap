...
Candidate and incumbent can have different remaining durations. Replacement therefore resamples both trajectories on a fixed 200-point lattice over the shared forward-time window `min(candidate duration, incumbent remaining duration)`, beginning at the same planning epoch and risk query `tau=0`. Candidate self-descent and optimizer ranking still use the candidate's complete fixed-200 profile; only the cross-trajectory replacement comparison uses the common window. STEP3 repeats both the full self-comparison and the common-window incumbent comparison after timing refinement. This avoids penalizing a longer candidate for a future tail that the shorter incumbent does not represent without hiding that tail from candidate evidence.

The formal P1-1/P1-2 comparison applies the same future-only principle across runs. Receding-horizon scheduling can leave the final accepted profiles with different remaining arc lengths, so the strict mean/max gate uses the terminal arc length common to both recorded profiles. It does not resample or rewrite either authoritative 200-sample CSV; aligned and raw full-profile statistics are both retained in the report. This prevents earlier history present only in the longer reference from changing the mean while preserving the terminal maximum and all raw evidence.

Every accepted publication force-emits its immutable planning snapshot on the existing predicted-risk cloud topic with the snapshot's own timestamp. This bypasses only visualization rate limiting and gives the formal bag an exact generation/stamp/frame scene binding; it does not refresh, reacquire, or alter any P0 value or occupancy decision.

Metrics-only reference trajectories may be accepted before their terminal arc enters the finite P0 prediction horizon. Re-querying that arc against the publication snapshot would require extrapolation, while choosing an earlier supported arc would compare a different spatial region. Therefore each metrics-only accepted trajectory receives at most one read-only observation when its genuine remaining duration first fits a later immutable snapshot horizon. The observer samples the unchanged B-spline from its elapsed source offset on a fixed 200-point lattice, restarts risk-query `tau` at the captured query base, retains the original B-spline publish/start identity in `trajectory_start_stamp_s`, records the later observation/snapshot epoch in `accepted_stamp_s`, and force-emits that exact snapshot cloud. It never republishes or mutates the trajectory. Enabled P1 retains publication-bound candidate evidence and never uses this reference-only observation path.
...
`incumbent_available` records whether an already-published trajectory exists and can be retained. It is set before the shared-window risk query and remains true when that query fails full support. Comparison mode, duration, and statistics remain separately conditional on a valid fixed-200 comparison; incomplete comparison still rejects replacement closed. This prevents an occupied corner from erasing retained-trajectory identity without treating the corner as valid risk.

The formal metrics-only side is consequently an observed remaining accepted trajectory rather than an unsupported publication-time extrapolation. The context row, `accepted_stamp_s`, and exact cloud identify the new snapshot/generation/query base and scene epoch, while trajectory ID and `trajectory_start_stamp_s` identify the unchanged original publication. Missing, stale, out-of-horizon, or failed observations remain fail closed.
