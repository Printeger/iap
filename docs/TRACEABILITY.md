...
| IAP-RQ-423 | Reuse immutable product bytes through a repository-local no-compile overlay | fixed-hash sanitized driver, closed install environment, complete retained-task SHA inventory, overlay byte equality, actual ament/CMake resolution, duplicate-prefix rejection and evidence/analyzer bindings | **IMPLEMENTED; OVERLAY NOT YET INVOKED** |
