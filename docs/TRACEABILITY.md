...
| IAP-RQ-015 | Expose Σ_p 位置协方差块（3×3）到 EstimationFrame；供 PL proxy 使用 | 保护级别需基于不确定性 | `odometry_estimation_imu.*` (marginalCovariance) | replay 一致；`lambda_max(Σ_p)` 打印 | `sigma_p_trace, lambda_max_sigma_p` | TODO |
| IAP-RQ-020 | GNSS 紧耦合观测：伪距 + 多普勒建因子（含 clock bias/drift，per-sat） | GNSS tightly-coupled | `src/iap/gnss/` | 关闭/开启 GNSS 对比 | `res_pr, res_dop, clk, clk_dot` | TODO |
