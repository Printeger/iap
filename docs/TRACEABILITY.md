...
| IAP-RQ-220 / IAP-RQ-320 | Prove fresh valid GNSS plus positive GNSS/LiDAR/fused predictor use and satellites | fail-closed analyzer gates in `launch/icra_p0_p5_qualification.py`; raw normalizer gates and hash-consistent adversaries in the runner and focused tests | **IMPLEMENTED; 41/41 FOCUSED PASS** |
...
| IAP-RQ-423 | Reuse immutable product bytes through a repository-local no-compile overlay | dependency resolver, sanitized install driver, complete overlay inventory, binary/library equality, package/prefix resolution and analyzer binding in the ICRA-070 runner; focused overlay adversaries | **IMPLEMENTED; OVERLAY NOT YET INVOKED** |
