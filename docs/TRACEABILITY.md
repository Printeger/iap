...
| Req ID | Requirement/evidence seam | Implementation and retained evidence | Status |
|---|---|---|---|
| IAP-RQ-421, IAP-RQ-422, IAP-RQ-423 | Render parseable replacement commands without changing effective profile | `run_icra_p0_p5_qualification.py` omits the exact registered 19 empty defaults, preserves false/zero/nonempty values, rejects duplicate/malformed/unregistered omissions and freezes three `-002` IDs; focused tests render all cases | **29/29 RUNNER/ANALYZER, 20/20 LAUNCH AND 549/549 FULL TESTS PASS** |
| IAP-RQ-320, IAP-RQ-423 | Adopt unchanged product runtime with distinct runner provenance | dual-provenance adoption binds ICRA-068 commit `005ce1a`, manifest `7662a2c4...34d420`, current runner/analyzer hashes and zero installed-runtime source overlap; analyzer reconciles only the proven commit split | **IMPLEMENTED; LIVE ADOPTION PROOF PENDING COMMITTED HEAD** |
| IAP-RQ-423 | Prove real ROS parser acceptance before GPU or live registration | exact rendered argv feeds three installed `--show-args` commands with stdout/stderr/exit/process-group evidence under `results/icra27/icra069` | **IMPLEMENTED; REAL PARSER INVOCATIONS 0 PENDING COMMITTED HEAD** |
| IAP-RQ-423 | Fail closed before the sole replacement analyzer invocation | `require_replacement_evidence_ready()` verifies exact COMPLETE runner state plus canonical path/hash-bound ordered parser proof; malformed-value, omission, tamper and pre-claim tests in `test/test_run_icra_p0_p5_qualification.py` | **IMPLEMENTED; 33/33 FOCUSED AND 553/553 HERMETIC PASS** |
| IAP-RQ-421 / IAP-RQ-423 | Execute replacement qualification once and stop at the first technical failure | `results/icra27/icra069/parse_only/parser_proof.json` proves parser `0/0/0`; `preflight/gpu_preflight.json` proves GPU PASS; SAFE_NORMAL process evidence records 15/16 required identities with `test_planner_gnss_sim_node` never started; runner state freezes `1/0/1/0`, no orphan/later arm/analyzer | **BLOCKED_ICRA069_SAFE_NORMAL_REQUIRED_PROCESS_NEVER_STARTED** |
