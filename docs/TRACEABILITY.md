...
| IAP-RQ-421 / IAP-RQ-422 / IAP-RQ-423 | P5 compatibility | Repository-local diagnosis binds 2,137 final observations, frozen HAL/VAL `10/20 m`, GNSS-selected `max_pl` HPL `24.3673612..27.733391 m`, VPL `68.8205779..86.6898998 m`, map frame, monitor stamp, metre units and launch/config authorities; SHA-256 `f19395a4…460be5` | **FROZEN_CONTRACT_INCOMPATIBLE** |
...
| IAP-RQ-320 / IAP-RQ-321 / IAP-RQ-423 / IAP-RQ-424 | Offline/claim boundary | Relevant regression 51/51; shared build used only for test targets; pushed source `07ca00a6d435b874e0f6b9529975974fd0f51d70` is bound by `results/icra27/icra074/offline-targeted-001.json` (SHA-256 `79989ac8c128977e37d91f0f3cd30ec3a0618f3818b44d34da53981893fefc67`); no production/config change, six-package build, ROS, GPU, live, effect, qualification or campaign | **READY FOR SUPERVISOR REVIEW** |
...
|---|---|---|---|
| IAP-RQ-423 / IAP-RQ-424 | Hermetic Git admission | `build_suite_environment` removes inherited count/key/value, file selectors and `GIT_CONFIG_PARAMETERS`, supplies exactly one command-local `safe.directory` entry for `/home/dev/ws_iap/src/iap`, isolates HOME/XDG and disables system config; source admission and every suite share and record that provenance. Focused real Git/source-binding commands prove accepted trust exposes exactly one command-line origin while absent/wrong trust rejects; repository/worktree config hashes stay fixed and no isolated config/root is created | **RED→GREEN / 5/5 RUNNER PASS** |
| IAP-RQ-320 / IAP-RQ-321 / IAP-RQ-423 / IAP-RQ-424 | Skip/disabled fail-closed parsing | `_observations` distinguishes successful, skipped and disabled Python/gtest assertions and total counts; ambient `GTEST_ALSO_RUN_DISABLED_TESTS` is removed and even a forced `[ OK ] DISABLED_*` record is typed disabled. `_suite_failures` makes every dependent matrix row fail even if name/count otherwise matches | **RED→GREEN / FOCUSED PASS** |
| IAP-RQ-400 / IAP-RQ-410 / IAP-RQ-421 / IAP-RQ-422 | Preserved production matrix | No product C++, production test or shared build root changed. Existing tools suite passes 17/17 under repository-local HOME plus exact command-local trust; retained shared 6/6 and C++ 8+2+2+4 results remain authoritative | **UNCHANGED / OFFLINE TOOLS PASS** |
| IAP-RQ-423 / IAP-RQ-424 | Fresh canonical evidence | Pushed repair `7a5aa58` and divergence `0 0` preceded the sole `repair-001` command. Exact source status exposed untracked `.claude/settings.local.json` (72 bytes, SHA-256 `27aac0cc…8f64`) hidden by ambient global ignore; source binding exited 2 before suite/output creation. File, immutable `final_*` and PDF remain untouched; zero retry | **BLOCKED_ICRA072B_UNTRACKED_SOURCE_NOT_ALLOWLISTED** |
...
| IAP-RQ-320 / IAP-RQ-321 / IAP-RQ-423 / IAP-RQ-424 | Operational closure and canonical gate | Existing 17-test runner/analyzer suite explicitly drives GPU/source/required-process/cleanup failures. New `run_icra072b_stabilization.py` validates exact pushed source/PDF, eight row/five suite cardinalities and explicit counts, matches each assertion only inside its declared suite, retains logs and emits no qualification/effect claim | **IMPLEMENTED; PRE-CANONICAL PASS** |
| IAP-RQ-423 / IAP-RQ-424 | Sole canonical retained result | Pushed source `8b75d81`; four C++ suites pass 8/8 + 2/2 + 2/2 + 4/4. Tools observe 17 tests but exit 1 because isolated repository-local HOME lacks ambient safe-directory trust, causing eight `git rev-parse HEAD` calls to return 128. `final_summary.json` SHA-256 `2669167a…624`, five logs retained, zero retry | **BLOCKED_ICRA072B_HERMETIC_GIT_SAFE_DIRECTORY** |
...
| IAP-RQ-320 / IAP-RQ-321 / IAP-RQ-400 / IAP-RQ-410 / IAP-RQ-421 / IAP-RQ-422 / IAP-RQ-423 / IAP-RQ-424 | First repaired-source continuation | Implementation `b7b5357` was pushed at divergence `0 0` before `run-024`. Runner/analyzer PASS; all seven stages true; P0 54, decisions 37, both-complete 20, selected 11, lineage 3, final/publish 16, runtime-bound 65; accepted selected ID `12`, start `1657065616411275703`, final identity `36cb40d791d9b347`; four matching fused runtime rows contain 44/44 exact samples, effective/raw OK, empty reasons and positive minima. Required processes 15/15 and cleanup pass | **ICRA-072A READY FOR SUPERVISOR REVIEW** |
...
| Req ID | Implemented/tested seam | Evidence and disposition | Status |
|---|---|---|---|
| IAP-RQ-421 / IAP-RQ-422 / IAP-RQ-423 | Runtime safety acceptance | Analyzer admits runtime only when every exact-identity record has effective/raw `action=OK`, authoritative `FUSED`, empty active/current/future rejection reasons and no final rejection. Matching-ID replan, production emergency, latent raw replan and later emergency fixtures all fail. Fresh `run-023` has four exact-identity runtime records, all effective/raw OK with empty reasons | **SOURCE-BOUND LIVE PASS** |
| IAP-RQ-400 / IAP-RQ-410 / IAP-RQ-423 | Explicit exact identity and selected terminal | Lineage must be v2 with strict positive integer ID/start, nonempty control-point/final identity and one consistent identity across the selected three-stage chain. Missing, malformed, float, sentinel, empty and mismatched fixtures fail; a later unselected trajectory does not replace `accepted_terminal_identity`. `run-023` accepts selected ID `6`, start `1657065614522279439`, final identity `4388eac04c2cc922` | **SOURCE-BOUND LIVE PASS** |
| IAP-RQ-423 / IAP-RQ-424 | Consumed-attempt closure and source binding | Runner finalizes gate-import/GPU-preflight and later exceptions, records analyzer/outcome, requires clean tracked source with exact pushed HEAD, and rechecks after GPU/before ROS and after cleanup. Source rejection, gate import and preflight exception fixtures start no ROS. `run-023` binds all three checks and analyzer `exercised_commit` to pushed `c59de16e864701f6da5291f51aebfbc48388d92a` | **SOURCE BINDING PASS** |
...
| IAP-RQ-320 / IAP-RQ-321 / IAP-RQ-400 / IAP-RQ-410 / IAP-RQ-421 / IAP-RQ-422 / IAP-RQ-423 / IAP-RQ-424 | First acceptable continuation | `run-023` is runner/analyzer PASS: GPU, required processes and cleanup pass; all seven stages are true; counts are P0 56, decisions 78, both-complete 49, selected 35, lineage 3, final/publish 14, runtime-bound 57. Accepted terminal fused final margins are H `4.6995420912 m`, V `5.60549867666 m`; four exact runtime records remain equally positive and fully OK | **ICRA-072A READY FOR SUPERVISOR REVIEW; RUN-024 NOT CREATED** |
...
|---|---|---|---|
| IAP-RQ-320 / IAP-RQ-321 / IAP-RQ-423 / IAP-RQ-424 | Archive the completed Builder checkpoint without manufacturing a PASS | HEAD `6a6bdd3`; 6/6 build, 199/199 C++, 29/29 Python, GPU/15-process live admission, P0 ready 124, natural P4 risk selections 76 and both-guide-complete 339; sole analyzer has zero terminal lineage/P5/publication/runtime | **ARCHIVED_AS_FOUND / BLOCKED_TERMINAL_CHAIN_MISSING** |
| IAP-RQ-400 / IAP-RQ-410 / IAP-RQ-421 / IAP-RQ-422 / IAP-RQ-423 | Layer 1 iterative full-flow target | `NEXT_TASK.md` ICRA-072A; shared build, unique repeatable runs and ordered first-missing-stage analysis until one real identity reaches final B-spline, P5-final-before-publish, normal publish and P5 runtime | **TASK_READY / DEVELOPMENT ONLY** |
| IAP-RQ-423 / IAP-RQ-424 | Four-layer sequencing | `docs/icra27/ICRA_FOUR_LAYER_DEVELOPMENT_WORKFLOW.md`; ICRA-072A integration -> ICRA-072B stabilization -> ICRA-073..075 diagnostics -> ICRA-076..079 formal validation | **USER-DIRECTED / GATE SEQUENCE UNCHANGED** |
| IAP-RQ-000 / IAP-RQ-424 | Regenerable-artifact retirement boundary | Explicit USER authority plus `AGENTS.md` narrow exception; inventory commit `3f06a45` was pushed first; 61 literal repository-local roots totaling `122694791115` logical bytes revalidated and were deleted, `0/61` remain, and filesystem availability increased by `123042209792` bytes; raw/compact/live/scientific/log/shared-workspace/PDF checks pass | **COMPLETED / RETENTION BOUNDARY PASS** |
...
This table preserves the verdict at that Review. Its `TASK_READY`/inactive-next-gate cells were superseded by
the user-owned route recovery above and do not define the current task or route.
...
| Req ID | Requirement/evidence seam | Implementation and retained evidence | Status |
|---|---|---|---|
| IAP-RQ-320 | Preserve reviewed P0 and frozen P5 decisions | `config/icra27/icra_p0_p5_qualification_v1.json`, `launch/icra_p0_p5_qualification.py`; verified by `test/test_icra_p0_p5_qualification.py` and `results/icra27/icra067/compact/validation_manifest.json` | **PROFILE CONTRACT PASS; NO LIVE CLAIM** |
...
| Req ID | Requirement/evidence seam | Implementation and retained evidence | Status |
|---|---|---|---|
| IAP-RQ-320 | Bind every arm to reviewed P0 identity | Implementation: `config/icra27/icra_p0_p5_qualification_v1.json`, `launch/icra_p0_p5_qualification.py`; tests: `test/test_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra068/compact/final_result.json` binds worker 4, sigma `0.01`, `legacy_iap_rq320_baseline_v1` and exact profile | **IMPLEMENTED; NON-LIVE TESTS PASS** |
| IAP-RQ-421, IAP-RQ-422 | Run only the P0+P5 contingency route | Implementation: `scripts/dev_planner/run_icra_p0_p5_qualification.py` and canonical contract preserve P5 final/runtime plus P0 while disabling P1/P2/P3/P4 and auxiliary paths; tests: `test/test_run_icra_p0_p5_qualification.py`, `test/test_test_planner_launch.py`; evidence: `results/icra27/icra068/compact/final_result.json` | **PROFILE PRESERVED** |
| IAP-RQ-423 | Prove isolated current runtime closure | Implementation/checker: `scripts/dev_planner/run_icra_p0_p5_qualification.py`; tests: `test/test_run_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra068/icra068_install_manifest.json` SHA `7662a2c4...34d420` binds commit `005ce1a`, 17/17 builds, 18 package identities, 54 libraries, 83 hashes and three aliases | **BUILD / FINAL MANIFEST PASS** |
| IAP-RQ-423 | Bind full live ownership and real evidence | Implementation: `scripts/dev_planner/run_icra_p0_p5_qualification.py`, `launch/icra_p0_p5_qualification.py`; tests: `test/test_run_icra_p0_p5_qualification.py`, `test/test_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra068/live/runner_state.json` and each registered run directory bind 16 children, three topics, source files, shutdown, IDs and retry count | **IMPLEMENTED; 23/23 FOCUSED TESTS PASS** |
| IAP-RQ-423 | Reject synthetic qualification claims | Implementation: `launch/icra_p0_p5_qualification.py::analyze_live_bundle`; tests: `test/test_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra068/compact/final_result.json` records 543/543 full discovery and zero analyzer invocation after the live blocker | **FAIL-CLOSED OFFLINE PASS** |
| IAP-RQ-423 | Fail closed on early launch exit, orphan and ambiguous lifecycle evidence | Implementation: task process-group monitor/normalizer in `scripts/dev_planner/run_icra_p0_p5_qualification.py`; tests: `test/test_run_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra068/live/icra-p0-p5-live-safe-normal-001/process_result.json` plus raw bag payload hashes and `results/icra27/icra068/live/runner_state.json` | **PRE-LIVE REVIEW BLOCKERS REMEDIATED** |
| IAP-RQ-423 | Preserve single-call and isolated-environment boundaries | Implementation: marker/environment validation in `scripts/dev_planner/run_icra_p0_p5_qualification.py`; tests: `test/test_run_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra068/preflight/` and `results/icra27/icra068/live/runner_state.json` retain one preflight, one launch, zero retry and repository-local roots | **FAIL-CLOSED PRE-LIVE CONTRACT** |
| IAP-RQ-423 | Require exact install and registered fixture evidence | Implementation: manifest/fixture validation in `scripts/dev_planner/run_icra_p0_p5_qualification.py` and `launch/icra_p0_p5_qualification.py`; tests: `test/test_run_icra_p0_p5_qualification.py`, `test/test_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra068/icra068_install_manifest.json` and `results/icra27/icra068/live/runner_state.json` | **SECOND REVIEW BLOCKERS REMEDIATED** |
| IAP-RQ-423 | Bind exact event attribution and linkage identity | Implementation: live event/linkage checks in `launch/icra_p0_p5_qualification.py`; tests: `test/test_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra068/icra068_install_manifest.json`, `results/icra27/icra068/live/icra-p0-p5-live-safe-normal-001/launch_command.json` and `process_result.json` | **FINAL REVIEW BLOCKERS REMEDIATED** |
| IAP-RQ-423 | Execute one ordered live attempt per case and stop on first failure | Implementation: ordered orchestration in `scripts/dev_planner/run_icra_p0_p5_qualification.py`; tests: `test/test_run_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra068/live/icra-p0-p5-live-safe-normal-001/launch_command.json`, `stdout.log`, `process_result.json`, `results/icra27/icra068/live/runner_state.json` and `results/icra27/icra068/compact/final_result.json` prove GPU PASS, malformed `p1.debug_csv_path:=`, 0/16 children, `1/0/1/0`, no orphan/later arm/analyzer | **ICRA068 BLOCKED: RUNNER COMMAND SERIALIZATION; ICRA-069 REPAIR READY** |

## 2026-08-26 ICRA-069 replacement launch repair

| Req ID | Requirement/evidence seam | Implementation and retained evidence | Status |
|---|---|---|---|
| IAP-RQ-421, IAP-RQ-422, IAP-RQ-423 | Render parseable replacement commands without changing effective profile | Implementation: `scripts/dev_planner/run_icra_p0_p5_qualification.py::render_live_launch_command`; tests: `test/test_run_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra069/parse_only/parser_proof.json` contains the three exact argv and binds exactly 19 empty omissions, retained false/zero/nonempty values and `-002` IDs | **33/33 FOCUSED, 20/20 LAUNCH, 553/553 FULL PASS** |
| IAP-RQ-320, IAP-RQ-423 | Adopt unchanged product runtime with distinct runner provenance | Implementation: `build_adoption_payload`/`require_replacement_evidence_ready` in the live runner; tests: `test/test_run_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra069/compact/icra069_adoption_manifest.json` binds product commit `005ce1a`, manifest `7662a2c4...34d420`, runner commit `d982276`, 83 files/54 libraries and zero three-alias source overlap | **LIVE ADOPTION PASS; COMPLETE-INVENTORY PROOF STRENGTHENING REQUIRED** |
| IAP-RQ-423 | Prove real ROS parser acceptance before GPU or live registration | Implementation: `parse_only_command`, `_run_parse_only` and `run_parser_proofs` in `scripts/dev_planner/run_icra_p0_p5_qualification.py`; tests: `test/test_run_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra069/parse_only/parser_proof.json` plus three case directories record ordered exit `0/0/0`, zero required child/remnant and SHA `b2983502...922566` | **REAL INSTALLED PARSER PASS BEFORE GPU** |
| IAP-RQ-423 | Fail closed before the sole replacement analyzer invocation | Implementation: `require_replacement_evidence_ready()` and `reconcile_replacement_analysis()` in the live runner; tests: `test/test_run_icra_p0_p5_qualification.py`; evidence: `results/icra27/icra069/live/runner_state.json` and compact result prove analyzer invocation/marker `0/0` because runner state is FAILED | **FAIL-CLOSED PASS; 553/553 HERMETIC** |
| IAP-RQ-421, IAP-RQ-423 | Execute replacement qualification once and stop at the first technical failure | Implementation: `_execute_live_attempt`/`run` in the live runner; tests: `test/test_run_icra_p0_p5_qualification.py`; evidence: parser proof, `results/icra27/icra069/preflight/gpu_preflight.json`, SAFE `process_result.json`, runner state and compact result show 15/16, `test_planner_gnss_sim_node` absent, `1/0/1/0`, no orphan/later arm/analyzer | **ICRA069 IMPLEMENTATION PASS / GATE BLOCKED: QUALIFICATION CASES RESOLVE GNSS-DISABLED; REVISED ICRA-070 READY** |

## 2026-08-26 revised ICRA-070 full-sensor qualification authorization

| Req ID | Requirement/evidence seam | Authorized implementation and acceptance evidence | Status |
|---|---|---|---|
| IAP-RQ-020, IAP-RQ-030, IAP-RQ-040, IAP-RQ-220 | Prospective qualification must instantiate the specified GNSS pseudorange+doppler + IMU + LiDAR system | Keep all 16 processes; introduce one qualification-specific scenario composed from existing corridor geometry and degraded-GNSS presets; require GNSS/ARAIM and LiDAR integrity, `max_pl` fusion, positive GNSS diagnostic/range, IMU and LiDAR topic counts, and `n_sv_used>0` | **ICRA070 TASK_READY / first 15-process command superseded** |
| IAP-RQ-320, IAP-RQ-421, IAP-RQ-422 | P0 advisory qualification must use both live sources, not a LiDAR-only/fallback surrogate | Contract/launch/normalizer/analyzer tests and live evidence must prove worker 4, sigma 0.01, legacy baseline, valid/fresh GNSS epoch, `predictor_gnss_used_count>0`, `predictor_lidar_used_count>0` and `predictor_horizon_fusion_count>0` in every arm | **FULL-SENSOR FAIL-CLOSED GATE AUTHORIZED** |
| IAP-RQ-423 | Preserve P5 qualification behavior while correcting only its sensor binding | Keep existing route geometry, P5-6/P5-7 fixture values, thresholds/actions/event ordering and `-003` identities; use static dependency/contract proof, no-compile overlay, parser `0/0/0`, one GPU preflight, exactly three ordered one-shot arms and one analyzer | **REVISED ICRA070 TASK_READY** |

## 2026-08-26 ICRA-070 Builder implementation

| Requirement | Verification target | Implementation / test | Status |
|---|---|---|---|
| IAP-RQ-020 / IAP-RQ-030 / IAP-RQ-040 | Instantiate GNSS pseudorange+doppler, IMU and LiDAR in every qualification arm | `icra_p0_p5_fused_degraded_corridor_v1` in `launch/test_planner.launch.py`; frozen `qualification_values` and ten required topics in the canonical contract; launch/contract regressions | **IMPLEMENTED; STATIC PASS** |
| IAP-RQ-220 / IAP-RQ-320 | Prove fresh valid GNSS plus positive GNSS/LiDAR/fused predictor use and satellites | fail-closed analyzer gates in `launch/icra_p0_p5_qualification.py`; exact scenario/RINEX/trigger/fallback=false argv; raw normalizer gates and hash-consistent adversaries | **IMPLEMENTED; 46/46 FOCUSED PASS** |
...
| Requirement | Verification target | Implementation / test | Status |
|---|---|---|---|
| IAP-RQ-000 / IAP-RQ-423 | Isolated Git identity succeeds without persistent trust mutation | `trusted_git`, reviewed-HEAD/clean-worktree preflight and the real recorded-`HOME` subprocess regression in `test_run_icra_p0_p5_qualification.py`; negative cases include missing/mismatched trust, relative/aliased/alternate repository, changed HEAD and dirty tracked status | **IMPLEMENTED; RAW GIT 128 / COMMAND-LOCAL TRUST PASS** |
| IAP-RQ-423 | Replacement contains every base non-cache file, only three current aliases, exact modes and independent bytes | `construct_complete_overlay`, `audit_complete_overlay`; adversaries cover missing/extra/mode/binary/alias/cache/source-cache/symlink/hard-link/partial/pre-existing evidence/second construction | **IMPLEMENTED; 57/57 RUNNER PASS** |
