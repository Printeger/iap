...
| IAP-RQ-423 | Prove isolated current runtime closure | 17/17 sequential merged non-symlink Release/CUDA packages; final manifest `7662a2c4...34d420` binds commit `005ce1a`, 18 package identities, 54 task-local libraries, 83 file hashes, three aliases, clean linkage and exact prefixes | **BUILD / FINAL MANIFEST PASS** |
| IAP-RQ-423 | Bind full live ownership and real evidence | `run_icra_p0_p5_qualification.py`, canonical contract and analyzer bind 16 child identities, three exact topics, bag/P0/P5/bspline sources, controlled shutdown, fixed ordered IDs and zero retries | **IMPLEMENTED; 23/23 FOCUSED TESTS PASS** |
| IAP-RQ-423 | Reject synthetic qualification claims | `analyze_live_bundle` rejects `validation_only=true`; full hermetic discovery passes 543/543 with the 17,762-entry external inventory unchanged | **FAIL-CLOSED OFFLINE PASS** |
| IAP-RQ-423 | Fail closed on early launch exit, orphan and ambiguous lifecycle evidence | task-owned process-group shutdown/audit, exact per-process rows, `process_result.json` and complete bag payload hashes are independently checked by normalizer/analyzer | **PRE-LIVE REVIEW BLOCKERS REMEDIATED** |
| IAP-RQ-423 | Preserve single-call and isolated-environment boundaries | exact repository-local HOME/ROS/TMP/XDG plus ICRA-068/GLIM/Jazzy prefixes are required before GPU; existing preflight/live/analyzer markers reject overwrite or retry | **FAIL-CLOSED PRE-LIVE CONTRACT** |
| IAP-RQ-423 | Require exact install and registered fixture evidence | complete task-local shared-library inventory and manifest SHA are independently rebound; runtime failure contradictions and coincidental non-fixture rejection/emergency evidence fail closed | **SECOND REVIEW BLOCKERS REMEDIATED** |
| IAP-RQ-423 | Bind exact event attribution and linkage identity | selected rejection/emergency rows must themselves carry the registered fixture sample; canonical address-independent `ldd` SHA is compared for every inventoried library | **FINAL REVIEW BLOCKERS REMEDIATED** |
| IAP-RQ-423 | Execute one ordered live attempt per case and stop on first failure | sole GPU preflight PASS; SAFE_NORMAL launch exits 1 on malformed empty `p1.debug_csv_path:=` before 0/16 required processes start; runner attempted/completed/launch/retry = 1/0/1/0, orphan audit PASS, later identities and analyzer invocation = 0 | **BLOCKED_ICRA068_SAFE_NORMAL_MALFORMED_LAUNCH_ARGUMENT; SUPERVISOR REVIEW REQUIRED** |
