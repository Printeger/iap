...
| IAP-RQ-423 | Deny unclassified filesystem/process-output operations in every reachable scope | Module/sync/async/nested scopes, aliases, explicit os/shutil/pathlib/Path sets, five subprocess helpers and 32 mutation cases are covered; canonical-child, positional-stream, final/dynamic-flag and unknown nested-namespace adversaries reject; exact 50-record/five-root/four-action/eight-output production contract is validated without filtering extras | **FAIL-CLOSED; synthetic only** |
