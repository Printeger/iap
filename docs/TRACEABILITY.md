...
- 2026-03-29: IAP-RQ-300 / IAP-RQ-410
  ...
  - `ActiveSplineSegmentConstraint` 已进一步删掉 runtime `target_mode / target_frame_count / target_point_count / snapshot_* / target_build_ms` 镜像字段；segment 只保留 authoritative `target_handle`，LiDAR factor cache key、trace log、degeneracy warning 与 pipeline timing 全部改为直接从 `ISharedTargetHandle` 读取 target metadata。
  - 这样进一步消除了“segment metadata 看起来一致，但 factor 实际绑定的 target entity 已经漂移”的双重 ownership 风险，并把 per-segment target 语义彻底收口到 shared-handle 层。
