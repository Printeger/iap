...
| IAP-RQ-020 | GNSS 紧耦合观测：伪距 + 多普勒建因子（含 clock bias/drift，per-sat） | GNSS tightly-coupled | `include/iap/gnss/`, `src/iap/gnss/` (PseudorangeFactor, DopplerFactor, GnssHandler, **GnssExtensionModule** ROS2 bridge) | 关闭/开启 GNSS 对比 | `res_pr, res_dop, clk, clk_dot` | **DONE** |
