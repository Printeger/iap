...
| IAP-RQ-300 | Hybrid CT architecture boundary for local frontend + compact backend | this plan | `ct_local_frontend.*`, `ct_compact_backend.*`, `odometry_estimation_bspline.*` | `test_ct_local_frontend` (2), `test_ct_compact_backend` (2), `test_ct_hybrid_pipeline` (4) | local solve size, backend factor count, GNSS/backend counts | **LOCKED** |
