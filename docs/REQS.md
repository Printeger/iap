...
> Current ICRA disposition (2026-08-27): user decision 006 accepts and bypasses ICRA-075's frozen incompatibility,
> 0/40, absent power inputs and two P1 defects; ICRA-075 remains BLOCKED/NOT PASS. ICRA-076 is TASK_READY for
> outcome-blind preregistration and exact source/install freeze only. Without empirical power inputs it must use
> conservative `n=60` per scene and make no demonstrated-power claim. Held-out ICRA-077 and campaign remain blocked.
...
- [ ] Retain ICRA-072B as blocked stabilization debt under user decision 003; do not relabel it PASS or use it as
  scientific/qualification evidence while ICRA-073 proceeds.
...
ICRA-072B Supervisor Review (2026-08-27): Builder HEAD `a63d3cc` is `REQUEST_CHANGES`. The shared six-package
build and all four production/P4/P5 C++ suites are green, but the retained canonical result does not satisfy the
Layer 2 exit contract: hermetic Git ownership admission makes tools exit 1 and three rows fail. The runner also
counts skipped verbose Python unittests as observed, leaving required-row skip admission fail-open. Continue the
same Layer 2 Gate with only exact non-persistent command-local Git trust and Python/gtest skip/disabled TDD; push
...
User-directed transition (2026-08-27): the user explicitly requires ICRA-073 to proceed regardless of the
ICRA-072B blocker. Decision `USER-ICRA-ROUTE-20260827-003`, anchored at pushed Builder HEAD `a30468e`, retains
ICRA-072B as BLOCKED/NOT PASS and accepts its source-admission/stabilization debt for Layer 3 only. ICRA-073 now
