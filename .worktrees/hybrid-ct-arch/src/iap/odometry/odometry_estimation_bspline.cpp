...
// spline window path. Follow-up work will move LiDAR/IMU local factor assembly
// behind `CTLocalFrontend`, while GNSS/shared-state/mapping orchestration stays
// here on the compact backend side. For this commit we only freeze the
// migration boundary and CPU-first frontend default; math behavior, residual
// models, public interfaces, plugin names, shared-state integration, ROS
// topics, and log keywords stay unchanged.
