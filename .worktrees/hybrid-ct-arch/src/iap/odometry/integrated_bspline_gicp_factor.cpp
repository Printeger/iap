...
IntegratedSplineGICPFactor::IntegratedSplineGICPFactor(
...
: gtsam::NonlinearFactor(gtsam::KeyVector(ctx.support.pose_keys.begin(), ctx.support.pose_keys.end())),
  ...
  target_(target),
  source_(source),
  basis_weights_(BSplineControlWindow::basis(ctx.support.u)) {
  if (!frame::has_points(*target_) || !frame::has_covs(*target_)) {
    throw std::runtime_error("IntegratedSplineGICPFactor requires target points and covariances");
  }
  if (!frame::has_points(*source_) || !frame::has_covs(*source_) || !frame::has_times(*source_)) {
    throw std::runtime_error("IntegratedSplineGICPFactor requires source points, covariances, and times");
  }
...
std::array<gtsam::Pose3, kBSplineControlPointCount> IntegratedSplineGICPFactor::control_poses(const gtsam::Values& values) const {
...
double IntegratedSplineGICPFactor::inlier_fraction() const {
  return ctx_.point_indices.empty() ? 0.0 : static_cast<double>(accepted_inlier_count_) / ctx_.point_indices.size();
}

void IntegratedSplineGICPFactor::update_pose(const gtsam::Values& values) const {
  ...
  if (jacobian_mode_ == JacobianMode::NUMERIC_FULL) {
    for (std::size_t k = 0; k < kBSplineControlPointCount; ++k) {
      ...
      for (int d = 0; d < 6; ++d) {
        ...
        const gtsam::Pose3 pose_plus = BSplineControlWindow::interpolate(perturbed, ctx_.support.u);
        const gtsam::Vector6 xi = gtsam::Pose3::Logmap(bucket_pose_.between(pose_plus));
        J.col(d) = xi / numeric_eps_;
      }
      pose_jacobians_[k] = J;
    }
    blended_quaternion_coeffs_.setZero();
    return;
  }
  ...
  for (std::size_t k = 0; k < kBSplineControlPointCount; ++k) {
    ...
    aligned[k] = q;
    blended_coeffs += basis_weights_[k] * q.coeffs();
  }
  ...
  blended_quaternion_coeffs_ = normalized_coeffs;

  for (std::size_t k = 0; k < kBSplineControlPointCount; ++k) {
    blend_quaternion_jacobians_[k] =
      normalization_jacobian * (basis_weights_[k] * quaternion_right_local_jacobian(aligned[k]));
  }
}

void IntegratedSplineGICPFactor::update_correspondences() const {
  const std::size_t count = ctx_.point_indices.size();
  correspondences_.resize(count);
  mahalanobis_.resize(count);
  correspondence_sq_distances_.resize(count);
  correspondence_best_scores_.resize(count);
  correspondence_second_best_scores_.resize(count);
  matched_correspondence_count_ = 0;
  rejected_distance_count_ = 0;
  rejected_ambiguity_count_ = 0;
  candidate_evaluation_count_ = 0;

  for (std::size_t local_idx = 0; local_idx < count; ++local_idx) {
    const int point_index = ctx_.point_indices[local_idx];
    const auto& source_pt = frame::point(*source_, point_index);

    const gtsam::Point3 transed_pt = bucket_pose_ * source_pt.template head<3>();
    ...
    if (num_found == 0) {
      correspondences_[local_idx] = -1;
      mahalanobis_[local_idx].setZero();
      correspondence_sq_distances_[local_idx] = std::numeric_limits<double>::max();
      correspondence_best_scores_[local_idx] = std::numeric_limits<double>::max();
      correspondence_second_best_scores_[local_idx] = std::numeric_limits<double>::max();
      rejected_distance_count_++;
      continue;
    }

    const auto& source_cov = frame::cov(*source_, point_index);
    const Eigen::Matrix4d pose_matrix = bucket_pose_.matrix();
    ...
    if (correspondence_accept_ratio_ > 0.0 &&
      ...
      correspondences_[local_idx] = -1;
      mahalanobis_[local_idx].setZero();
      correspondence_sq_distances_[local_idx] = best_sq_dist;
      correspondence_best_scores_[local_idx] = best_score;
      correspondence_second_best_scores_[local_idx] = second_best_score;
    ...
    correspondences_[local_idx] = best_index;
    mahalanobis_[local_idx] = best_mahalanobis;
    correspondence_sq_distances_[local_idx] = best_sq_dist;
    correspondence_best_scores_[local_idx] = best_score;
    correspondence_second_best_scores_[local_idx] = second_best_score;
...
IntegratedSplineGICPFactor::PointJacobianArray IntegratedSplineGICPFactor::point_jacobians(
  ...
  if (jacobian_mode_ == JacobianMode::NUMERIC_FULL) {
    ...
    bucket_pose_.transformFrom(source_point, H_transed_pose);

    for (std::size_t k = 0; k < kBSplineControlPointCount; ++k) {
      jacobians[k] = H_transed_pose * pose_jacobians_[k];
    }
    return jacobians;
  }

  const Eigen::Matrix<double, 3, 4> H_transed_quat =
    rotated_point_quaternion_jacobian(blended_quaternion_coeffs_, source_point);

  for (std::size_t k = 0; k < kBSplineControlPointCount; ++k) {
    jacobians[k].setZero();
    jacobians[k].block<3, 3>(0, 0) = H_transed_quat * blend_quaternion_jacobians_[k];
    jacobians[k].block<3, 3>(0, 3) = basis_weights_[k] * control_poses[k].rotation().matrix();
  }

  return jacobians;
}

void IntegratedSplineGICPFactor::update_profiling_stats(
  const char* stage,
  ...
  last_profile_.source_point_count = ctx_.point_indices.size();
  last_profile_.target_point_count = target_->voxel_points().size();
  last_profile_.time_bucket_count = 1;
  last_profile_.max_time_bucket_population = ctx_.point_indices.size();
  ...
  last_profile_.mean_time_bucket_population = static_cast<double>(ctx_.point_indices.size());
...
double IntegratedSplineGICPFactor::robust_cost(const double mahalanobis_error) const {
...
double IntegratedSplineGICPFactor::robust_weight(const double mahalanobis_error) const {
  const double residual_norm = residual_norm_from_mahalanobis(mahalanobis_error);
...
double IntegratedSplineGICPFactor::error(const gtsam::Values& values) const {
  ...
  for (std::size_t local_idx = 0; local_idx < ctx_.point_indices.size(); ++local_idx) {
    const long target_index = correspondences_[local_idx];
    if (target_index < 0) {
      continue;
    }
    ...
    const gtsam::Point3 transed_source_pt = bucket_pose_.transformFrom(source_pt.template head<3>().eval());
    const Eigen::Vector3d residual = transed_source_pt - target_pt.template head<3>();
    const double mahalanobis_error = residual.transpose() * mahalanobis_[local_idx] * residual;
...
gtsam::GaussianFactor::shared_ptr IntegratedSplineGICPFactor::linearize(const gtsam::Values& values) const {
  using Clock = std::chrono::steady_clock;
  const auto t_start = Clock::now();
  update_pose(values);
  const auto t_pose = Clock::now();
  ...
  for (std::size_t local_idx = 0; local_idx < ctx_.point_indices.size(); ++local_idx) {
    ...
    const gtsam::Point3 transed_source_pt = bucket_pose_.transformFrom(source_pt.template head<3>().eval());
    const Eigen::Vector3d residual = transed_source_pt - target_pt.template head<3>();
    const Eigen::Matrix3d& M = mahalanobis_[local_idx];
    ...
    const auto point_jacs = point_jacobians(poses, source_pt.template head<3>().eval());
    for (std::size_t k = 0; k < kBSplineControlPointCount; ++k) {
...
IntegratedSplineGICPFactor::LinearizationCheckResult IntegratedSplineGICPFactor::check_linearization(
...
IntegratedSplineGICPFactor::NumericReferenceCheckResult IntegratedSplineGICPFactor::check_against_numeric_full(
  const gtsam::Values& values,
  double perturbation_scale) const {
  ...
  auto configure_factor = [&](JacobianMode mode) {
    IntegratedSplineGICPFactor factor(ctx_, target_, source_, target_tree_);
...
IntegratedSplineGICPFactor::DegeneracyDiagnostics IntegratedSplineGICPFactor::diagnose_degeneracy(
...
BSplineLidarFactorProfile IntegratedSplineGICPFactor::profiling_report() const {
...
BSplineLidarFactorResult IntegratedSplineGICPFactor::make_result(
  double factor_error,
  int inlier_count,
...
std::vector<Eigen::Vector4d> IntegratedSplineGICPFactor::deskewed_source_points(const gtsam::Values& values, bool local) const {
  update_pose(values);
