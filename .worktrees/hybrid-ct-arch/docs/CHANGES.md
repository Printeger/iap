...
## Unreleased
- plan(dev-ct-hybrid-architecture-boundary): IAP-RQ-300 / IAP-RQ-410 — freeze the next refactor boundary around a hybrid CT architecture before changing runtime ownership.
  - The intended split is now explicit in project docs: a local frontend owns dense LiDAR/IMU continuous-time estimation, while a compact backend owns GNSS, shared navigation states, mapping/publication handoff, and carried priors over summarized state only.
  - CPU remains the mainline frontend path, GPU `BUCKET` remains the supported accelerated variant, and GPU `KERNEL` stays experimental outside the validated architecture baseline.
- feat(dev-ct-local-frontend-stub): IAP-RQ-300 / IAP-RQ-410 — add the first executable local-frontend handoff stub and empty-input contract.
  - Added `ct_local_frontend.cpp` and wired it into the `iap` library with a minimal `CTLocalFrontend::run(...)` implementation that returns an empty compact handoff without changing runtime algorithms.
  - Extended `test_ct_local_frontend` with an empty-input regression so the hybrid frontend boundary now has a first executable contract beyond type-only compilation.
  - Tightened top-level migration comments in the BSpline odometry headers/source to state that LiDAR/IMU local solve ownership moves behind `CTLocalFrontend`, while GNSS/shared-state/mapping remain backend-orchestrated.
- feat(dev-ct-local-frontend-counts): IAP-RQ-300 / IAP-RQ-410 — expose minimal frontend-owned LiDAR/IMU counts on the handoff surface.
  - `CTLocalFrontend::Input` now carries `imu_sample_count`, and `CTLocalFrontendResult` now reports `lidar_source_frame_count` plus `imu_sample_count` so the frontend boundary can describe local ownership without exposing factor internals.
  - `CTLocalFrontend::run(...)` now forwards those counts into the result surface while keeping the backend summary compact and empty in this stub phase.
  - `test_ct_local_frontend` now locks this contract down with both empty-input and non-empty-count regressions.
- feat(dev-ct-compact-backend-stub): IAP-RQ-300 / IAP-RQ-410 — add the first executable compact-backend interface and pipeline contracts.
  - Added `ct_compact_backend.hpp/.cpp` with a minimal compact backend result that preserves only compact frontend summary state plus backend-owned GNSS/carried-prior counters.
  - Added `test_ct_compact_backend` and `test_ct_hybrid_pipeline` contract tests so the planned frontend→backend split now has executable coverage on both sides of the handoff.
  - `odometry_estimation_bspline.hpp` now includes `CTLocalFrontend` and `CTCompactBackend` members as the planned orchestrator boundary without changing the active runtime path.
- docs(dev-ct-hybrid-validation-notes): IAP-RQ-300 / IAP-RQ-410 — document the current stub-only validation boundary for the hybrid CT split.
  - README developer notes now state that the current hybrid CT surface is covered by contract tests only and does not yet move real factor assembly behind the new interfaces.
  - README also records that validation in this environment is currently limited to focused compile/harness checks plus those GTest source contracts because standalone worktree configure is blocked by a missing ROS message artifact.
...
- feat(dev-ct-active-state-marginalization): IAP-RQ-020 / IAP-RQ-300 / IAP-RQ-410 — rewrite fixed-lag marginalization around active spans / active keys instead of legacy segment ownership.
  ...
  - Active-window pruning now retains control-buffer states and auxiliary values through `prune_to_active_state_set(...)`, preventing multi-span bucket/support references from dropping still-live control points while keeping the carried-prior pipeline unchanged.
