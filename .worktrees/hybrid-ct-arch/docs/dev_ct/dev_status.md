...
- 当前 hybrid CT 目标边界已冻结：
  - local frontend 负责 LiDAR registration、IMU sample fitting、local spline control update，以及 dense per-bucket factor construction
  - compact backend 负责 GNSS、shared navigation states、mapping/publication handoff，以及仅基于 summarized state 的 carried priors
  - CPU frontend + optional GPU `BUCKET` acceleration 是当前主线目标
  - GPU `KERNEL` 继续保持 experimental，不定义主线架构边界
- 当前 `src/iap` 已具备：
  - 连续时间轨迹统一接口
