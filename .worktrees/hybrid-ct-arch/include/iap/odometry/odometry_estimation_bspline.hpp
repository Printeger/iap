...
// window as its operational path. Follow-up commits will migrate LiDAR/IMU
// local assembly behind `CTLocalFrontend`, while keeping GNSS/shared-state /
// mapping-facing ownership on the compact backend/orchestrator side. This
// commit intentionally does not change algorithms, residuals, public
// interfaces, ROS/plugin wiring, or logging keywords; it only freezes the
// migration boundary and the CPU-first rollout order.
...
class OdometryEstimationBSpline : public OdometryEstimationCPU {
 ...
 private:
  ...
  // Planned hybrid split: odometry becomes the orchestrator that bridges
  // `CTLocalFrontend` and `CTCompactBackend` while preserving the current
  // runtime path until the local/backend logic is migrated behind those types.
