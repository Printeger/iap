...
- Hybrid CT stub validation: the current planned boundary now has contract tests for `CTLocalFrontend`, `CTCompactBackend`, and a minimal frontend→backend pipeline handoff, but it is still stub-only and does not yet move real LiDAR/IMU/GNSS assembly behind those interfaces.
- Because standalone worktree configure is currently blocked by a missing ROS message artifact in this environment, the current regression gate remains focused compile/harness checks plus the GTest source contracts above.
