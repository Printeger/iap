...
## Unreleased
- fix(ct-compact-backend-quality): IAP-RQ-300 / IAP-RQ-410 — fix code quality issues in CTCompactBackend.
  - CRITICAL: Use stable auxiliary index from `active_control_indices.back()` for `bspline_clock_key` / `bspline_velocity_key` instead of loop counter `epoch_index`, preventing key aliasing across calls.
  - CRITICAL: Seed `velocity_key` into values before constructing Doppler factor (was missing, causing undefined key reference).
  - IMPORTANT: Fix PR sigma formula to `std::max({1e-3, sat.pr_sigma, input.gnss_pr_noise_base})` matching reference pattern.
  - IMPORTANT: Fix Doppler sigma formula to elevation-dependent model `dop_noise_base / pow(max(0.052, sin_el), elev_noise_exp)` matching reference pattern.
  - IMPORTANT: Add `gnss_elev_noise_exp{2.0}` to `Input` struct.
  - IMPORTANT: Remove stale `last_gnss_factor_count_ = 0` reset on null-pointer early return.
  - MINOR: Add comment explaining layout copy into `shared_ptr<const SplineStateLayout>`.
  - MINOR: Strengthen test assertion to `EXPECT_EQ(graph.size(), 2U)` and add `NullGraphOrValuesIsNoOp` test.
