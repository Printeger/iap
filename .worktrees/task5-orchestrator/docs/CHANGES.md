...
## Unreleased
- test(ct-hybrid-pipeline-nontrivial): IAP-RQ-300 / IAP-RQ-410 — add two non-trivial pipeline tests to test_ct_hybrid_pipeline.cpp.
  - Test 5 (FrontendWithSourceFramesProducesNonEmptySummary): exercises CTLocalFrontend::run() with a real EstimationFrame target and a RawPoints source frame; asserts layout controls/knots non-empty, local_values seeded, has_bias_state true, and backend update with no GNSS anchor produces empty graph.
  - Test 6 (BackendWithGnssAnchorAddsPseudorangeAndDopplerFactors): exercises CTCompactBackend::update() with a GNSS-initialized anchor and one satellite epoch; asserts exactly 2 factors (1 PR + 1 Doppler) added to graph and gnss_factor_count == 2.
  - Added includes: estimation_frame.hpp, raw_points.hpp, gtsam/geometry/Rot3.h, Eigen/Core, memory.
- feat(ct-orchestrator): IAP-RQ-300 / IAP-RQ-410 — wire CTLocalFrontend and CTCompactBackend into insert_frame_ct_lidar orchestration boundary (Task 5).
  - Replaced three `// Future:` comment stubs in `insert_frame_ct_lidar` with real calls: `make_frontend_input()` → `ct_local_frontend_.run()` at the top of the function, and `make_backend_input()` → `ct_compact_backend_.update()` after the monolithic LM solve.
  - Added private `make_frontend_input(const PreprocessedFrame::Ptr&) const` helper that populates `CTLocalFrontend::Input` from the current frame history and raw scan.
  - Added private `make_backend_input(const CTLocalFrontendResult&) const` helper that populates `CTCompactBackend::Input` from the fixed-lag registry's GNSS epochs and shared ECEF anchor state.
  - The monolithic solver path (LiDAR/IMU factor assembly + LM solve) remains intact and continues to run as the active runtime path; the orchestration call runs in addition to it.
  - Replaced stub `test_ct_hybrid_pipeline.cpp` with four real pipeline tests exercising the actual `CTLocalFrontend::run()` → `CTCompactBackend::update()` call chain.
