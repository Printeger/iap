...
| IAP-RQ-300 | Hybrid CT architecture boundary for local frontend + compact backend | this plan | `ct_local_frontend.*`, `ct_compact_backend.*` (`Input` struct, `update()`, `last_gnss_factor_count_`), `odometry_estimation_bspline.*` (`make_frontend_input`, `make_backend_input`, orchestration calls in `insert_frame_ct_lidar`) | `test_ct_local_frontend` (2), `test_ct_compact_backend` (5: 2 contract + 3 update), `test_ct_hybrid_pipeline` (6: FrontendResultFlowsToBackendUpdate, BackendNeverAddsRawLidarFactors, GnssStaysBackendSideInAllVerifiedModes, BackendGraphStaysSmallerThanFrontendAssembly, FrontendWithSourceFramesProducesNonEmptySummary, BackendWithGnssAnchorAddsPseudorangeAndDopplerFactors) | local solve size, backend factor count, GNSS/backend counts | **DONE** |
