...
if [ -e "$GUARD" ]; then printf 'analyzer_already_invoked\n'; exit 64; fi
printf 'invoked_once=1\n' > "$GUARD"
printf '%s\n' \
  'python3 scripts/dev_planner/gate0_analyzer.py --gate0-root results/icra27/icra031/runs --output-dir results/icra27/icra031/runs/smoke/analyzer' \
  > "$TASK_ROOT/smoke_analyzer_invocation.txt"
python3 scripts/dev_planner/gate0_analyzer.py \
  --gate0-root results/icra27/icra031/runs \
  --output-dir results/icra27/icra031/runs/smoke/analyzer \
