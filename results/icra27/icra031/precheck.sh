...
ATTEMPT_ROOT=$TASK_ROOT/prechecks/${ICRA031_PRECHECK_ATTEMPT:-attempt_01}
IAP_BUILD=$TASK_ROOT/build_iap
IAP_INSTALL=$TASK_ROOT/install
PLAN_BUILD=$REPO_ROOT/results/icra27/icra026/build_plan_env
PLAN_INSTALL=$REPO_ROOT/results/icra27/icra026/install_plan_env
PATH_BUILD=$REPO_ROOT/results/icra27/icra026/build_path_searching
PATH_INSTALL=$REPO_ROOT/results/icra27/icra026/install_path_searching
BSPLINE_BUILD=$REPO_ROOT/results/icra27/icra026/build_bspline_opt
BSPLINE_INSTALL=$REPO_ROOT/results/icra27/icra026/install_bspline_opt
EGO_BUILD=$REPO_ROOT/results/icra27/icra026/build_ego
EGO_INSTALL=$REPO_ROOT/results/icra27/icra026/install_ego
...
run_check artifact_mapping bash -c '
  test -d results/icra27/icra031/install
  test -L results/icra27/icra031/install_ego
  test "$(readlink -f results/icra27/icra031/install)" = /home/dev/ws_iap/src/iap/results/icra27/icra031/install
  test "$(readlink -f results/icra27/icra031/install_ego)" = /home/dev/ws_iap/src/iap/results/icra27/icra026/install_ego
  printf "iap_mapping=%s\nego_mapping=%s\n" "$(readlink -f results/icra27/icra031/install)" "$(readlink -f results/icra27/icra031/install_ego)"
'
run_check installed_launch_exact bash -c '
  cmp launch/test_planner.launch.py results/icra27/icra031/install/share/iap/launch/test_planner.launch.py
  sha256sum launch/test_planner.launch.py results/icra27/icra031/install/share/iap/launch/test_planner.launch.py
'
run_check python_compile python3 -m py_compile launch/test_planner.launch.py scripts/dev_planner/run_gate0_qualification.py
...
run_check ldd_direct_consumers bash -c '
  for binary in "$@"; do printf "BINARY=%s\n" "$binary"; ldd "$binary" || exit $?; done
' _ \
...
run_check linkage_assertion python3 "$TASK_ROOT/linkage_precheck.py" \
  "$ATTEMPT_ROOT/ldd_manager.log" "$ATTEMPT_ROOT/ldd_demo11.log" \
  "$ATTEMPT_ROOT/ldd_direct_consumers.log" "$ATTEMPT_ROOT/linkage.json" \
  "$IAP_INSTALL/lib/libiap.so" "$PLAN_INSTALL/lib/libplan_env.so" \
...
run_check external_log_snapshot bash -c '
  ...
  count=$(wc -l < "$output"); bytes=$(du -sb log | awk "{print \$1}")
...
run_check process_precheck bash -c '
  ...
  for proc_path in /proc/[0-9]*; do
    pid=${proc_path#/proc/}; [ -n "${excluded[$pid]:-}" ] && continue
