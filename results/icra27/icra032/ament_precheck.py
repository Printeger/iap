...
required = (
    "iap", "ego_planner", "plan_env", "path_searching", "bspline_opt",
    "local_sensing", "odom_visualization", "poscmd_2_odom", "gnss_sim",
    "so3_quadrotor_simulator", "so3_control", "rclcpp_components",
)
expected = {
    "iap": Path("results/icra27/icra032/install").resolve(),
    "ego_planner": Path("results/icra27/icra032/install_ego").resolve(),
    "plan_env": Path("results/icra27/icra026/install_plan_env").resolve(),
    "path_searching": Path("results/icra27/icra026/install_path_searching").resolve(),
    "bspline_opt": Path("results/icra27/icra026/install_bspline_opt").resolve(),
}
raw = os.environ.get("AMENT_PREFIX_PATH", "")
prefixes = [str(Path(value).resolve()) for value in raw.split(":") if value]
failures = []
packages = []
for package in required:
    ...
    active = resolved is not None and str(resolved) in prefixes
    ...
    packages.append({
        ...
        "in_active_ament_prefix_path": active,
        "matches_exact_task_artifact": exact,
        "prefix_exists": exists,
        "error": error,
    })
result = {
    "ament_prefix_path_raw": raw,
